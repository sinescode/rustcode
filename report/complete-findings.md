...
- **Description**: All 95 modules in `rustcode-core` are `pub mod` — zero visibility filtering. Every module is accessible to every consumer.
...
- **Severity**: Critical
- **Location**: `rustcode/crates/rustcode-core/src/lib.rs:1-95`, `rustcode/src/main.rs:1-8575`
- **Description**: All 95 modules in `rustcode-core` are flat-scoped public modules. The binary (`main.rs, 8,575 lines`) directly imports and uses `sqlx`, `rustcode_core::config`, `rustcode_core::*`, and embeds business logic. The provider module references `database.rs`, `config.rs`, `tool.rs` — a dense dependency web.
- **Consequence**: High coupling makes RustCode brittle. A change to `config.rs` can ripple through all 94 other modules. Testing any module in isolation requires importing the entire core crate.
- **Recommendation**: Introduce trait-based dependency inversion within `rustcode-core`. Split into multiple crates.
...
- **Description**: Dependency rule is violated: `rustcode-core` (inner layer) imports `sqlx`, `reqwest`, `serde_json`, `tracing` — infrastructure concerns. Core code directly constructs HTTP clients and makes network requests in provider code. `rustcode-core` has `pub mod database` with SQLite schema definitions and queries inline.
- **Consequence**: Impossible to swap infrastructure (e.g., SQLite to PostgreSQL, reqwest to hyper) without modifying core code. Business logic is polluted with serialization formats, HTTP status codes, and SQL queries. This is the single largest architectural debt in RustCode.
- **Recommendation**: Invert all dependencies. Define `Database` trait in core, implement in `rustcode-database-sqlite`. Define `HttpClient` trait in core, implement with `reqwest` in `rustcode-http`.
...
- **Description**: RustCode has 5 crates (4 of which are stubs) vs OpenCode's 26 packages. No infrastructure crates (database, http, filesystem), no plugin SDK crate, no event-store crate, no CLI library crate, no schema/migration crate.
- **Consequence**: Build times degrade as core grows. No reuse path (cannot publish equivalent of `@opencode-ai/plugin`). Third-party contributions are harder.
...
- **Severity**: Critical
- **Location**: `rustcode/src/main.rs:1-8575`
- **Description**: RustCode's binary is a monolith (8,575 lines) that merges CLI argument parsing, business logic, and infrastructure. The binary depends on wrapper crates but the wrapper crates are stub-level thin. The main.rs directly imports `rustcode_core::config::Config`, `sqlx`, `rustcode_core::*`, blurring the boundary.
- **Consequence**: Makes testing, swapping, and parallel development harder. RustCode has effectively 1.5 layers (core + thin wrappers) vs OpenCode's 4+ layers.
- **Recommendation**: Move logic from main.rs into `rustcode-core` or the appropriate wrapper crate. Make main.rs a thin CLI dispatch (<500 lines).
...
- **Description**: OpenCode V2 has a mature domain model with explicitly separated concerns: System Context algebra (epoch, baseline, snapshot, mid-conversation messages), EventV2 event sourcing, Location-scoped services, Account identity domain. RustCode lacks System Context (context source, epoch, and reconciliation concepts are absent), EventV2 (the event-sourcing architecture with replayable event streams is not present), Location (the Location-scoped service pattern is not yet represented), and Account (account module exists but without the full Identity domain).
- **Consequence**: RustCode will diverge in capabilities as OpenCode's V2 architecture matures. The System Context algebra (CONTEXT.md, 129 rules) is the core of OpenCode's session intelligence — without it, RustCode cannot match session behavior.
...
- **Description**: RustCode has good port/adapter for LLM providers (Provider trait with ProviderCatalog holding `Box<dyn Provider>`; OpenAICompatibleProvider with 14 pre-configured profiles). However, the database (`sqlx`), HTTP server (`axum`), and filesystem access are imported directly in `main.rs` and throughout `rustcode-core`. There is no infrastructure abstraction layer — core code directly calls SQL queries and filesystem operations.
- **Consequence**: Testing RustCode requires real infrastructure (SQLite files, real filesystem, network). OpenCode's hexagonal architecture allows each domain to be tested with in-memory or mock adapters.
...
- **Severity**: High
- **Location**: `rustcode/crates/rustcode-core/src/database.rs`, `rustcode/crates/rustcode-core/src/filesystem.rs`
...
- **Severity**: High
- **Location**: `tool.rs:47`
- **Description**: `ToolContext` stores `Vec<crate::provider::ChatMessage>` — a full clone of the message history into every tool invocation. Messages come from the session, passed by reference in OpenCode.
...
- **Severity**: High
- **Location**: `rustcode-core/src/auth.rs:195-206`
- **CWE**: CWE-522
...
- **Severity**: High
- **Location**: `rustcode-core/src/mcp.rs:2263-2276`
- **CWE**: CWE-312
...
- **Severity**: High
- **Location**: `rustcode-core/src/credential.rs:352-353`
- **CWE**: CWE-312
...
- **Severity**: High
- **Location**: `rustcode-core/src/encryption/hmac.rs` — file does not exist
- **CWE**: CWE-1240
- **Description**: HMAC-based encryption module from OpenCode not implemented. The file does not exist despite being declared in the module tree.
...
- **CWE**: CWE-1104
- **Description**: A RUSTSEC advisory is ignored without documented rationale. Must investigate: which crate is affected, is it exploitable in RustCode's usage context?
...
- **Severity**: High
- **Location**: `rustcode-core/src/config.rs:2796-2789`
- **CWE**: CWE-73
...
- **Severity**: Medium
- **Location**: `rustcode-core/src/providers/*:resolve_api_key()` (all providers)
- **CWE**: CWE-257
...
- **Severity**: Medium
- **Location**: `rustcode-core/src/config.rs:2505-2515`
- **CWE**: CWE-20
...
- **Severity**: Medium
- **Location**: `rustcode-core/src/config.rs:2804-2818`
- **CWE**: CWE-73
...
- **Severity**: Medium
- **Location**: `rustcode-core/src/config.rs:559-566`
- **CWE**: CWE-1104
...
- **Severity**: Medium
- **Location**: `rustcode-core/src/config.rs:1836-1886`
- **CWE**: CWE-77
...
- **Severity**: Medium
- **Location**: `rustcode-core/src/tool_impls.rs:560-584`
- **CWE**: CWE-78
...
- **Severity**: Medium
- **Location**: `rustcode-core/src/auth.rs:223-234`
- **CWE**: CWE-312
...
- **Severity**: Low
- **Location**: `rustcode-server/src/auth.rs:41-46`
- **CWE**: CWE-522
- **Description**: Password visible in `/proc/self/environ`, process listings.
- **Recommendation**: Support file-based secret injection (`OPENCODE_SERVER_PASSWORD_FILE`).

### Finding SEC-22: URL injection via browser open
- **Severity**: Low
- **Location**: `rustcode-core/src/mcp_oauth.rs:839-868`
- **CWE**: CWE-77
...
- **Severity**: Low
- **Location**: `rustcode-core/src/permission.rs:744-764`
- **CWE**: CWE-754
...
- **Severity**: Low
- **Location**: `rustcode-core/src/mcp_oauth.rs:1003-1013`
- **CWE**: CWE-311
...
- **Severity**: Low
- **Location**: `rustcode-core/src/mcp.rs:1105-1114`
- **CWE**: CWE-200
...
- **Severity**: Low
- **Location**: `rustcode-core/src/providers/anthropic.rs:985-988`
- **CWE**: CWE-200
...
### Finding SEC-28: OPENCODE_AUTH_CONTENT parsed as trusted JSON
- **Severity**: Low
- **Location**: `rustcode-core/src/auth.rs:198`
- **CWE**: CWE-502
...
- **Severity**: Low
- **Location**: `rustcode-core/src/auth.rs:204`
- **CWE**: CWE-754
...
- **Severity**: Low
- **Location**: `rustcode-core/src/mcp.rs:1212`
- **CWE**: CWE-502
...
- **Severity**: Low
- **Location**: `rustcode-core/src/credential.rs:265`
- **CWE**: CWE-754
...
- **Severity**: Low
- **Location**: `rustcode-core/src/mcp_oauth.rs:676-682`
- **CWE**: CWE-74
...
- **Severity**: Low
- **Location**: `rustcode-core/src/mcp.rs:1296-1322`
- **CWE**: CWE-200
...
- **Severity**: Low
- **Location**: `rustcode-core/src/permission.rs:1099-1166`
- **CWE**: CWE-284
...
- **Severity**: Low
- **Location**: `rustcode-core/src/permission.rs:1126-1128`
- **CWE**: CWE-459
...
- **Severity**: Low
- **Location**: `rustcode-core/src/config.rs:2422-2457`
- **CWE**: CWE-22
...
### Finding DEP-1: 0 AI provider SDKs vs 20+ in OpenCode
- **Severity**: Critical
- **Location**: `Cargo.toml` workspace dependencies
- **Description**: RustCode has zero provider-specific crates. OpenCode has 20+ `@ai-sdk/*` packages for Anthropic, OpenAI, Google, Bedrock, Azure, Groq, Mistral, Cohere, Perplexity, XAI, DeepInfra, Together, Cerebras, Alibaba, etc.
...
- **Severity**: Critical
- **Location**: `Cargo.toml`
- **Description**: Missing `git2` crate. OpenCode uses `@octokit/rest` for git operations.
...
- **Description**: `tree-sitter` at 0.24 (latest 0.25+), `tree-sitter-bash` at 0.23 (latest 0.25+). OpenCode uses `web-tree-sitter@0.25.10`.
...
- **Severity**: Medium
- **Location**: `Cargo.toml`
- **Description**: OpenCode uses `@zip.js/zip.js` for JAR/SARIF handling. No `zip` crate in RustCode.
...
- **Severity**: Medium
- **Location**: `crates/rustcode-server/src/routes/session.rs:311-440+`
...
- **Severity**: Medium
- **Location**: `crates/rustcode-core/src/session.rs:1007-1032`
...
- **Severity**: Medium
- **Location**: `crates/rustcode-core/src/tool_impls.rs:57-58` (similarity thresholds), `:1225` (MAX_READ_BYTES), `crates/rustcode-lsp/src/lib.rs:151-177` (timeouts)
...
- **Severity**: High
- **Location**: `crates/rustcode-server/src/routes/session.rs:328, 373, 388`
...
- **Severity**: Critical
- **Location**: `rustcode/Cargo.toml:57` (notify dependency)
...
- **Severity**: High
- **Location**: `rustcode/` (no IDE config)
...
- **Severity**: High
- **Location**: `rustcode/` (no test fixtures directory)
...
- **Recommendation**: Port key specs from OpenCode: session architecture, provider model, config schema.

### Finding DEVEX-10: No database migration infrastructure
- **Severity**: High
- **Location**: `rustcode/Cargo.toml:21` (sqlx)
...
- **Severity**: Medium
- **Location**: `rustcode/.github/workflows/ci.yml:49-62`
...
- **Severity**: Medium
- **Location**: `rustcode/docs/plugin-system.md:1-293`
...
- **Description**: Skill module exists as scaffold. `discover()` function from `.opencode/skills/*.md`. Integrated with instruction context and system context registry. No usage examples, no tests.
- **Recommendation**: Complete skill module implementation. Add integration tests.

### Finding DEVEX-15: Release workflow is mature
- **Severity**: Low
- **Location**: `rustcode/.github/workflows/release.yml:1-276`, `rustcode/scripts/version.sh:1-212`
...
### Finding DEVEX-16: Security auditing stronger than OpenCode
- **Severity**: Low
- **Location**: `rustcode/.github/workflows/audit.yml:1-98`
...
- **Severity**: Medium
- **Location**: `rustcode/crates/rustcode-core/src/error.rs`
...
- **Severity**: High
- **Location**: `rustcode/.github/` (no templates)
...
- **Severity**: Critical
- **Location**: RustCode -- no monitoring infrastructure
...
- **Severity**: High
- **Location**: RustCode -- tracing crate only
...
- **Severity**: High
- **Location**: RustCode -- local SQLite only
...
- **Severity**: High
- **Location**: `rustcode/.github/workflows/release.yml`
...
- **Severity**: Medium
- **Location**: RustCode -- no Dockerfiles
...
- **Severity**: Medium
- **Location**: RustCode -- tracing deps present but unconfigured
...
- **Description**: Missing `aarch64-pc-windows-msvc` target. Only `x86_64-pc-windows-msvc` for Windows. OpenCode supports all 6 platform targets.
- **Recommendation**: Add `aarch64-pc-windows-msvc` to release matrix.

### Finding INFRA-9: No Nix flake
- **Severity**: Medium
- **Location**: RustCode -- no flake.nix
- **Description**: No Nix support. Nix users cannot build/run RustCode without manual setup. OpenCode has full Nix support: flake.nix, 4 system platforms, devShell.
...
- **Severity**: Medium
- **Location**: `rustcode/install`
- **Description**: Two install methods: curl|sh install script (400 lines, feature-rich) and direct download from GitHub Releases. No crates.io (`cargo install rustcode`), no Homebrew, no Scoop, no Chocolatey.
...
- **Recommendation**: Implement `~/.config/rustcode/config.toml` with serde deserialization.
...
- **Severity**: Info
- **Location**: `sst.config.ts` (OpenCode)
- **Description**: RustCode is a CLI tool. OpenCode has SaaS backend deployed via SST to Cloudflare Workers + AWS ECS Fargate, with PlanetScale, Stripe billing.
...
- **Severity**: Info
- **Location**: N/A
- **Description**: Neither project uses Kubernetes. OpenCode uses AWS ECS Fargate + Cloudflare Workers. RustCode runs locally.
- **Recommendation**: If server-side components needed, consider serverless approach.

### Finding INFRA-15: No environment concept for RustCode
- **Severity**: Info
- **Location**: N/A
- **Description**: RustCode is a local tool. No environment management needed (unlike OpenCode's production/dev/personal stages with SST).
...
- **Description**: RustCode runs projectors inside the database transaction (`event.rs:943-948`). If a projector fails, the entire event write is rolled back. OpenCode runs projectors after the transaction commits.
...
- **Description**: RustCode duplicates all 20 table schemas as `const &str` SQL literals. No compile-time verification. A typo in a column name passes `cargo build` and `cargo test`.
- **Consequence**: Schema drift between RustCode and OpenCode is silent. If OpenCode adds a column via migration, RustCode's `INITIAL_MIGRATION` will create the table without that column.
...
- **Description**: Neither OpenCode nor RustCode implements database backups. If the SQLite file is corrupted, all session data, events, accounts, and credentials are lost.
- **Recommendation**: Add a `rustcode db backup` command that runs `PRAGMA wal_checkpoint(TRUNCATE)`, copies the database file, adds backup metadata.
...
- **Description**: OpenCode detects fresh installs (no tables) and creates full schema + marks all 35 migrations complete in one transaction. RustCode runs all 35 migrations sequentially even on a fresh database.
...
- **Description**: OpenCode supports `version` in `SyncConfig` and `versioned_type()` for forward compatibility. RustCode implements the versioning types but does not handle event data migration during version upgrade.
...
- **Description**: Projection state is `RwLock<HashMap<String, ProjectionState>>` -- lost on restart. OpenCode persists projection checkpoints to the `event_sequence` table. DB is fallback, not primary.
...
- **Description**: Uses default deferred transaction mode. OpenCode uses `{ behavior: "immediate" }`. SQLite's default `BEGIN DEFERRED` starts in read mode and upgrades to write on first mutation.
...
- **Description**: RustCode lacks Drizzle's `$default` / `$onUpdate` automatic timestamp handling. Every INSERT must explicitly pass `time_created` and `time_updated`.
...
- **Description**: RustCode adds path normalization (POSIX `/` slashes, absolute path validation) that OpenCode does at the application layer, not the database layer. Strict superset of OpenCode's functionality.
...
- **Severity**: Info
- **Location**: `database.rs:472-807`
- **Description**: All 15+ foreign key constraints are present in both OpenCode and RustCode.
...
- **Description**: All ~40+ session event types ported as Rust structs with Serialize/Deserialize. RustCode adds `session_event_types` constants module that OpenCode lacks.
- **Recommendation**: None needed.

### Finding DB-24: Multi-DB architecture difference
- **Severity**: Info
- **Location**: `infra/lake.ts:1-327`
- **Description**: OpenCode has Local SQLite, PlanetScale (MySQL), AWS S3 Tables/Athena, Stats Server. RustCode only implements the local SQLite tier. This is by design.
...
- **Severity**: Critical
- **Location**: RustCode -- no equivalent
- **Description**: OpenCode has `http-recorder` package for deterministic replay of LLM provider API calls. RustCode has no equivalent. No recorded cassettes.
...
- **Severity**: High
- **Location**: RustCode -- no coverage tool configured
...
- **Description**: 11 modules have `mod tests {}` blocks but zero test functions. OpenCode has test files for corresponding modules.
- **Recommendation**: Add tests for each module's public API surface.

### Finding TEST-6: No test infrastructure
- **Severity**: High
- **Location**: RustCode -- no test helpers module
- **Description**: No shared test fixtures, no `TestDb`, no `TempDir`, no `TestConfig` builders. OpenCode has `packages/core/test/fixture/` with `tmpdir.ts`, `git.ts`, `location.ts`, `recordings/`.
- **Consequence**: Each test module reimplements setup boilerplate. No consistent pattern for test setup.
- **Recommendation**: Create `crates/rustcode-core/src/test_support.rs` with `TestDb::new()`, `TempDir::new()`, `TestConfigBuilder`, `MockProviderBuilder`.

### Finding TEST-7: 56 documentation examples not compile-checked
- **Severity**: Medium
- **Location**: RustCode -- 58 code blocks, 2 explicitly marked as ```rust
...
- **Severity**: Critical
- **Location**: RustCode -- only 56 references to mock/stub/fake across entire codebase
...
- **Description**: OpenCode has `database-migration.test.ts` for all 35 migrations. RustCode has no migration tests that verify table/column parity.
...
- **Description**: The `Provider` trait is fully defined but no real provider implementations exist. OpenCode has full Anthropic, OpenAI, Gemini, Bedrock, Azure, etc. implementations.
...
- **Severity**: Medium
- **Description**: `OPENCODE_SERVER_PASSWORD` read from environment at request time, not startup.
...
- **Severity**: Critical
- **Description**: OpenCode uses Effect v4 with `Effect.gen`, `Context.Service`, `Layer`, `Scope`, `Stream`. RustCode uses raw `tokio::spawn` + `thiserror` + struct-field DI. No structured concurrency, no typed dependency injection, no algebraic error handling.
...
- **Severity**: Critical
- **Description**: OpenCode uses four-axis route composition (Protocol, Endpoint, Auth, Framing) allowing 14 OpenAI-compatible providers to share 1 protocol implementation. RustCode conflates protocol, URL, auth, and framing in each provider.
...
- **Severity**: High
- **Description**: OpenCode `sessions.prompt(...)` admits durable `session_input` rows with promotion lifecycle and retry reconciliation. RustCode appends to in-memory message list with no durability.
...
- **Severity**: High
- **Description**: OpenCode has durable event replay with aggregate sequence cursor. RustCode uses `tokio::sync::broadcast` -- in-memory only, no persistence.
...
- **Severity**: High
- **Description**: OpenCode has System Context Registry with scoped Context Source producers, immutable baselines, and mid-conversation system messages. RustCode has no concept of context epochs -- system prompt is static.
...
- **Severity**: Medium
- **Description**: OpenCode publishes `@opencode-ai/plugin` on npm with SDK, scoped tool registration, plugin-defined context sources. RustCode has `ProviderPlugin` trait but no publishable SDK, no plugin-scoped tool registration.
- **Recommendation**: Publish `rustcode-plugin` crate with Plugin trait, scope-based registration, lifecycle hooks.

### Finding CI-7: Cloud Infrastructure gap
- **Severity**: Low
- **Description**: OpenCode has full cloud deployment via SST on AWS/Cloudflare with PlanetScale, Athena, Honeycomb. RustCode is local-only SQLite.
- **Recommendation**: Document as local-first by design.

### Finding CI-8: Multi-Platform Support gap
- **Severity**: Medium
- **Description**: OpenCode supports CLI, TUI, Desktop (Electron), Web (Next.js), VS Code extension, LSP, MCP, Slack. RustCode has only CLI (in progress) and TUI (stub).
- **Recommendation**: Focus on CLI + MCP as primary surfaces. Skip Electron.

### Finding CI-9: Community gap
- **Severity**: High
- **Description**: OpenCode has 20k+ GitHub stars, active Discord, regular releases. RustCode has <10 stars, no community, no release cadence.
- **Recommendation**: Open-source as soon as MVP works. Publish to crates.io. Set up Discord.

### Finding CI-10: Strategic Innovations for RustCode
- **Severity**: Info
- **Description**: RustCode should exploit unique advantages: proc macros for zero-boilerplate tool definitions, single binary distribution, WASM plugin sandboxing, local AI inference via llama.cpp, compile-time safety for permission-critical tools.
