# Database Architecture Analysis: BlazeCode vs BlazeCode
...
| # | Table | BlazeCode (Drizzle ORM) | BlazeCode (sqlx raw SQL) | Parity |
|---|-------|----------------------|------------------------|--------|
...
- **BlazeCode**: Uses Drizzle ORM `sqliteTable("name", {...})` with snake_case column names. Schema defined as TypeScript objects with Drizzle type system.
- **BlazeCode**: Duplicates all 20 table schemas as `const &str` SQL literals. `CREATE_TABLE_*` constants mirror the exact Drizzle-generated SQL.
- **Gap**: **No compile-time schema verification.** BlazeCode's raw SQL strings are not validated against the actual SQLite schema until runtime. A typo in a column name or constraint will not be caught by `cargo build` or `cargo test` (unless a test runs migration against an actual database).
- **Consequence**: Schema drift between BlazeCode and BlazeCode is silent. If BlazeCode adds a column via migration, BlazeCode's `INITIAL_MIGRATION` will create the table *without* that column, and the subsequent ALTER TABLE migration will fail.
...
- **BlazeCode**: Drizzle types: `text()`, `integer()`, `real()`, `text().primaryKey()`, `integer().notNull()`, with `$default` and `$onUpdate` hooks.
- **BlazeCode**: Raw SQL types: `text PRIMARY KEY`, `integer NOT NULL`, `real DEFAULT 0 NOT NULL`. Uses `chrono::Utc::now().timestamp_millis()` for timestamps.
- **Gap**: BlazeCode lacks Drizzle's `$default` / `$onUpdate` automatic timestamp handling. Every INSERT must explicitly pass `time_created` and `time_updated`.
...
- **BlazeCode**: Drizzle stores JSON as `text()` with `$type<T>()` for type safety at the TS level. Deserialization is manual via `JSON.parse`.
- **BlazeCode**: Provides `JsonColumn<T>`, `json_column_serialize`, `json_column_deserialize` wrappers using `serde_json`. Also has path-specific helpers: `db_absolute_path`, `db_path`, `db_absolute_path_array`, `json_absolute_path_array_column`.
- **Gap**: BlazeCode adds path normalization (POSIX `/` slashes, absolute path validation) that BlazeCode does at the application layer, not the database layer. This is a **strict superset** of BlazeCode's functionality.
- **Consequence**: BlazeCode's path column helpers are more strict than the TS equivalent — they reject relative paths at the column level, which could cause silent failures if a caller passes a non-absolute path.
...
- **BlazeCode**: Uses Drizzle's type-safe query builder: `yield* db.insert(schema.session).values({...})`. Drizzle generates parameterized SQL with `?` placeholders.
- **BlazeCode**: All queries use `sqlx::query("INSERT ... VALUES (?1, ?2, ...)").bind(...).execute()`. Every parameter is explicitly numbered (`?1`, `?2`).
- **Gap**: BlazeCode uses `?1` (1-indexed, sqlite-specific) while Drizzle generates `?` (positional). Both are correct SQLite but the numbering convention differs.
...
- **BlazeCode**: Drizzle supports eager loading and JOINs via `eq(session_message.session_id, session.id)`.
- **BlazeCode**: Does **not use SQL JOINs**. Instead, the `get_messages_with_parts` method performs **N+1 queries**: it calls `list_messages(session_id)` then for each message calls `list_parts(msg.id)`.
- **Gap**: **N+1 query problem** — loading all messages for a session requires 1 query for messages + N queries for parts (where N = number of messages). BlazeCode likely uses JOINs or Drizzle's `relations` for eager loading.
...
- **BlazeCode**: Drizzle's query builder can conditionally add `.where()` clauses: `db.select().from(session).where(and(...optional filters...))`.
- **BlazeCode**: Builds SQL strings dynamically via `format!()` with `next_bind` tracking. String interpolation of column names (`directory = ?{next_bind}`).
...
- **Location**: `database.rs:1098-1135` (drizzle journal import), `storage.rs:621-1363` (BlazeCode migrations)
- **BlazeCode**: 35 individual migration files under `packages/core/src/database/migration/`. Each is a `.ts` file exporting `{ id, up }` where `up` is an Effect-returning function receiving a Drizzle transaction. Uses a semaphore (`Semaphore.makeUnsafe(1)`) to ensure serial migration application. Supports: fresh install (creates all tables + marks all migrations done), existing install (runs pending `applyOnly`), Drizzle journal import (seeds from `__drizzle_migrations`).
- **BlazeCode**: All 35 migrations are defined as `Migration { id, sql }` structs in `storage.rs:1226-1364`. A single `INITIAL_MIGRATION` (id `20260127222353_familiar_lady_ursula`) creates all 20 tables at once via `CREATE TABLE IF NOT EXISTS`. Subsequent migrations are ALTER TABLE / CREATE INDEX / DROP TABLE statements. Rollback tested via `FAILING_MIGRATION`.
- **Gap 1**: **BlazeCode skips the fresh-install optimization.** BlazeCode detects fresh installs (no tables) and creates the full schema + marks all 35 migrations as complete in one transaction. BlazeCode runs all 35 migrations sequentially, even on a fresh database. Each migration's `CREATE TABLE IF NOT EXISTS` is a no-op after the first, but this adds needless overhead.
- **Gap 2**: **BlazeCode's Drizzle journal import is in `database.rs:1098-1135`** (a separate function) while the TS version is integrated into `migration.ts:apply()`. The Rust import is a `pub async fn` rather than being called automatically during migration — callers must explicitly invoke it.
- **Gap 3**: **No rollback support.** BlazeCode migrations are forward-only (no `down`). BlazeCode follows the same pattern but has a test-only `FAILING_MIGRATION` that proves transaction rollback works during migration failure.
...
- **BlazeCode**: Migration IDs are timestamp-based strings like `20260127222353_familiar_lady_ursula` (timestamp + random words).
- **BlazeCode**: Mirrors exactly with `KNOWN_MIGRATION_IDS` showing all 35 IDs. Chronological order verified by tests.
...
- **BlazeCode**: 17 indexes defined in `schema.gen.ts` lines 242-274.
- **BlazeCode**: Same 17 indexes duplicated as `CREATE_INDEXES` and `ALL_INDEX_SQL`.
...
- **BlazeCode**: Drizzle's `db.transaction(tx => Effect.gen(...))` with `{ behavior: "immediate" }`. Transaction per migration, per event publish, per aggregate removal. Supports nested transactions (reuses parent tx).
- **BlazeCode**: Uses `sqlx::SqlitePool::begin()` / `tx.commit()` / `tx.rollback()`. Transaction per:
...
- **Gap 1**: **BlazeCode runs projectors inside the database transaction** (`event.rs:943-948`). If a projector fails, the entire event write is rolled back. BlazeCode runs projectors after the transaction commits (post-commit hooks). Running projectors inside the tx keeps the transaction open longer and can trigger deadlocks.
- **Gap 2**: **No nested transaction support.** BlazeCode's `Database.use` inside `Database.transaction` reuses the parent tx. BlazeCode always opens a new `begin()`, which would create a savepoint in SQLite but the code doesn't account for this.
- **Gap 3**: **No `immediate` mode.** BlazeCode uses `{ behavior: "immediate" }` to avoid SQLITE_BUSY errors in concurrent scenarios. BlazeCode uses default deferred transaction mode.
...
- **BlazeCode**: Single transaction that writes event + updates sequence.
- **BlazeCode**: `commit_sync_event` does **not use a transaction** at all. It calls `db.insert_event()` then `db.upsert_event_sequence()` as separate non-transactional queries.
...
- **BlazeCode**: Uses `@effect/sql-sqlite-bun` or `@effect/sql-sqlite-node` as the underlying driver. Effect manages the connection lifecycle via `Layer`. Single connection per process (Bun SQLite is single-connection by default).
- **BlazeCode**: Uses `sqlx::SqlitePool` with `sqlx::SqlitePool::connect()` via `sqlite:...?mode=rwc` URL. Pool is created with default settings (sqlx pool defaults: max 10 connections for SQLite, but SQLite only supports one writer at a time).
...
- **BlazeCode**: Uses Effect's `SqliteClient.layer({ filename })`.
- **BlazeCode**: URI format `sqlite:{path}?mode=rwc` — `rwc` means read-write-create. This implicitly creates the database if it doesn't exist.
- **Gap**: BlazeCode does not distinguish between "database doesn't exist, create it" and "database does exist, open it". If the path points to the wrong location, a new empty database is silently created instead of returning an error.
...
| Aspect | BlazeCode (Drizzle ORM) | BlazeCode (sqlx raw SQL) |
|--------|----------------------|------------------------|
...
- **BlazeCode**: Drizzle returns typed JavaScript objects directly from queries.
- **BlazeCode**: Requires a two-tier mapping: `RowRaw` (with `#[derive(sqlx::FromRow)]`) → `into_row()` → `Row`. This is because Drizzle column names like `type` conflict with Rust keywords, requiring `#[sqlx(rename = "type")]`.
...
- **BlazeCode**: `EventV2` system with typed definitions (`EventV2.define()`), sync/async events, pub/sub, projectors, commit guards, replay, and aggregate ownership tracking. Events stored in `event` table with `event_sequence` table for aggregate tracking.
- **BlazeCode**: Faithful port of the entire EventV2 system. Implements:
...
- **Gap 2**: **Event uniqueness check is an extra query** (`event.rs:917-931`). The `event_aggregate_seq_idx` UNIQUE index on `(aggregate_id, seq)` would catch duplicate seq violations without the explicit SELECT query. BlazeCode relies on the DB constraint.
- **Gap 3**: **Projectors run inside DB transaction** (see Section 5).
- **Gap 4**: **No event version upgrade path.** BlazeCode supports `version` in `SyncConfig` and `versioned_type()` for forward compatibility (e.g., `session.next.prompted.1` → `session.next.prompted.2`). BlazeCode implements the versioning types but does not handle event data migration during version upgrade.
...
- **BlazeCode**: ~40+ session event types defined as TypeScript interfaces with `define()`.
- **BlazeCode**: All types ported as Rust structs with `Serialize`/`Deserialize`. Well-known event type constants in `session_event_types` module.
- **Gap**: None — full type parity. BlazeCode even adds `session_event_types` constants module that BlazeCode lacks as a single import.
- **Severity**: **Info**
...
- **BlazeCode**: Uses a sideband git repository (`~/.local/share/blazecode/snapshot/<project>/<hash>`) separate from the user's repo. Tracks filesystem state with `git write-tree`, `git read-tree`, `git checkout-index`. Supports: `track()`, `patch()`, `restore()`, `revert()`, `diff()`, `diff_full()`, `cleanup()`.
- **BlazeCode**: Full port of the snapshot service. Uses `std::process::Command` to run git with `--git-dir` and `--work-tree` flags. Supports:
...
- **Gap 3**: **Heartbeat / stale detection only in `flock.rs`** — the snapshot service has no equivalent of BlazeCode's `SnapshotService.timer` heartbeat for long-running operations.
...
- **BlazeCode**: Context epochs track the compaction state of a session. Each epoch has a `baseline` (compaction summary text), `snapshot` (git tree hash of the workspace at epoch start), `baseline_seq` / `replacement_seq` (event sequence numbers), and `revision` (optimistic lock counter for concurrent updates).
- **BlazeCode**: Full port including:
...
- **Gap**: **Context epoch revision guard is the only optimistic concurrency control.** BlazeCode also uses event sourcing for the compaction state (compaction events are durable sync events). BlazeCode's `replace_context_epoch` update is guarded by `WHERE revision = ?7`, which prevents lost updates but does not guarantee ordering with respect to the event stream.
...
- **BlazeCode**: Read-heavy during session browsing (listing sessions, loading messages). Write-heavy during LLM streaming (inserting messages, parts, events). Balanced during compaction (reads all messages, writes summary + context epoch).
- **BlazeCode**: Same workload profile. SQLite WAL mode allows concurrent reads during writes.
- **Gap**: **No read replica or CQRS.** Both use a single SQLite database for read and write. BlazeCode's architecture spec mentions PlanetScale + stats DB, but the core app uses a single SQLite file.
...
- **BlazeCode**: EventV2 uses a CQRS-like pattern where:
...
- **BlazeCode**: Same pattern with `EventProjector` supporting `catch_up` and `rebuild`.
- **Gap**: **Projection state is in-memory only** (`event_projector.rs:66`). The `state: RwLock<HashMap<String, ProjectionState>>` is lost on restart. BlazeCode persists projection checkpoints to the `event_sequence` table (which the `db_last_seq` fallback reads). But the in-memory state is the primary source and the DB is a fallback — this is inverted.
...
- **BlazeCode**: Foreign keys defined in Drizzle schema with `references(() => table.columns, { onDelete: "cascade" })`.
- **BlazeCode**: Foreign keys defined as inline constraints in CREATE TABLE SQL:
...
- **BlazeCode**: Relies on DB-level CASCADE for most deletes. `delete_session_cascade` handles child sessions explicitly (no self-referencing FK CASCADE).
- **BlazeCode**: Same pattern. `delete_session_cascade` explicitly deletes child sessions, then the cascade handles messages → parts.
- **Gap**: **No explicit cascade for `delete_project`** in BlazeCode. Deleting a project should cascade to sessions → messages → parts → todos → permissions → workspaces → context_epochs → session_shares. The FK constraints handle this if `PRAGMA foreign_keys = ON` is set, which it is.
- **Severity**: **Low** (covered by FK constraints)

### Token Storage

- **Location**: `database.rs:514-525` (account table)
- **BlazeCode**: Account tokens stored in `account.access_token` / `account.refresh_token` as `text` columns.
- **BlazeCode**: Same — plaintext storage.
- **Gap**: **No encryption at rest for access tokens or refresh tokens.** Both BlazeCode and BlazeCode store tokens as plain text in SQLite.
...
| Query Pattern | BlazeCode | BlazeCode | Issue |
|-------------|----------|----------|-------|
...
- **BlazeCode**: PRAGMAs: `journal_mode=WAL`, `synchronous=NORMAL`, `busy_timeout=5000`, `cache_size=-64000`, `foreign_keys=ON`, `wal_checkpoint(PASSIVE)`.
- **BlazeCode**: Same 6 PRAGMAs in `CONNECTION_PRAGMAS` and `Database::open()`. Additionally, the `DatabaseConfig.pragmas()` method allows per-config customization.
...
- **BlazeCode**: Directory-based advisory locking using `mkdir` as atomic acquire, with heartbeat files, stale detection, breaker pattern, and token-verified release.
- **BlazeCode**: Full port of the flock system. Uses `tokio::fs::create_dir` for atomic acquire, `uuid::Uuid::new_v4()` for tokens, heartbeat background task, exponential backoff with jitter, stale detection via mtime.
...
- **BlazeCode**: Serialized via SQLite WAL mode + `busy_timeout` + `{ behavior: "immediate" }`.
- **BlazeCode**: Tested with concurrent pool connections. WAL mode enables concurrent readers but only one writer.
- **Gap**: **BlazeCode does not use `BEGIN IMMEDIATE` for write transactions.** SQLite's default `BEGIN DEFERRED` starts in read mode and upgrades to write on the first mutation. If two concurrent transactions both start in deferred mode, one will get `SQLITE_BUSY` when trying to upgrade. The `busy_timeout=5000` handles this by retrying, but it adds latency.
...
- **BlazeCode**: No built-in backup. Relies on filesystem backup of `~/.local/share/blazecode/blazecode.db`. The snapshot git repository provides a form of filesystem-level point-in-time recovery but not for the database itself.
- **BlazeCode**: Same — no backup feature.
...
- **Recommendation**: Add a `blazecode db backup` command that:
...
### BlazeCode Infrastructure

- **Location**: `infra/lake.ts:1-327`
- **BlazeCode** operates on multiple database tiers:
...
- **BlazeCode**: **Only implements the local SQLite tier.** The PlanetScale and AWS infrastructure are not ported.
- **Gap**: **BlazeCode has no multi-database architecture.** Features that depend on PlanetScale (session sharing, sync) and the analytics lake (usage statistics, telemetry) are not available.
- **Consequence**: BlazeCode is a single-user, offline-only tool. It cannot participate in BlazeCode's cloud features.
- **Recommendation**: This is by design (BlazeCode is a port of the core client). Document this limitation explicitly. If cloud features are needed, add a `blazecode-server` that connects to PostgreSQL or the BlazeCode API.
...
4. **Add compile-time schema validation**: Create a build script or integration test that compares Rust SQL table definitions against the canonical BlazeCode schema to detect drift. **Severity: High**
