# BlazeCode Feature Gap Analysis

**Agent 08 — Feature Gap Agent**
**Date:** 2026-06-21
**Source:** BlazeCode (TypeScript) ↔ BlazeCode (Rust port)

---

## Methodology

Each BlazeCode module (`crates/blazecode-core/src/`) is compared against the BlazeCode TypeScript source (`packages/core/src/` + `packages/blazecode/src/`). Status reflects implementation completeness at the pinned commit `5d0f866`. BlazeCode is currently in **scaffold phase** — many modules exist as type skeletons with key traits stubbed out.
...
| MISSING | Module not present in BlazeCode at all |

### Module-by-Module Analysis

| # | Module | Status | BlazeCode LOC | BlazeCode LOC | BlazeCode Coverage | BlazeCode Coverage | Gap Description | Effort | Priority |
|---|--------|--------|-------------|-------------|-------------------|-------------------|----------------|--------|----------|
| 1 | **account** | PARTIAL | 2004 | ~300 (core/account.ts + blazecode/account/) | Account struct, CRUD operations, session management | Account lifecycle, auth linking, workspace membership | BlazeCode lacks workspace membership, org-level account linking, billing integration | Medium | Medium |
| 2 | **agent** | PARTIAL | 1654 | ~500 (core/agent.ts + blazecode/agent/) | Agent struct, AgentMode, permission evaluation | Sub-agent permissions, agent config parsing, instance management | Missing sub-agent orchestration, config-driven agent loading, subagent-permissions.ts logic | Medium | High |
...
| 4 | **auth** | PARTIAL | 658 | ~300 (blazecode/auth/) | Auth struct, token validation | GitHub OAuth, device flow, server-side auth middleware | Missing OAuth flows, device auth, GitHub token exchange | Medium | Medium |
...
| 6 | **bus** | PARTIAL | 941 | ~100 (blazecode/bus/global.ts) | EventBus, SharedBus, GlobalEvent enum | Global event bus with typed events, topic subscription | Core types exist but lacks full event subscription routing and typed event variants | Small | Medium |
...
| 8 | **command** | PARTIAL | 715 | ~150 (core/command.ts + blazecode/command/) | Command struct, basic parsing | Command definitions, config commands, CLI command routing | Missing command execution pipeline, argument parsing, help generation | Small | Low |
| 9 | **config** | PARTIAL | 4861 | ~1200 (core/config/ + blazecode/config/) | Config struct, ProviderConfig, AgentConfig, McpConfig | Full config parsing (toml/json/yaml), env interpolation, plugin config, tool-output config, markdown config, formatter config, watcher config, TUI config, entry-name config | Missing config migrations, managed config sync, env variable interpolation, TUI config, plugin config layering | Large | High |
...
| 12 | **env** | PARTIAL | 718 | ~100 (blazecode/env/) | Env (HashMap wrapper) | Environment variable loading, .env support, platform detection | Missing .env file parsing, platform detection utilities | Small | Low |
| 13 | **error** | PARTIAL | 1315 | ~100 (core/util/error.ts) | Error enum (14 variants) | Error types with Effect integration, typed error handling | Error enum defined but missing many BlazeCode error variants and conversion traits | Small | Medium |
...
| 15 | **event_projector** | PARTIAL | 466 | N/A (distinct module) | EventProjector struct | Projector pattern (separate blazecode/server/init-projectors.ts + projectors.ts) | Lacks event-driven projector architecture, replay-based state rebuilding | Medium | Medium |
...
| 20 | **format** | PARTIAL | 475 | ~100 (blazecode/format/) | Token/cost formatting | Token and cost formatting utilities | Basic token formatting but missing cost calculation and model-specific formatters | Small | Low |
...
| 22 | **git** | PARTIAL | 1436 | 445 (core/git.ts + blazecode/git/) | Git struct, status/diff/worktree | Git operations: status, diff, commit, branch, worktree, stash | Missing worktree operations, branch management, stash/unstash, integration with snapshot | Medium | High |
...
| 24 | **id** | PARTIAL | 539 | ~80 (core/id/id.ts + blazecode/id/id.ts) | ascending(), descending(), create() | ID generation (ascending, descending, creation timestamps) | Basic ID generation works but missing format variants | Small | Low |
| 25 | **ide** | PARTIAL | 176 | ~50 (blazecode/ide/) | IDE struct (stub) | IDE integration: detection, protocol adapters | Stub only — no VS Code/IDE protocol implementation | Medium | Low |
| 26 | **image** | PARTIAL | 836 | ~100 (core/image/ + blazecode/image/) | Image struct, MIME type detection | Image handling, Photon/CDN, MIME detection | Missing Photon CDN integration, image optimization, size validation | Small | Low |
| 27 | **installation** | PARTIAL | 533 | ~100 (core/installation/ + blazecode/installation/) | Installation struct | Installation version management, self-update | Missing self-update mechanism, version comparison logic | Small | Low |
...
| 31 | **lsp** | PARTIAL | 957 | ~300 (blazecode/lsp/) | LSP struct (stub, main impl in blazecode-lsp) | LSP client: language server launch, diagnostic handling, code actions | blazecode-lsp has 3099 LOC but still incomplete — missing full LSP protocol, diagnostics, completion, hover | Large | Medium |
| 32 | **mcp** | PARTIAL | 3033 | ~400 (blazecode/mcp/) | MCP struct (stub, main impl in blazecode-mcp) | MCP server/client: OAuth, catalog, tool integration | blazecode-mcp has 1774 LOC — missing OAuth flow, catalog management, SSE transport | Large | Medium |
| 33 | **mcp_oauth** | PARTIAL | 1447 | ~150 (blazecode/mcp/auth.ts + oauth-*.ts) | MCP OAuth implementation | MCP OAuth: provider flow, callback handling, token management | OAuth flow partially implemented — missing callback server, token refresh, provider registration | Medium | Medium |
...
| 38 | **permission** | PARTIAL | 2154 | ~400 (core/permission/ + blazecode/permission/) | evaluate(), PermissionRule, wildcard matching | Permission system: rule evaluation, wildcard matching, arity checking, saved permissions | Missing arity evaluation, saved permission persistence, permission schema validation | Medium | High |
| 39 | **plugin** | PARTIAL | 6236 | ~600 (core/plugin/ + blazecode/plugin/) | PluginManager, Plugin struct | Plugin system: loading, provider plugins, agent plugins, skill plugins, command plugins, boot | Missing dynamic loading, plugin lifecycle, boot orchestration, N-layer plugin layering | Large | High |
...
| 42 | **project** | PARTIAL | 1381 | ~300 (core/project/ + blazecode/project/) | Project struct, project management | Project lifecycle: bootstrap, copy, directories, VCS, instance management | Missing project bootstrap flow, copy strategies, directory management, VCS integration | Medium | High |
| 43 | **provider** | PARTIAL | 3018 | ~400 (core/provider.ts + blazecode/provider/) | Provider trait, Model, StreamChunk, ChatMessage | Provider abstraction: 30+ providers, auth, transform, model status | Missing 25+ provider implementations (only Anthropic implemented in detail), provider auth, model status tracking | Large | High |
...
| 45 | **pty** | PARTIAL | 2109 | ~400 (core/pty/ + blazecode/plugin/pty-environment.ts) | PTY struct, terminal emulation | PTY: protocol, ticket system, cross-platform (bun+node), schema | Missing ticket auth, cross-platform PTY implementation, protocol codec | Medium | High |
...
| 47 | **question** | PARTIAL | 1297 | ~150 (core/question.ts + blazecode/question/) | Question struct, prompt types | User question handling: schema, prompts, resolution | Missing question schema, prompt templates, resolution workflow | Medium | Medium |
...
| 51 | **runtime** | PARTIAL | 328 | ~200 (core/effect/runtime.ts + blazecode/effect/) | Runtime struct | Effect runtime, platform layers (bun/node), instance management | Only scaffold — needs full Effect-like runtime with scopes, fibers, interruption | Large | High |
...
| 64 | **session_reminders** | PARTIAL | 197 | ? (blazecode/session/reminders.ts) | Session reminders | Session reminders: periodic prompts, context refresh | Missing reminder scheduling, injection logic, content generation | Small | Low |
| 65 | **session_revert** | PARTIAL | 330 | ~80 (blazecode/session/revert.ts) | Session revert | Session revert: undo messages, restore previous state | Missing revert logic, state checkpointing, undo chain management | Small | Medium |
...
| 68 | **share** | PARTIAL | 637 | ~150 (blazecode/share/ + core/share/) | Share struct, session sharing | Session sharing: URL generation, access control, expire | Missing share URL generation, access control, expiration, next-sharing | Medium | Low |
...
| 70 | **shell_parser** | PARTIAL | 427 | N/A (blazecode tool integration) | Shell command parser | Command parsing: argument splitting, quoting | Missing comprehensive shell parsing, quoted string handling, escape sequences | Small | Low |
| 71 | **skill** | PARTIAL | 1763 | ~300 (core/skill/ + blazecode/skill/) | Skill struct, discover() | Skill system: discovery from .blazecode/skills/*.md, guidance injection | Missing skill directory scanning, guidance merging, skill dependency resolution | Medium | High |
...
| 75 | **storage** | PARTIAL | 2018 | ~200 (blazecode/storage/) | Storage (JSON), Database (SQLite placeholder) | File storage abstraction with schema validation, SQLite persistence | Missing storage schema, path management, file organization strategy | Medium | Medium |
| 76 | **sync** | PARTIAL | 72 | ~100 (blazecode/sync/) | Sync struct (tiny stub) | Sync: session sync across devices, schema, conflict resolution | Stub only — missing sync protocol, conflict resolution, schema versioning | Medium | Low |
...
| 78 | **tool** | PARTIAL | 1797 | ~400 (core/tool/ + blazecode/tool/) | Tool trait, ToolRegistry, ToolResult | Tool system: Bash, Read, Write, Edit, Glob, Grep, WebFetch, WebSearch, Question, Skill, TodoWrite, ApplyPatch, Task, Plan, LSP, Truncate, Schema, Registry, JSON Schema | Only traits scaffolded — missing 15+ tool implementations, tool registry lifecycle, JSON schema generation, streaming | Large | High |
| 79 | **tool_impls** | PARTIAL | 7235 | ~1000 (core/tool/*.ts + blazecode/tool/*.ts) | Tool implementations | All tool implementations: Bash, File Read/Write/Edit, Glob, Grep, WebFetch, WebSearch, Question, Skill, TodoWrite, ApplyPatch | 7235 LOC of stub tool implementations — need full logic for each tool type | Large | High |
...
| 81 | **tool_stream** | PARTIAL | 294 | N/A (tool/tool.ts in blazecode) | Tool streaming | Tool streaming: real-time output, chunked responses | Missing stream assembly, chunk formatting, progress tracking | Small | Low |
| 82 | **truncate** | PARTIAL | 389 | ~60 (blazecode/tool/truncate.ts) | Content truncation | Content truncation: token-aware, strategy selection | Missing token counting, truncation strategies, content summarization | Small | Low |
| 83 | **util** | PARTIAL | 1060 | ~500 (core/util/ + blazecode/util/) | Utility functions | Utilities: array, binary, encoding, error, glob, hash, identifier, path, retry, slug, token, which, wildcard, flock, archive, bom, data-url, defer, HTTP client, HTML, IIFE, lazy, locale, media, process, proxy, queue, record, repository, RPC, signal, timeout | Many utility functions not ported — 40% of util surface area missing | Medium | Low |
...
| 86 | **worktree** | PARTIAL | 1058 | ~200 (blazecode/worktree/) | Worktree management | Git worktree: create, delete, list, sync | Missing worktree creation/deletion, branch management, sync logic | Medium | Medium |

---

## 2. BlazeCode-Only Features (No BlazeCode Equivalent)

These features exist in BlazeCode but have **zero** BlazeCode implementation. Listed in descending order of size/complexity.

| # | Feature | BlazeCode Packages | LOC (approx) | Description | Priority |
|---|---------|-------------------|-------------|-------------|----------|
...
| 13 | **ACP (Agent Client Protocol)** | packages/blazecode/src/acp/ | ~2000 | Agent Client Protocol implementation | Medium |
| 14 | **Plugin SDK** | packages/sdk/ + packages/plugin/ | ~2000 | Published @blazecode-ai/plugin npm package | Medium |
...
| 20 | **TUI (Terminal UI)** | packages/tui/ | ~5000 | React/Ink terminal UI (maps to blazecode-tui) — partially exists | Medium |
...
### Tier 1 — Critical (blockers for BlazeCode viability)
These gaps prevent BlazeCode from running even basic sessions:
...
| P1 | **config** | No config parsing pipeline — can't load blazecode.json |
...
### BlazeCode Core Modules (86 modules)
...
### BlazeCode-Only Features (21 features)
...
| BlazeCode-only features | 283 | 13 | 1.1 |
| **Total** | **906** | **42** | **3.5** |
...
| BlazeCode-only features | $339,600 |
| **Total** | **$1,087,200** |

### Current BlazeCode Progress

| Metric | Value |
|--------|-------|
| Total BlazeCode LOC (blazecode-core) | 115,477 |
| Total BlazeCode LOC (core + blazecode) | ~101,847 |
| BlazeCode scaffold coverage | ~100% (all 86 modules exist) |
| BlazeCode implementation completeness | ~20% (types + traits, minimal logic) |
| BlazeCode actual working features | ~5% (config scaffold, error types, basic ID generation) |
| BlazeCode features not ported | 21 major features |
...
1. **Structural parity is high (100%)** — all 86 BlazeCode modules from the pinned BlazeCode commit have corresponding `.rs` files. No module is MISSING.
...
3. **The biggest gap is the session system** — BlazeCode's V2 Session architecture (Effect-native, durable prompt, algebraic system context) represents ~4,000 LOC of sophisticated state machine logic that BlazeCode has barely begun.

4. **Provider ecosystem gap** — BlazeCode supports 30+ LLM providers; BlazeCode currently implements only Anthropic well. Each provider requires ~300-500 LOC of protocol adapter.

5. **BlazeCode has grown beyond the pinned commit** — 21 features exist in BlazeCode that weren't in the commit BlazeCode was pinned to. This includes major items (Console, Web App, VSCode Extension, Desktop App, GitHub Copilot) and infrastructure (Stats, Docs, Storybook, Plugin SDK, Enterprise, Slack).

6. **BlazeCode has supplementary crates** — 23,454 LOC in `blazecode-server`, `blazecode-tui`, `blazecode-lsp`, `blazecode-mcp` that extend beyond the core module list. These show significant investment in the server (3,769 LOC) and TUI (8,190 LOC).

7. **Estimated 3.5 person-years** to reach full parity across both the existing 86 modules (scaffold → complete) and the 21 unported BlazeCode features.
...
```
BlazeCode (TypeScript, pinned commit):
  packages/core/src/     — 32,856 LOC (232 files)
  packages/blazecode/src/ — 68,991 LOC (355 files)
  Total                  — 101,847 LOC

BlazeCode (Rust):
  blazecode-core/         — 115,477 LOC (86 files)
  blazecode-server/       —  3,769 LOC
  blazecode-tui/          —  8,190 LOC
  blazecode-lsp/          —  3,099 LOC
  blazecode-mcp/          —  1,774 LOC
  blazecode-main/         —  8,575 LOC (src/main.rs)
  Total                  — 140,884 LOC
```

Despite BlazeCode having more total LOC, the majority of `blazecode-core` LOC is in `tool_impls.rs` (7,235), `plugin.rs` (6,236), and `config.rs` (4,861) — which are largely stub/trait-heavy. BlazeCode's TypeScript is more compact due to higher-level abstractions (Effect TS, Drizzle ORM, Vercel AI SDK).
