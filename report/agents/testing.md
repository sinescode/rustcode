# Testing Agent Report — RustCode vs OpenCode

**Date**: 2026-06-21  
**RustCode**: 2,386 test functions, 112 `#[cfg(test)]` modules, 3,023 total test attributes  
**OpenCode**: 532 test files, 27 CI workflows, Playwright e2e, http-recorder, 140 core package tests

---

## 1. Unit Tests

- **Location**: RustCode — inline `mod tests` in every `.rs` file across `crates/rustcode-core/src/`.  
- **OpenCode**: 140 test files in `packages/core/test/`, organized by module (e.g., `agent.test.ts`, `config/`, `provider/`, `skill/`). Uses `bun test` with `describe`/`it`/`expect`.  
- **RustCode**: 2,386 test functions across 112 modules. Top modules by test function count: `config.rs` (82), `catalog.rs` (80), `tool_impls.rs` (75), `location.rs` (72), `repository.rs` (70), `ripgrep.rs` (67), `session.rs` (66), `permission.rs` (63), `pty.rs` (59), `plugin.rs` (56), `provider.rs` (55), `integration.rs` (30).  
- **Gap**: OpenCode separates tests into a `test/` directory with fixtures; RustCode tests are inline only. RustCode has no test helpers, no test utility modules, and no shared test infrastructure.  
...
- **Location**: RustCode — `crates/rustcode-core/src/integration.rs:885` has 30 tests. `database.rs:3304` has 20 tests covering SQL migrations and queries.  
- **OpenCode**: `integration.test.ts` (Effect-based test layer composition), `database-migration.test.ts` (18 SQLite tables, 35 migrations), `session-runner-recorded.test.ts` (recorded provider responses).  
- **Gap**: RustCode's integration tests only cover business logic composition — no database migration tests, no provider integration tests, no network-recorded session tests. OpenCode's `http-recorder` package enables deterministic replay of LLM provider API calls; RustCode has no equivalent.  
- **Consequence**: RustCode integration tests cannot safely exercise LLM provider code paths. Session runner tests lack recorded cassettes for deterministic replay.  
...
- **OpenCode**: Playwright-based e2e in `packages/app/e2e/` (smoke, regression, performance). `bun run test:e2e:local` in CI.  
- **Gap**: RustCode has zero end-to-end tests. No CLI invocation tests, no TUI integration tests, no full workflow tests.  
...
- **OpenCode**: No explicit coverage tool mentioned in workflows, but `bun test` provides built-in coverage via `--coverage`.  
- **RustCode**: Estimated coverage ~60-70%. Well-tested core modules (`permission`, `config`, `catalog`, `location`). Untested paths: providers (many have 0–3 tests), CLI entry points, TUI components, LSP handlers, MCP handlers, error recovery paths, concurrent session execution.  
...
- **Location**: RustCode — tests use `assert!` and `assert_eq!` primarily.  
- **OpenCode**: Tests use `expect()` matchers with `bun:test`. Pattern: `describe` blocks grouping related tests, `beforeAll`/`afterAll` setup/teardown.  
- **RustCode**: Tests are predominantly data-structure serialization roundtrips, rule evaluation matrices, and edge-case enumerations. Example: `permission.rs` has 63 tests covering wildcard matching exhaustively. `image.rs` has 47 tests covering MIME detection for every format. Tests are well-structured with clear names.  
...
- **OpenCode**: Uses Effect's `Layer` system for dependency injection. `http-recorder` package records/replays HTTP traffic deterministically. No explicit mocking — tests use real implementations with recorded responses.  
- **RustCode**: No mocking framework (no `mockall`, `mockito`, `wiremock` in dependencies). Providers are traits but tests don't mock them — they construct real provider instances in tests. No recorded HTTP cassettes.  
...
- **Location**: RustCode — no dedicated test infrastructure.  
- **OpenCode**: `packages/core/test/fixture/` provides `tmpdir.ts` (temp directory creation with `Symbol.asyncDispose`), `git.ts`, `location.ts`, `recordings/` directory. `packages/http-recorder/` provides deterministic HTTP recording/replay.  
- **RustCode**: `tempfile` crate is in dependencies but used ad-hoc. No shared test fixtures, no test database setup helpers, no standardized temp directory management.  
...
- **Recommendation**: Create `crates/rustcode-core/src/test_support.rs` with: `TestDb::new()`, `TempDir::new()`, `TestConfigBuilder`, `MockProviderBuilder`. Add `#[cfg(test)]` module in `lib.rs` that re-exports test utilities.  
- **Severity**: High
...
- **Location**: RustCode — 58 code blocks in doc comments, but only 2 explicitly marked ````rust``.  
- **OpenCode**: Not applicable (TypeScript — `bun test` doesn't run doc tests).  
- **RustCode**: Most doc comments have examples, but few are runnable doctests. `env.rs:143` and `npm.rs:270` have verified doctests; the rest are plain code fences.  
...
- **Location**: RustCode — none.  
- **OpenCode**: `packages/app/e2e/performance/` directory with Playwright performance benchmarks.  
- **RustCode**: No benchmarks, no `criterion` or `divan` in dependencies, no `iai` for instruction counting. No load tests for the SSE server or session processing pipeline.  
...
- **Location**: RustCode — `audit.yml` runs `cargo-audit` weekly. `ci.yml` runs `cargo-deny` on every commit.  
- **OpenCode**: No dedicated security workflow visible in the CI files examined.  
- **RustCode**: `cargo-audit` checks for known vulnerabilities in dependency tree. `cargo-deny` checks license allowlist and advisories. No fuzzing (`cargo-fuzz` / `cargo-afl` not configured). No SAST (static analysis beyond clippy). No DAST for the HTTP server.  
...
- **Location**: RustCode — 1 CI workflow.  
- **OpenCode**: 27 CI workflows including test.yml, typecheck.yml, review.yml, triage.yml, and many others.  
- **RustCode**: 4 jobs in `ci.yml`: `fmt` (ubuntu), `clippy` (ubuntu), `test` (ubuntu+macos+windows), `deny` (ubuntu). `audit.yml` runs weekly. `release.yml` builds on tag. No typecheck job in CI. Test matrix: 3 OS, no feature flag combinations, no MSRV check.  
...
- **Location**: RustCode — generally deterministic.  
- **OpenCode**: Unknown from examined files.  
- **RustCode**: SQLite in-memory database tests are deterministic. PTY tests rely on real process execution (potential flakiness). File system tests create/clean up temp directories (potential race conditions under concurrent test execution). No test retry mechanism.  
...
- **Location**: RustCode — none.  
- **OpenCode**: none.  
- **RustCode**: No `cargo-mutants`, no `mutagen`, no mutation testing infrastructure.  
...
- **Location**: RustCode — none.  
- **OpenCode**: none (TypeScript lacks built-in PBT support; `fast-check` not in dependencies).  
- **RustCode**: No `proptest` or `quickcheck` in dependencies. All tests use hand-written example inputs.  
...
| E2E tests | Playwright (app e2e) | None | RustCode missing entire category |
| HTTP recording | `http-recorder` package | None | Cannot test providers deterministically |
| Dependency injection | Effect Layers | Trait + struct fields | RustCode DI is simpler but harder to mock |
| CI workflows | 27 | 3 (ci, audit, release) | OpenCode has richer CI pipeline |
| Test fixtures | `tmpdir.ts`, `recordings/`, `git.ts` | None | RustCode lacks shared test infrastructure |
...
| Performance tests | Playwright benchmarks | None | RustCode gap |
| Security tests | Not visible | cargo-audit + cargo-deny | RustCode slightly ahead |
