# Master Gap Analysis: BlazeCode vs BlazeCode

**Generated**: 2026-06-21
**Source Agents**: 02–20 (19 reports)
**Scope**: BlazeCode (Rust port) vs BlazeCode (TypeScript source)
...
- **Gap ID**: GAP-ARCH-001
- **Location**: `blazecode-core/src/lib.rs:1-95`
- **BlazeCode**: 26 packages with clean dependency DAG; `core → llm`, `server → core`, `cli → core`
- **BlazeCode**: Single monolithic core crate with 95 flat public modules; every module is `pub mod`
...
- **Gap ID**: GAP-ARCH-002
- **Location**: `blazecode-core/src/lib.rs:1-95`, `src/main.rs:1-8575`
- **BlazeCode**: Low coupling via package boundaries + Effect's injection system (`Layer`, `Context.Service`)
- **BlazeCode**: Extreme coupling — 95 flat modules, 8,575-line `main.rs` mixing CLI parsing, business logic, infrastructure
...
- **Gap ID**: GAP-ARCH-003
- **Location**: `blazecode-core/src/lib.rs:1-95` (`pub mod`), `blazecode-core/src/database.rs`, `blazecode-core/src/config.rs`
- **BlazeCode**: Clean Architecture — core defines ports, infrastructure implements them; dependencies point inward
- **BlazeCode**: Core imports `sqlx`, `reqwest`, `std::fs` directly — infrastructure concerns in domain code
...
- **Recommendation**: Define `Database` trait in core → implement in `blazecode-database-sqlite`; same for `HttpClient`, `FileSystem`
- **Severity**: Critical
- **Source Agent**: Agent 02

- **Gap ID**: GAP-ARCH-004
- **Location**: `blazecode/Cargo.toml:1-96` (5 crates), `blazecode/packages/` (26 packages)
- **BlazeCode**: 26 packages with clear single-responsibility boundaries; infrastructure-layer packages (`effect-drizzle-sqlite`, `effect-sqlite-node`)
- **BlazeCode**: 5 crates — 4 are stubs (server, tui, lsp, mcp); no infrastructure crates; no CLI crate (logic in main.rs)
- **Description**: Insufficient modularization — 5× fewer crates than BlazeCode's 26 packages; core acts as dump for all concerns
...
- **Recommendation**: Extract into granular crates: `blazecode-core-types`, `blazecode-provider`, `blazecode-session`, `blazecode-database-sqlite`, `blazecode-http`, `blazecode-plugin-sdk`, `blazecode-cli`
- **Severity**: Critical
- **Source Agent**: Agent 02

- **Gap ID**: GAP-ARCH-005
- **Location**: `src/main.rs:1-8575`
- **BlazeCode**: Thin CLI dispatch (~200 lines) delegating to Effect layers; `packages/blazecode/src/index.ts`
- **BlazeCode**: 8,575-line monolith merging CLI argument parsing, business logic, database queries, SSE handling, configuration
...
- **Recommendation**: Move logic from main.rs into `blazecode-core` or wrapper crates; make main.rs a thin CLI dispatch (<500 lines)
- **Severity**: Critical
- **Source Agent**: Agent 02
...
- **Gap ID**: GAP-ARCH-006
- **Location**: `blazecode-core/src/` (95 flat modules)
- **BlazeCode**: Domain-cohesive modules: `packages/llm/src/anthropic/`, `packages/core/src/session/`
- **BlazeCode**: Flat name prefixing (`session_*`) instead of sub-modules — 14 session-related files, 8 provider-related files all flat
...
- **Gap ID**: GAP-ARCH-007
- **Location**: `blazecode-core/src/` (module list), `blazecode/CONTEXT.md:1-129`
- **BlazeCode**: Explicit V2 domain model — System Context algebra, EventV2 event sourcing, Location-scoped services, Account identity
- **BlazeCode**: Flattened domain — System Context missing algebra (epoch, baseline, snapshot, mid-conversation messages), EventV2 absent, Location not first-class
- **Description**: BlazeCode V2 has mature domain model with carefully separated concerns; BlazeCode ported V1/early-V2 structure but lacks V2 innovations
- **Consequence**: BlazeCode will diverge in capabilities as BlazeCode's V2 matures; System Context algebra is core of session intelligence
...
- **Gap ID**: GAP-ARCH-008
- **Location**: `blazecode-core/src/provider.rs` (Provider trait), whole codebase
- **BlazeCode**: Emerging hexagonal architecture — ports defined in `llm` (provider-neutral), infrastructure injected via Effect layers
- **BlazeCode**: Good port/adapter for LLM providers only; database (`sqlx`), HTTP server (`axum`), filesystem imported directly in core
...
- **Gap ID**: GAP-ARCH-009
- **Location**: `blazecode-crates/` (server, tui, lsp, mcp)
- **BlazeCode**: 4+ real layers — `blazecode(CLI) → core → llm → infrastructure`
- **BlazeCode**: 1.5 layers — core + thin wrapper crates that are stu-level; business logic for server, tui, lsp, mcp lives in core
...
- **Gap ID**: GAP-ARCH-010
- **Location**: `blazecode-core/src/session_runner.rs` (V1 vs V2 code duplication)
- **BlazeCode**: Single clean runner with Effect.stream pipeline
- **BlazeCode**: V1 `run_loop` (~200 lines) and V2 `run_turn_attempt` (~222 lines) duplicate LLM streaming loop, tool call collection, tool execution
...
- **BlazeCode**: Single unified error type via Effect's `Schema.TaggedErrorClass`
- **BlazeCode**: Five separate error types: `Error` (34 variants), `SessionError` (12 variants), `DatabaseServiceError` (3 variants), `LspError` (10 variants), `McpError`
...
- **Gap ID**: GAP-RUST-002
- **Location**: `tool.rs:47`
- **BlazeCode**: Messages passed by reference
- **BlazeCode**: `ToolContext.messages` is `Vec<ChatMessage>` — full deep clone of entire message history per tool call
...
- **Gap ID**: GAP-RUST-003
- **Location**: `blazecode-lsp/src/lib.rs:113`
- **BlazeCode**: Unified error hierarchy
- **BlazeCode**: LSP crate defines own `Result<T>` alias incompatible with `crate::error::Result<T>`
...
- **Recommendation**: Make `blazecode_lsp` use `crate::error::Result` or unified error type
- **Severity**: High
- **Source Agent**: Agent 03
...
- **BlazeCode**: TS passes mutable refs
- **BlazeCode**: `fork()` clones each `Part` via `p.clone()` then mutates IDs — all 13 Part variants cloned
...
- **BlazeCode**: Effect `Layer` + `Ref<Info>`
- **BlazeCode**: `RwLock<Info>` with `.clone()` on every read
...
- **BlazeCode**: Simple function closures
- **BlazeCode**: 4 function pointer fields with complex types suppressed by `#[allow(clippy::type_complexity)]`
...
- **BlazeCode**: Config loading is Effect (async)
- **BlazeCode**: Filesystem reads done synchronously with `std::fs::read_to_string`
...
- **BlazeCode**: Union of 13 types in TS
- **BlazeCode**: Enum with 13 variants — largest variant (`ToolPart` with `serde_json::Value`) determines enum size
...
- **BlazeCode**: Branded string types (`type ModelId = string & { readonly __brand: "ModelId" }`)
- **BlazeCode**: `pub type SessionId = String;` — type aliases with zero type safety
...
- **BlazeCode**: Single update pattern in TS
- **BlazeCode**: Three functions (`set_id`, `set_message_id`, `set_session_id`) each with 13-arm matches — all exhaustive
...
- **BlazeCode**: Strict tsconfig with dead code detection
- **BlazeCode**: `#![allow(dead_code, unused_imports, unused_variables)]` crate-wide
...
- **BlazeCode**: Branded types for all IDs
- **BlazeCode**: Mixed — `EventId(String)` is proper newtype, but `SessionId`, `ModelId`, `ProviderId` etc. are type aliases
...
- **BlazeCode**: Effect's `Context.Service` + `Layer` DI
- **BlazeCode**: Manual injection via `Arc<DatabaseService>`, `SharedBus`, `Arc<ToolRegistry>`, etc.
...
- **BlazeCode**: `packages/blazecode/src/session/session.ts:828-830`
- **BlazeCode**: `clear_revert` writes literal string `"null"` instead of SQL `NULL` — `Some("null")` passed to `update_session`
...
- **BlazeCode**: V1 run loop with permission checks
- **BlazeCode**: V1 `run_loop` sets `ask_fn: None`, `permission_source: None`, calls `execute_by_name` — zero permission checks
...
- **BlazeCode**: `packages/core/src/session/runner/llm.ts:345-357`
- **BlazeCode**: `compact_result.as_ref().unwrap()` violates rule #3; `serde_json::json!` with `.map()` inside produces `Some(...)` wrappers in JSON
...
- **BlazeCode**: `packages/blazecode/src/session/session.ts fromRow()`
- **BlazeCode**: `cost: f64` — prevents deriving `Eq` on `SessionInfo`; floating-point precision loss on monetary values
...
- **BlazeCode**: TOCTOU race in `wake()` — reads then drops lane reference, then reacquires for mutation
...
- **BlazeCode**: `fork` loop breaks before inserting stop message into `id_map`
...
- **BlazeCode**: `await_idle` — unbounded busy-wait with no timeout, no cancellation token
...
- **BlazeCode**: `part_id` generation failure falls back to empty string via `unwrap_or_default()`
...
- **BlazeCode**: FiberSet spawn results silently dropped if receiver is closed (`let _ = result_tx.send(...)`)
...
- **BlazeCode**: `check_context_overflow` assumes 1 token = 4 bytes; real tokenizers average 3-5 chars/token for English, 8-20 for code/JSON
...
- **BlazeCode**: `parse_turn_control` — fragile string matching on encoded control flow
...
- **BlazeCode**: Permission check always passes `"*"` as resource, regardless of tool
...
- **BlazeCode**: `JoinHandle` stored but never awaited in `cancel()` or `cancel_all()` — tasks leak on drop
...
- **BlazeCode**: V1 and V2 ignore `StepFinish` reason — `LlmEvent::StepFinish { reason, .. } => { let _ = reason; }`
...
- **BlazeCode**: Sends two system messages when `input.system` is set — most LLM providers only support single system message
...
- **BlazeCode**: `Running` state never explicitly set to `Idle` after wake completes
...
- **Gap ID**: GAP-SEC-001
- **Location**: `blazecode-core/src/auth.rs:195-206`, `mcp.rs:2263-2276`, `credential.rs:352-353`
- **BlazeCode**: Same patterns (upstream)
- **BlazeCode**: `auth.json`, `mcp-auth.json`, SQLite credential values all stored in plaintext
...
- **Gap ID**: GAP-SEC-002
- **Location**: `blazecode-core/src/providers/*:resolve_api_key()`
- **BlazeCode**: Same pattern
- **BlazeCode**: API keys read from env vars into `String` — live in heap until process exit
...
- **BlazeCode**: N/A
- **BlazeCode**: `RUSTSEC-2024-0436` ignored without documented rationale
...
- **Gap ID**: GAP-SEC-004
- **Location**: `blazecode-core/src/encryption/hmac.rs`
- **BlazeCode**: HMAC-based encryption module exists
- **BlazeCode**: File does not exist — encryption module not ported
- **Description**: HMAC-based credential encryption from BlazeCode has not been implemented
...
- **BlazeCode**: Same pattern
- **BlazeCode**: MCP local servers spawned with full user privileges via `tokio::process::Command::new(cmd)`
...
- **Gap ID**: GAP-SEC-006
- **Location**: `blazecode-core/src/config.rs:2796-2789, 2804-2818`
- **BlazeCode**: Same pattern
- **BlazeCode**: `{file:path}` variable substitution reads arbitrary files — path traversal via `{file:../../etc/passwd}`
...
- **BlazeCode**: Same pattern
- **BlazeCode**: File read in variable substitution with no path restriction
...
- **BlazeCode**: Same pattern
- **BlazeCode**: `auth_token` query parameter supported — credentials in URL
...
- **BlazeCode**: `fs/promises` (async I/O via libuv thread pool)
- **BlazeCode**: Synchronous `std::fs` operations on tokio async runtime
...
- **BlazeCode**: Delegates to ripgrep (memory-mapped, streaming)
- **BlazeCode**: Reads entire matching files into memory (`std::fs::read_to_string`)
...
- **BlazeCode**: Reads up to cap
- **BlazeCode**: Reads full file then truncates to 50KB — for 500MB log file, reads all 500MB into memory
...
- **BlazeCode**: Messages by reference
- **BlazeCode**: `ctx.messages = messages.clone()` — clones entire `Vec<ChatMessage>` history per tool call
...
- **BlazeCode**: Single-path event publishing
- **BlazeCode**: Each sync event clones `EventPayload` 8+ times (guards, projectors, sync handlers, aggregate subscribers, listeners, typed channel, global channel)
...
- **BlazeCode**: Transaction per atomic operation
- **BlazeCode**: Transaction held open during async operations (commit guards, projectors, commit hook)
...
- **Location**: Whole codebase, `blazecode/infra/`
- **BlazeCode**: Cloudflare Workers (300+ locations), Durable Objects, PlanetScale (Vitess), distributed coordination
- **BlazeCode**: Single-process, single-node SQLite. No distributed primitives — no service discovery, no leader election
...
- **Location**: `blazecode-core/src/database.rs:59-66`, `blazecode/infra/console.ts:11-44`
- **BlazeCode**: PlanetScale (MySQL-compatible Vitess) with horizontal read replicas, sharding
- **BlazeCode**: SQLite with WAL mode — single-writer bottleneck
...
- **Location**: `blazecode-core/src/event.rs:775-799`, `blazecode/infra/app.ts`
- **BlazeCode**: Stateless Workers — crash means next request hits another worker; Durable Objects persist to CF storage
- **BlazeCode**: No cross-node fault tolerance; crash loses all in-memory state (SharedBus subscribers, AppState, in-flight streams)
...
- **Location**: `blazecode-core/src/database.rs:59-66`
- **BlazeCode**: PlanetScale MySQL (Vitess) — thousands of writers
- **BlazeCode**: SQLite single-file (WAL) — one writer
...
- **Location**: `blazecode-core/src/`, database schema
- **BlazeCode**: Full multi-tenant SaaS; PlanetScale, Cloudflare R2, auth middleware, org management, Stripe billing
- **BlazeCode**: Single-user, single-workspace. No tenant concept. Tables for `account`, `workspace` exist but are scaffold-only
...
- **BlazeCode**: EventV2 replay + SessionRunCoordinator.resume()
- **BlazeCode**: EventV2 port structurally complete but not wired into session recovery; no mechanism to resume interrupted turn
...
- **BlazeCode**: Effect Stream with per-type PubSub + backpressure
- **BlazeCode**: `tokio::sync::broadcast` channel (capacity 1024) — lagged receivers skip events; no per-client backpressure
...
- **BlazeCode**: EventV2 unified — all events persisted, single pipeline
- **BlazeCode**: Dual bus — `SharedBus` (in-memory broadcast) and `EventV2` (DB-backed); they don't interoperate
...
- **BlazeCode**: Enterprise tier-based limits (ZenLite/ZenBlack), stats pipeline, billing limits
- **BlazeCode**: Step limits only (MAX_STEPS=25); no per-session resource budgets (memory, tokens, cost); no global caps
...
- **BlazeCode**: Upstash Redis for caching; Cloudflare CDN; PlanetScale query cache
- **BlazeCode**: SQLite page cache only (64MB); no in-memory session cache; no query result caching
...
- **Location**: `blazecode-server/src/server.rs:217`
- **BlazeCode**: Upstash Redis + Cloudflare rate limiting + tier-based limits
- **BlazeCode**: No rate limiting; no token bucket, no leaky bucket, no per-user rate limits
...
> Note: All 86 BlazeCode modules from the pinned BlazeCode commit have corresponding `.rs` files — **structural parity is 100%**. However, functional parity is ~20% — most modules are type skeletons with key traits; actual business logic is largely unported.

### Critical

- **Gap ID**: GAP-FEAT-001
- **Location**: `blazecode-core/src/session*` (14 sub-modules)
- **BlazeCode**: V2 Session architecture — Effect-native, durable prompt, algebraic system context, epoch-based state, input inbox, message projection, revert, reminders, todo, runner lifecycle
- **BlazeCode**: Core types exist but ~4,000 LOC of sophisticated state machine logic barely begun
...
- **Consequence**: Core session loop missing — BlazeCode cannot run basic agent sessions
...
- **Gap ID**: GAP-FEAT-002
- **Location**: `blazecode-core/src/event.rs`
- **BlazeCode**: EventV2 — durable event streams with SQL persistence, replay, algebraic projection (~2,000 LOC)
- **BlazeCode**: EventV2 types and port exist but durable event streams, replay, and algebraic projection not fully implemented
...
- **Gap ID**: GAP-FEAT-003
- **Location**: `blazecode-core/src/provider.rs`, `blazecode-core/src/providers/`
- **BlazeCode**: 30+ LLM providers using route-based architecture with 4-axis composition (Protocol, Endpoint, Auth, Framing)
- **BlazeCode**: Provider trait defined, only Anthropic implemented in detail; no route architecture
...
- **Gap ID**: GAP-FEAT-004
- **Location**: `blazecode-core/src/tool_impls.rs:7235 LOC`
- **BlazeCode**: All tool implementations complete (Bash, Read, Write, Edit, Glob, Grep, WebFetch, etc.)
- **BlazeCode**: 7,235 LOC of stub tool implementations — function signatures exist but logic is placeholders
...
- **Gap ID**: GAP-FEAT-005
- **Location**: `blazecode-core/src/config.rs:4861 LOC`
- **BlazeCode**: Full config parsing (toml/json/yaml), env interpolation, plugin config, tool-output config, TUI config
- **BlazeCode**: Config struct and types exist but config loading pipeline is not wired
- **Description**: Config parsing logic is scaffolded but cannot actually load `blazecode.json`
...
- **BlazeCode**: NPM packages use semver; `@blazecode-ai/sdk@1.17.8`; V1/V2 coexistence with feature flags
- **BlazeCode**: All crates at `0.1.0`; no versioning policy; no changelog; no migration guide
...
- **BlazeCode**: Server routes in `packages/server/src/api.ts` + 30+ group files, all implemented
- **BlazeCode**: 25+ route handlers are stubs — `api_session_prompt`, `api_session_compact`, `api_session_wait` return placeholder data
...
- **Gap ID**: GAP-API-003
- **Location**: `packages/sdk/js/package.json` (BlazeCode SDK)
- **BlazeCode**: `@blazecode-ai/sdk@1.17.8` with auto-generated REST client, typed V2 API surface, lifecycle helpers
- **BlazeCode**: No equivalent SDK crate; no `blazecode-client` crate; no auto-generated client
...
- **Recommendation**: Create `blazecode-client` crate with typed async HTTP client similar to `@blazecode-ai/sdk`
- **Severity**: Critical
- **Source Agent**: Agent 09

- **Gap ID**: GAP-API-004
- **Location**: `packages/sdk/js/src/gen/` (auto-generated), `blazecode-core/src/` (hand-written)
- **BlazeCode**: SDK types and client auto-generated from OpenAPI spec via `@hey-api/openapi-ts`
- **BlazeCode**: All types hand-written ports from TypeScript; no code generation; no shared spec
- **Description**: Manual porting creates drift; as BlazeCode evolves, BlazeCode types must be manually updated
- **Consequence**: BlazeCode will fall behind BlazeCode's schema changes; type mismatches accumulate
...
- **BlazeCode**: Effect.ts `Context.Tag`, explicit service interfaces, `export`-controlled module boundaries
- **BlazeCode**: `pub mod` for all 95 modules — every module public with no sub-visibility
...
- **BlazeCode**: Consistent camelCase in JSON wire format
- **BlazeCode**: Mixed conventions — some serde `rename` attributes use camelCase, some use snake_case
- **Description**: Some Rust structs fail to match BlazeCode's JSON wire format due to inconsistent/missing serde rename attributes
...
- **BlazeCode**: `_internal.ts` / `internal/` directories + explicit export lists
- **BlazeCode**: Every module `pub` regardless of whether it's internal implementation detail
...
- **BlazeCode**: Two separate error enums with overlapping semantics; `IntoServerError` trait exists but is empty
...
- **Gap ID**: GAP-API-009
- **Location**: `blazecode-core/src/provider.rs:24-48` (ModelId, ProviderId), `session.rs:83-90` (SessionId, MessageId, PartId)
- **BlazeCode**: Branded string types with runtime validation
- **BlazeCode**: All IDs are `pub type X = String` — zero compile-time type safety
...
- **BlazeCode**: `packages/sdk/openapi.json` — auto-generated OpenAPI spec driving SDK generation
- **BlazeCode**: No OpenAPI spec; no SDK generation; API surface documented only in code comments
...
- **Gap ID**: GAP-API-011
- **Location**: `blazecode-lsp/src/lib.rs:48-113` (LspError)
- **BlazeCode**: LSP errors part of unified error hierarchy
- **BlazeCode**: `blazecode-lsp` defines its own `LspError` enum (10 variants); does NOT use `blazecode_core::error::Error`
...
- **Recommendation**: Remove `LspError`; use `blazecode_core::error::Error` with Lsp variant; implement `From<LspError>`
- **Severity**: High
- **Source Agent**: Agent 09
...
- **Gap ID**: GAP-TEST-001
- **Location**: `crates/blazecode-core/src/integration.rs:885` (30 tests), `database.rs:3304` (20 tests)
- **BlazeCode**: `http-recorder` package for deterministic replay of LLM provider API calls; Effect test layers
- **BlazeCode**: No HTTP recording/replay infrastructure; provider tests cannot safely execute code paths
- **Description**: No equivalent of BlazeCode's `http-recorder` — cannot deterministically replay LLM provider conversations
...
- **BlazeCode**: Playwright-based e2e in `packages/app/e2e/` (smoke, regression, performance)
- **BlazeCode**: Zero end-to-end tests
...
- **Gap ID**: GAP-TEST-003
- **Location**: All crates
- **BlazeCode**: Effect's Layer system for DI; `http-recorder` records/replays HTTP traffic
- **BlazeCode**: No mocking framework (`mockall`, `mockito`, `wiremock` not in deps); providers are traits but tests don't mock them
...
- **Gap ID**: GAP-TEST-004
- **Location**: All crates
- **BlazeCode**: No explicit coverage tool, but `bun test --coverage` available
- **BlazeCode**: No coverage tool configured; estimated coverage ~60-70% but 11 modules have zero tests
...
- **Gap ID**: GAP-TEST-005
- **Location**: All crates
- **BlazeCode**: `packages/core/test/fixture/` provides `tmpdir.ts`, `git.ts`, `location.ts`, `recordings/`
- **BlazeCode**: `tempfile` crate used ad-hoc; no shared test fixtures, no database test setup helpers
...
- **BlazeCode**: 20+ `@ai-sdk/*` packages (Anthropic, OpenAI, Google, Bedrock, Azure, Groq, etc.)
- **BlazeCode**: 0 provider-specific crates; must implement 15+ HTTP protocol adapters from scratch
...
- **Gap ID**: GAP-DEP-002
- **Location**: `Cargo.toml`
- **BlazeCode**: `@octokit/rest`, `@octokit/graphql`
- **BlazeCode**: `git2` not listed as dependency
- **Description**: Git library dependency missing; BlazeCode uses GitHub API for many operations
...
- **Gap ID**: GAP-DEP-003
- **Location**: `Cargo.toml`
- **BlazeCode**: `zod` 4.x for schema validation
- **BlazeCode**: `schemars` generates JSON Schema but does NOT validate
...
- **BlazeCode**: All 5 packages fully implemented
- **BlazeCode**: LSP, MCP, server, TUI crates labeled "stub" — ~70% of runtime logic missing or simplified
...
- **BlazeCode**: ~50 fields (component states, app state, backend services, streaming state, toggle flags, overlay states, etc.)
...
- **BlazeCode**: 19 positional parameters (17 `Option`); every setter passes 14-17 `None` values
...
- **BlazeCode**: TS source split into 668 files across 5 packages; most files 50-300 lines
- **BlazeCode**: 14 files contain all logic; `blazecode-core` alone has 13+ files all >1000 lines
...
- **BlazeCode**: Effect TestLayer, 1000s of tests
- **BlazeCode**: ~54 trivial unit tests; no integration tests; no mocking infrastructure
...
- **BlazeCode**: TypeScript enforces `strict: true` + `noUnusedLocals` + `noUnusedParameters`
- **BlazeCode**: `#![allow(dead_code, unused_imports, unused_variables)]` on both core library and binary crate
...
- **BlazeCode**: React handles this via props/context
- **BlazeCode**: `TuiApp::new()` and `TuiApp::new_remote()` are 80% identical — ~100 lines duplicated
...
- **BlazeCode**: Single 285-line match on `LlmEvent` with 13 arms, each containing nested match/if-let chains
...
- **BlazeCode**: Effect.gen `Session.update()` with `SessionPatch`
- **BlazeCode**: 10+ individual setter methods each calling `self.db.update_session(...)` with 19 params and 17 `None`s
...
- **BlazeCode**: 25+ handlers each repeat 6-10 lines of identical error wrapping (`match result { Ok => Json, Err => 500 }`)
...
- **BlazeCode**: Bun native runtime — near-instant startup; `bun dev` starts in <1s
- **BlazeCode**: CLAUDE.md Rule #1 prohibits all local compilation; CI-only model means zero local feedback loops
...
- **BlazeCode**: Comprehensive README.md with 26 locale translations
- **BlazeCode**: No user-facing README at all; `CLAUDE.md` serves as primary doc but targets AI agents
...
- **BlazeCode**: `bun install` (5-15s), `bun dev` (instant); Nix flake available
- **BlazeCode**: First build: 87 workspace dependencies, 15-30min; CLAUDE.md Rule #1 prohibits local build
...
- **BlazeCode**: Comprehensive CONTRIBUTING.md (299 lines) with PR expectations, style preferences, debug setup
- **BlazeCode**: No human-facing contribution documentation; all developer guidance in CLAUDE.md (targets AI agents)
...
- **BlazeCode**: Turborepo watch mode; `bun --watch`; Vite hot reload
- **BlazeCode**: `notify` crate in deps but no watch/reload mechanism; CLAUDE.md prohibits local builds
...
- **BlazeCode**: Turbo cache (remote caching); sccache; parallel job splitting
- **BlazeCode**: `Swatinem/rust-cache@v2` only; no sccache; no parallel job splitting
- **Description**: CI rebuilds I/O-bound on cache restore; first-time CI takes 20-40min
- **Consequence**: Developer feedback loop is 15-25 min vs BlazeCode's 5-10 min
...
- **BlazeCode**: `oxlint` + `prettier` + TypeScript compiler; `.editorconfig`
- **BlazeCode**: `cargo fmt` + `cargo clippy` (CI-only); no pre-commit hooks; relaxed lints
...
- **BlazeCode**: .vscode settings + launch configs; .zed/ config; debugger setup guide
- **BlazeCode**: No `.vscode/`, no `.zed/`, no rust-analyzer settings, no debug launch configurations
...
- **Location**: `blazecode/` (no hooks), `blazecode/.husky/pre-push`
- **BlazeCode**: Husky pre-push hook checking Bun version + typecheck
- **BlazeCode**: No pre-commit or pre-push hooks; CLAUDE.md explicitly prohibits local tooling
...
- **BlazeCode**: Full debug guide in CONTRIBUTING.md; VSCode launch configurations; `--inspect` flags
- **BlazeCode**: No debug documentation; no VSCode launch configs; CLI prohibits cargo build locally
...
- **BlazeCode**: Honeycomb for distributed tracing; Sentry for error tracking; 6 Honeycomb Triggers with Discord alerts; log processor pipeline
- **BlazeCode**: Zero monitoring infrastructure; no crash reporting; no performance tracking; no telemetry
...
- **BlazeCode**: Full observability stack — OTLP → Honeycomb; Sentry; structured alerting
- **BlazeCode**: Has tracing deps but no OTEL exporter wired; no Sentry; no metrics
...
- **Location**: `infra/console.ts:11-43` (BlazeCode), `storage.rs:658-697` (BlazeCode)
- **BlazeCode**: PlanetScale automatic backups + branching + point-in-time recovery
- **BlazeCode**: SQLite local file with zero backup mechanism
...
- **Location**: `.github/workflows/release.yml:166-180` (BlazeCode signing)
- **BlazeCode**: Azure Trusted Signing (Windows), Apple codesign (macOS); npm/Homebrew/AUR distribution
- **BlazeCode**: GPG signing only; no Windows/macOS code signing; no package manager distribution
...
- **BlazeCode**: Projectors run after transaction commits (post-commit hooks)
- **BlazeCode**: Projectors run inside the database transaction
...
- **BlazeCode**: Single transaction for event write + sequence update
- **BlazeCode**: `commit_sync_event` does NOT use a transaction — calls `db.insert_event()` then `db.upsert_event_sequence()` as separate queries
...
- **Gap ID**: GAP-DB-003
- **Location**: `database.rs:472-807` (BlazeCode), `schema.gen.ts` (BlazeCode)
- **BlazeCode**: Drizzle ORM `sqliteTable("name", {...})` with compile-time schema verification
- **BlazeCode**: Duplicates 20 table schemas as `const &str` SQL literals — no compile-time verification
...
- **BlazeCode**: Drizzle eager loading with JOINs
- **BlazeCode**: N+1 query problem — `get_messages_with_parts` performs 1 query for messages + N queries for parts
...
- **BlazeCode**: Account tokens stored in `text` columns (same issue)
- **BlazeCode**: No encryption at rest for `access_token` or `refresh_token` in SQLite
...
- **BlazeCode**: No global lock
- **BlazeCode**: `StdMutex<()>` global lock serializes ALL snapshot operations across ALL sessions/projects
...
- **Gap ID**: GAP-RELY-001
- **Location**: `crates/blazecode-core/src/database.rs:1280-1350`
- **BlazeCode**: Effect's `tryPromise` + typed errors; Drizzle ORM ensures schema consistency
- **BlazeCode**: `sqlx::query` (unchecked) instead of `sqlx::query!` (compile-time checked)
...
- **Gap ID**: GAP-RELY-002
- **Location**: `crates/blazecode-core/src/flock.rs:222-227`
- **BlazeCode**: Effect's structured concurrency + Fiber.join guarantees
- **BlazeCode**: TOCTOU race — checks `is_stale()`, creates `.breaker`, then re-checks staleness
...
- **BlazeCode**: Effect's built-in retry with `Schedule` (exponential backoff, jitter, max retries)
- **BlazeCode**: `LlmErrorReason::is_retryable()` defined but **never called**; no automatic retry implementation
...
- **BlazeCode**: Effect's `Effect.timeout()` at every layer
- **BlazeCode**: `provider.stream()` called with no timeout; `provider.complete()` has no timeout
...
- **BlazeCode**: Effect's `Fiber` handles SIGINT/SIGTERM with graceful shutdown sequence
- **BlazeCode**: No signal handling — Ctrl+C causes immediate, ungraceful termination
...
- **BlazeCode**: Effect's typed error system preserves full error context
- **BlazeCode**: `dispatch_inner` returns raw `i32` exit codes — all error context discarded
...
- **BlazeCode**: Transactional pipeline: validate → guard → persist → project → notify
- **BlazeCode**: Listener/notification failures after DB commit cannot be rolled back
...
- **BlazeCode**: Effect's `Ref` with clear transition validation
- **BlazeCode**: No runtime state machine validation; invalid sequences rely on caller correctness
...
- **BlazeCode**: `execa` git calls with Effect's tryPromise + timeout
- **BlazeCode**: `std::process::Command` (blocking) inside async context; no timeout on git commands
...
- **BlazeCode**: Effect's `Stream` with built-in error recovery
- **BlazeCode**: Control flow encoded as string inside `Error::Internal` — fragile string-parsing approach
...
- **BlazeCode**: Drizzle transaction for message removal
- **BlazeCode**: `DELETE FROM session_message` queries without wrapping transaction
...
- **BlazeCode**: Persists each tool call result and LLM event incrementally
- **BlazeCode**: No incremental persistence during tool loop; only final SessionRunResult returned
...
- **BlazeCode**: Effect's `CircuitBreaker` for provider calls
- **BlazeCode**: No circuit breaker anywhere in codebase
...
- **BlazeCode**: Fallback chains — if primary provider fails, try next configured provider
- **BlazeCode**: No provider fallback mechanism; single provider = single point of failure
...
- **BlazeCode**: Concurrent tool execution via `Promise.allSettled()`
- **BlazeCode**: Sequential tool execution in `for` loop — early exit on first failure
...
- **BlazeCode**: RunCoordinator tracks all in-flight drains with global shutdown
- **BlazeCode**: No mechanism to interrupt all active sessions on shutdown
...
- **BlazeCode**: Same PRAGMAs + transactional writes everywhere
- **BlazeCode**: PRAGMAs match but some code paths perform SQL writes without transactions
...
- **BlazeCode**: `fs.writeFileSync` guarantees flush to disk
- **BlazeCode**: `std::fs::write()` does NOT guarantee data is flushed to disk
...
- **Gap ID**: GAP-RELY-019
- **Location**: `snapshot.rs:138`
- **BlazeCode**: Single-threaded, no blocking mutex concerns
- **BlazeCode**: `std::sync::Mutex<()>` in SnapshotService used inside async code
...
- **BlazeCode**: Effect v4 — structured concurrency, algebraic effects, type-safe DI, `Effect.gen`, `Context.Service`, `Layer`, `Scope`, `Stream`
- **BlazeCode**: Raw `tokio::spawn` + `thiserror` + struct-field DI; no structured concurrency
...
- **Consequence**: BlazeCode will accumulate resource leaks, untestable service graphs, ad-hoc error handling
...
- **Gap ID**: GAP-COMP-002
- **Location**: `blazecode-core/src/provider.rs` (Provider trait), `blazecode/packages/llm/src/route/`
- **BlazeCode**: 4-axis route composition (Protocol, Endpoint, Auth, Framing) — 14 OpenAI-compatible providers share 1 protocol implementation
- **BlazeCode**: Simple `Provider` trait; non-OpenAI protocols are separate implementations with zero shared protocol logic
...
- **Gap ID**: GAP-COMP-003
- **Location**: `blazecode-core/src/session.rs`, `blazecode/packages/core/src/session/`
- **BlazeCode**: Algebraic prompt lifecycle with durable inbox, promotion, retry reconciliation
- **BlazeCode**: `Session::prompt(...)` appends to in-memory message list; no durable inbox; no promotion lifecycle
- **Description**: BlazeCode's prompt admission is durable across process restarts; BlazeCode is in-memory
...
- **Gap ID**: GAP-COMP-004
- **Location**: `blazecode-core/src/event.rs`, `blazecode/packages/core/src/event/`
- **BlazeCode**: EventV2 — durable event replay with aggregate sequence cursor
...
- **Gap ID**: GAP-COMP-005
- **Location**: `blazecode-core/src/system_context.rs`, `blazecode/CONTEXT.md:1-129`
- **BlazeCode**: System Context Registry with scoped Context Source producers; epoch-level snapshots; chronological mid-conversation system messages
- **BlazeCode**: No concept of context epochs; system prompt is static string; no mid-conversation context updates
- **Description**: BlazeCode dynamically adds/removes context sources across conversation turns
- **Consequence**: BlazeCode agents cannot adapt to changing context; users must restart sessions
...
- **Location**: `blazecode-core/src/lib.rs:2`, `src/main.rs:2`
...
- **Location**: Every production module in `blazecode-core/src/`
