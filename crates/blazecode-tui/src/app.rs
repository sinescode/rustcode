...
use blazecode_core::bus::SharedBus;
use blazecode_core::git::Git;
use blazecode_core::permission::{PermissionService, ReplyInput};
use blazecode_core::provider::{ChatMessage, LlmEvent, MessageContent, Provider, ToolDefinition};
use blazecode_core::session::{
...
///
/// In `Remote` mode, the TUI connects to a remote blazecode server via HTTP+SSE.
...
pub struct TuiApp {
    ...
    #[allow(dead_code)]
    runner: Option<Arc<blazecode_core::session_runner::SessionRunner>>,
...
impl TuiApp {
    ...
    pub async fn run_async(&mut self) -> anyhow::Result<()> {
        ...
        let mut bus_rx: Option<
            tokio::sync::mpsc::UnboundedReceiver<blazecode_core::bus::GlobalEvent>,
        > = None;
        let mut llm_rx: Option<
            tokio::sync::mpsc::UnboundedReceiver<(String, blazecode_core::provider::LlmEvent)>,
        > = None;
        ...
        if is_remote {
            ...
            let (bus_tx, local_bus_rx) =
                tokio::sync::mpsc::unbounded_channel::<blazecode_core::bus::GlobalEvent>();
            ...
            let (llm_tx, local_llm_rx) = tokio::sync::mpsc::unbounded_channel::<(
                String,
                blazecode_core::provider::LlmEvent,
            )>();
    ...
    fn spawn_llm_stream(&mut self, text: String) {
        ...
        let instructions = [
            "You are a helpful coding assistant running in a terminal (blazecode).".to_string(),
    ...
    fn handle_bus_event(&mut self, event: blazecode_core::bus::GlobalEvent) {
        ...
        {
            match serde_json::from_value::<blazecode_core::permission::PermissionRequest>(
                payload.clone(),
            ) {
                Ok(request) => {
                    tracing::info!(
                    ...
                    DialogType::ModelSelector => {
                        if !self.model_selector.visible {
                            ...
                            let provider_map: std::collections::HashMap<
                                String,
                                blazecode_core::provider::ProviderInfo,
    ...
    fn update_terminal_title(&self) {
        ...
        let title = if self.loading_session {
            "⟳ blazecode — loading session...".to_string()
            ...
            format!("{prefix}blazecode | {model} | {}", &sid[..sid.len().min(12)])
        } else {
            format!("blazecode TUI v{}", self.crate_version)
        };
                                ...
                                tokio::spawn(async move {
                                    ...
                                    let input = blazecode_core::session::CreateSessionInput {
                                        ...
                                        model: model_clone.map(|m| {
                                            blazecode_core::session::ModelSelection {
                                                id: m,
                        ...
                        match sessions.list(None).await {
                            Ok(all) => {
                                ...
                                if let Some(child) = children.first() {
                                    ...
                                    if let Some(bus) = bus {
                                        let _ = bus.publish(blazecode_core::bus::GlobalEvent::new(
                                            serde_json::json!({
                                                "type": "session.select",
                                                "sessionID": child.id,
                                            }),
                        ...
                        match sessions.list(None).await {
                            Ok(list) => {
                                ...
                                if let Some(ref bus) = bus {
                                    ...
                                    let _ = bus.publish(
                                        blazecode_core::bus::GlobalEvent::new(payload),
                                    );
                                }
                            }
    ...
    fn handle_prompt_submit(&mut self, text: String) {
        ...
        if let Some(ref sessions) = self.sessions {
            ...
            tokio::spawn(async move {
                match sessions
                    .create(blazecode_core::session::CreateSessionInput {
                        title: None,
                        agent: Some(agent),
                        model: model,
                        ..Default::default()
        ...
        if let Some(ref perm_svc) = self.permission_service {
            if let Some(ref rid) = request_id {
                ...
                let core_reply = match &reply {
                    PermissionReply::Once => blazecode_core::permission::PermissionReply::Once,
                    PermissionReply::Always => blazecode_core::permission::PermissionReply::Always,
                    PermissionReply::Reject { .. } => {
                        blazecode_core::permission::PermissionReply::Reject
                    }
                };
                let message = match &reply {
                    PermissionReply::Reject { message } => message.clone(),
    ...
    pub fn set_messages(
        &mut self,
        session_id: &str,
        messages: Vec<blazecode_core::session::Message>,
        parts: HashMap<String, Vec<blazecode_core::session::Part>>,
    ) {
    ...
    pub fn show_permission(&mut self, req: blazecode_core::permission::PermissionRequest) {
