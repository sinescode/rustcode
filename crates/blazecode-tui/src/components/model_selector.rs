...
use blazecode_core::provider::{Capabilities, Model, ProviderInfo};
...
impl ModelSelectorState {
    ...
    fn format_cost(cost: &blazecode_core::provider::Cost) -> String {
    ...
    fn capability_tags(caps: &Capabilities) -> String {
        ...
        }
        {
            let has_interleaved = match &caps.interleaved {
                blazecode_core::provider::InterleavedSupport::Bool(b) => *b,
                blazecode_core::provider::InterleavedSupport::Field { .. } => true,
            };
            if has_interleaved {
