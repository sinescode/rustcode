...
//!
//! Ported from: `packages/blazecode/src/server/routes/instance/httpapi/groups/integration.ts`
    ...
    match state.integration_service.get(&integration_id) {
        Some(info) => {
            ...
            let methods: Vec<serde_json::Value> = info
                ...
                .iter()
                .map(|m| match m {
                    blazecode_core::integration::AuthMethod::OAuth(oa) => {
                        serde_json::json!({
                            "type": "oauth",
                            "id": oa.id,
                            "label": oa.label,
                    ...
                    blazecode_core::integration::AuthMethod::Key(k) => {
                        serde_json::json!({
                            "type": "key",
                            "label": k.label,
                        })
                    }
                    blazecode_core::integration::AuthMethod::Env(e) => {
                        serde_json::json!({
                            "type": "env",
                            "names": e.names,
                        })
...
async fn connect_integration(
    ...
    let mut svc = blazecode_core::integration::IntegrationService::new();
