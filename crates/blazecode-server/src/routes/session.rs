...
//!
//! Ported from: `packages/blazecode/src/server/routes/instance/httpapi/groups/session.ts`
//!
...
///
/// # Source
/// `packages/blazecode/src/server/routes/instance/httpapi/groups/session.ts`
...
async fn list_sessions(
    ...
    let input = blazecode_core::session::ListSessionsInput {
...
async fn session_status(
    ...
    let input = Some(blazecode_core::session::ListSessionsInput {
        directory: query.directory,
        path: query.path,
        workspace_id: query.workspace,
        roots: query.roots,
...
async fn create_session(
    ...
    let input = blazecode_core::session::CreateSessionInput {
        ...
        model: payload
            .model
            .map(|m| blazecode_core::session::ModelSelection {
                id: m.id,
...
async fn update_session(
    ...
    let patch = blazecode_core::session::SessionPatch {
...
async fn list_children(
    ...
    let list_input = Some(blazecode_core::session::ListSessionsInput::default());
            ...
            let todos: Vec<serde_json::Value> = messages
                ...
                .flat_map(|msg| {
                    msg.parts.iter().filter_map(|part| {
                        if let blazecode_core::session::Part::Tool(tp) = part {
                            if tp.tool == "todowrite" || tp.tool == "todo_write" {
                                match &tp.state {
                                    blazecode_core::session::ToolState::Completed {
                                        output,
                                        title,
                                        ..
                                    ...
                                    blazecode_core::session::ToolState::Running { .. } => {
                                        Some(serde_json::json!({
                                            "id": tp.id,
                                            "tool": tp.tool,
                                            "state": "running",
                                    ...
                                    blazecode_core::session::ToolState::Pending { .. } => {
                                        Some(serde_json::json!({
                                            "id": tp.id,
                                            "tool": tp.tool,
                                            "state": "pending",
...
async fn get_diff(
    ...
    match state.sessions.get(&session_id).await {
        Ok(session_info) => {
            let git = blazecode_core::git::Git::new(&session_info.directory);
            if git.is_repo() {
...
async fn post_prompt(
    ...
    if let Some(payload_parts) = payload.parts {
        for part in payload_parts {
            if let Some(t) = part.get("text").and_then(|v| v.as_str()) {
                parts.push(blazecode_core::session_prompt::PromptPart::Text(
                    blazecode_core::session_prompt::PromptTextPart {
                        id: None,
                        text: t.to_string(),
                        synthetic: false,
                    },
    ...
    let input = blazecode_core::session_prompt::SessionPromptInput {
        session_id,
        message_id: None,
...
async fn delete_message(
    ...
    match state
        ...
        .update_message(
            &session_id,
            &message_id,
            blazecode_core::session::MessagePatch {
    ...
    match state.sessions.get_messages(&session_id).await {
        Ok(mut messages) => {
            if let Some(msg) = messages.iter_mut().find(|m| m.info.id() == message_id) {
                ...
                msg.parts.retain(|p| {
                    use blazecode_core::session::Part;
                    let pid = match p {
                        Part::Text(tp) => &tp.id,
                        Part::Tool(tp) => &tp.id,
        ...
        Ok(mut messages) => {
            ...
            if let Some(msg) = messages.iter_mut().find(|m| m.info.id() == message_id) {
                for part in &mut msg.parts {
                    match part {
                        blazecode_core::session::Part::Text(tp) if tp.id == part_id => {
...
async fn share_session(
    ...
    let share_url = format!("https://blazecode.ai/share/{session_id}");
    let patch = blazecode_core::session::SessionPatch {
        ..Default::default()
    };
...
async fn init_session(
    ...
    let model_selection = blazecode_core::session::ModelSelection {
...
async fn summarize_session(
        ...
        .map(|m| {
            format!(
                ...
                m.parts
                    .iter()
                    .filter_map(|p| {
                        if let blazecode_core::session::Part::Text(tp) = p {
                            Some(tp.text.as_str())
                        } else {
                            None
                        }
    ...
    let chat_messages = vec![blazecode_core::provider::ChatMessage::User {
        content: blazecode_core::provider::MessageContent::Text(summary_prompt),
    }];
    match provider.complete(&model, &chat_messages, &[]).await {
        Ok(response) => {
            ...
            let summary = blazecode_core::session::SessionSummary {
                additions: 0,
                deletions: 0,
                files: 0,
                diffs: None,
        ...
        if let Some(provider) = state_clone.providers.get(&model_selection.provider_id) {
            if let Ok(model) = provider.get_model(&model_selection.id).await {
                let input = blazecode_core::session_prompt::SessionPromptInput {
                    ...
                    system: None,
                    variant: None,
                    parts: vec![blazecode_core::session_prompt::PromptPart::Text(
                        blazecode_core::session_prompt::PromptTextPart {
                            id: None,
                            text: txt,
                            synthetic: false,
...
async fn post_command(
    ...
    match state.tools.get(&payload.command) {
        Some(tool_def) => {
            ...
            let ctx = blazecode_core::tool::ToolContext {
                ...
                messages: std::sync::Arc::from([] as [blazecode_core::provider::ChatMessage; 0]),
                ask_fn: None,
                permission_source: Some(blazecode_core::permission::PermissionSource::Session {
                    session_id: session_id.clone(),
                }),
                prompt_ops: None,
            };
...
async fn revert_session(
    ...
    let patch = blazecode_core::session::SessionPatch {
        revert: Some(Some(blazecode_core::session::RevertInfo {
...
async fn permission_respond(
    ...
    let reply = match payload.response.to_lowercase().as_str() {
        ...
        "reject" | "deny" => blazecode_core::permission::PermissionReply::Reject,
        _ => {
            return (
                axum::http::StatusCode::BAD_REQUEST,
                Json(serde_json::json!({
