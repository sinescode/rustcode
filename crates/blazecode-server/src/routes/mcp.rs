...
//!
//! Ported from: `packages/blazecode/src/server/routes/instance/httpapi/groups/mcp.ts`
...
use blazecode_core::mcp::{sanitize_name, McpServerConfig, McpServerRegistry};
...
async fn mcp_connect(
    ...
    let client = match store.connect(&name).await {
        ...
        Err(e) => {
            ...
            let status = if matches!(e, blazecode_core::error::Error::McpNotFound { .. }) {
...
async fn register_mcp_tools(
    tool_registry: &blazecode_core::tool::ToolRegistry,
    client: std::sync::Arc<blazecode_core::mcp::McpClient>,
) -> usize {
