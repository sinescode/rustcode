...
//!
//! Ported from: `packages/blazecode/src/server/routes/instance/httpapi/groups/instance.ts`
...
async fn path_info(State(_): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    Json(serde_json::json!({
        "home": home_dir(),
        "state": format!("{}/.local/state/blazecode", home_dir()),
        "config": format!("{}/.config/blazecode", home_dir()),
...
async fn vcs_diff(
    ...
    let git = blazecode_core::git::Git::new(&dir);
    if git.is_repo() {
        let opts = blazecode_core::git::PatchOptions {
...
async fn list_skills(State(_state): State<Arc<AppState>>) -> impl IntoResponse {
    // Scan .blazecode/skills/ directory for skill definitions
    let cwd = std::env::current_dir().unwrap_or_else(|_| std::path::PathBuf::from("."));
...
async fn lsp_status(State(_state): State<Arc<AppState>>) -> impl IntoResponse {
    // LSP integration status — placeholder until blazecode-lsp is implemented
    Json(serde_json::json!({
        "status": "not_available",
        "message": "LSP integration is planned for the blazecode-lsp crate",
    }))
}
