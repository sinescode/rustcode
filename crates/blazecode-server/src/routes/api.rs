...
async fn api_session_prompt(
    ...
    let parts = vec![blazecode_core::session_prompt::PromptPart::Text(
    ...
    if let Some((_pid, provider)) = state.providers.iter().next() {
        if let Ok(model) = provider.list_models().await.and_then(|models| {
            models.into_iter().next().ok_or_else(|| blazecode_core::error::Error::Internal("no models".to_string()))
        }) {
...
async fn api_credential(
    ...
    match blazecode_core::config::Config::save_auth(&payload.provider_id, &cred) {
...
async fn api_skill(State(_state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let skills_dir = cwd.join(".blazecode").join("skills");
...
async fn api_question_reply(
    ...
    let answers: Vec<blazecode_core::question::QuestionAnswer> = payload
        .answers
        .into_iter()
        .map(blazecode_core::question::QuestionAnswer::new)
        .collect();
    let request_id_obj = blazecode_core::question::QuestionId::new_unchecked(&request_id);
