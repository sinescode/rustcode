//! Error standardization — unified error types producing blazecode-compatible JSON.
//!
//! Ported from: `packages/server/src/errors.ts`
//!
//! All errors serialize to the blazecode wire format:
//! ```json
...
/// A unified server error that maps to blazecode error classes.
///
/// Each variant corresponds to a specific HTTP status code and
/// serializes to the blazecode JSON error format.
...
impl ServerError {
    /// Return the blazecode error class name.
    ...
    /// Build the JSON body in blazecode error format.
