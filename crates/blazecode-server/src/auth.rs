//! Auth middleware — checks `BLAZECODE_SERVER_PASSWORD` env var.
//!
//! Clients authenticate via:
//! - `Authorization: Basic <base64>` header (username: `blazecode` by default)
//! - `auth_token` query parameter (base64-encoded `username:password`)
//!
//! Public endpoints (`/health`, `/version`, `/global/health`) bypass auth.
//!
//! Ported from: `packages/blazecode/src/server/routes/instance/httpapi/middleware/authorization.ts`
...
pub struct AuthConfig {
    ...
    /// The expected username (defaults to `"blazecode"`).
    pub username: String,
}
...
impl AuthConfig {
    ...
    ///
    /// Reads:
    /// - `BLAZECODE_SERVER_PASSWORD` (env var, less secure — visible in /proc)
    /// - `BLAZECODE_SERVER_PASSWORD_FILE` (file path, more secure)
    /// - `BLAZECODE_SERVER_USERNAME` (defaults to `"blazecode"`)
    pub fn from_env() -> Self {
        let password = std::env::var("BLAZECODE_SERVER_PASSWORD")
            .ok()
            .or_else(|| {
                std::env::var("BLAZECODE_SERVER_PASSWORD_FILE").ok().and_then(|path| {
                    std::fs::read_to_string(&path).ok().map(|s| s.trim().to_string())
                })
            });
        let username = std::env::var("BLAZECODE_SERVER_USERNAME")
            .unwrap_or_else(|_| "blazecode".to_string());
...
///
/// Checks `BLAZECODE_SERVER_PASSWORD` and authenticates requests.
