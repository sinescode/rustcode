...
//! computes diff after mutation, and emits `x-opencode-sync` response header.
//!
//! Ported from: `packages/opencode/src/server/routes/instance/httpapi/middleware/fence.ts`
//! and `packages/opencode/src/server/shared/fence.ts`

use axum::extract::{Request, State};
use axum::middleware::Next;
use axum::response::{IntoResponse, Response};
use rustcode_core::database::EventSequenceRow;
...
pub const FENCE_HEADER: &str = "x-opencode-sync";
...
///
/// # Source
/// Ported from `packages/opencode/src/server/shared/fence.ts` `load()`.
async fn load_fence_state(
    db: &rustcode_core::database::DatabaseService,
...
///
/// # Source
/// Ported from `packages/opencode/src/server/shared/fence.ts` `diff()`.
...
/// Parse a fence state from a `x-opencode-sync` header value.
///
/// # Source
/// Ported from `packages/opencode/src/server/shared/fence.ts` `parse()`.
...
///
/// # Source
/// Ported from `packages/opencode/src/server/shared/fence.ts` `wait()`.
...
/// computes the diff, and emits it as the `x-opencode-sync` response header.
///
...
pub async fn fence_middleware(
    ...
    // Check if OPENCODE_WORKSPACE_ID env var is set (matches TS `Flag.OPENCODE_WORKSPACE_ID`)
    let has_workspace_id = std::env::var("OPENCODE_WORKSPACE_ID")
        .ok()
