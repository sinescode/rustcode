...
//! # Contract
//!
//! No other code in OpenCode++ may call `std::process::Command`.
...
///
/// This is THE single gate for all process execution in OpenCode++.
///
/// # Contract
///
...
async fn before_spawn_linux(
    ...
    if std::env::var("OPENCODE_LANDLOCK").is_ok() {
    ...
    if std::env::var("OPENCODE_SECCOMP").is_ok() {
