//! # opencode-session-core
//!
//! The formal session state machine. Every session in OpenCode++ is in exactly
//! one `SessionState` at any time. Transitions are validated at compile time.
//!
//! ## Why this exists
//!
//! The original OpenCode session.rs is 4,184 lines with ad-hoc state management
...
//!
//! OpenCode++ replaces this with a typed `SessionState` enum where every
