//! Property-based tests for core RustCode logic.
//!
...
mod tests {
    ...
    proptest! {
        ...
        fn ids_are_unique(count: u32) {
            ...
            let mut ids = Vec::new();
            for _ in 0..count {
                let id = rustcode_core::id::ascending(
        ...
        fn ids_are_ascending(count: u32) {
            ...
            for _ in 0..count {
                let id = rustcode_core::id::ascending(
                    rustcode_core::id::IdPrefix::Part,
                    None,
                ).unwrap();
                ids.push(id);
            }
        ...
        fn ids_have_correct_prefix(tag: String) {
            let prefix = match tag.to_lowercase().as_str() {
                "session" => rustcode_core::id::IdPrefix::Session,
                "message" => rustcode_core::id::IdPrefix::Message,
                "part" => rustcode_core::id::IdPrefix::Part,
                _ => rustcode_core::id::IdPrefix::Event,
            };
            let id = rustcode_core::id::ascending(prefix, None).unwrap();
            let expected_prefix = match prefix {
                rustcode_core::id::IdPrefix::Session => "ses_",
                rustcode_core::id::IdPrefix::Message => "msg_",
                rustcode_core::id::IdPrefix::Part => "part_",
                rustcode_core::id::IdPrefix::Event => "evt_",
                _ => "evt_",
            };
        ...
        fn session_info_roundtrip_preserves_data(
            ...
            let info = rustcode_core::session::SessionInfo {
                id: id.clone(),
                slug,
                directory,
                ..Default::default()
            ...
            let restored: rustcode_core::session::SessionInfo =
    ...
    fn encryption_roundtrip_arbitrary_bytes() {
        use rustcode_core::encryption::hmac::{EncryptionService, KEY_LENGTH};
