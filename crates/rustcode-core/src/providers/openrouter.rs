...
use crate::provider::{ChatMessage, ContentPart, FinishReason, LlmEvent, MessageContent, Model, Provider, ToolDefinition};
...
impl OpenRouterProvider {
    ...
    pub fn with_key(api_key: String, base_url: String) -> Result<Self, Error> {
        let client = reqwest::Client::builder()
            ...
            .build().map_err(|e| Error::Network(format!("HTTP: {e}")))?;
        Ok(Self { api_key, base_url, http_client: client, models: vec![
    ...
    fn chat_url(&self) -> String { format!("{}/chat/completions", self.base_url.trim_end_matches('/')) }
...
fn text_of(c: &MessageContent) -> String {
    match c { MessageContent::Text(t) => t.clone(), MessageContent::Parts(p) => p.iter().filter_map(|p| if let ContentPart::Text{text}=p {Some(text.as_str())}else{None}).collect::<Vec<_>>().join("") }
}

fn mk(id: &str, name: &str, family: &str) -> Model {
    Model { id: id.into(), provider_id: "openrouter".into(), name: name.into(),
        api: crate::provider::ApiInfo { id: id.into(), url: DEFAULT_BASE_URL.into(), npm: "@openrouter/ai-sdk-provider".into() },
        family: Some(family.into()),
        capabilities: crate::provider::Capabilities { temperature: true, reasoning: true, attachment: true, toolcall: true, ..Default::default() },
        cost: Default::default(), limit: crate::provider::TokenLimit { context: 200_000, input: None, output: 16_384 },
...
impl Provider for OpenRouterProvider {
    ...
    fn npm(&self) -> &str { "@openrouter/ai-sdk-provider" }
    ...
    async fn get_model(&self, id: &str) -> crate::error::Result<Model> {
        self.models.iter().find(|m| m.id == id).cloned()
            .ok_or_else(|| Error::ModelNotFound { provider_id: "openrouter".into(), model_id: id.into() })
    }
    ...
    async fn stream(&self, model: &Model, messages: &[ChatMessage], tools: &[ToolDefinition]) -> crate::error::Result<Box<dyn futures::Stream<Item = crate::error::Result<LlmEvent>> + Send + Unpin>> {
        ...
        let resp = self.http_client.post(self.chat_url())
            .header("Authorization", format!("Bearer {}", self.api_key))
            .header("Content-Type", "application/json")
            .header("HTTP-Referer", "https://github.com/sinescode/rustcode")
            .json(&body).send().await.map_err(|e| Error::Network(format!("OpenRouter: {e}")))?;

        if !resp.status().is_success() {
            let s = resp.status().as_u16(); let t = resp.text().await.unwrap_or_default();
            return Err(Error::Llm { module: "openrouter".into(), method: "stream".into(), reason: Box::new(LlmErrorReason::UnknownProvider { message: format!("HTTP {s}: {t}"), status: Some(s) }) });
        }

        let sse = crate::sse::parse_sse_stream(resp);
        let llm = futures::stream::unfold(
            (Box::pin(sse) as Pin<Box<dyn futures::Stream<Item=Result<crate::sse::SseEvent,crate::sse::SseError>>+Send+Unpin>>, false, VecDeque::new()),
            |(mut s, mut done, mut buf)| async move {
                loop {
                    if let Some(ev) = buf.pop_front() { return Some((ev, (s, done, buf))); }
                    if done { return None; }
                    match s.next().await {
                            ...
                            if let Ok(v) = serde_json::from_str::<serde_json::Value>(&e.data) {
                                if let Some(ch) = v["choices"][0].as_object() {
                                    if let Some(dc) = ch["delta"]["content"].as_str() {
                                        buf.push_back(Ok(LlmEvent::TextDelta{id:"text-0".into(),text:dc.into(),provider_metadata:None}));
                                    }
                                    if let Some(fr) = ch["finish_reason"].as_str() {
                                        ...
                                        buf.push_back(Ok(LlmEvent::Finish{reason,usage:None,provider_metadata:None})); done=true;
                                    }
                                }
                            }
                            if let Some(ev) = buf.pop_front() { return Some((ev, (s, done, buf))); }
                        }
                        Some(Err(e)) => return Some((Err(Error::ResponseStream(format!("OpenRouter SSE: {e}"))), (s, done, buf))),
                        None => return None, _ => continue,
                    }
                }
            });
        Ok(Box::new(llm))
    ...
    async fn complete(&self, m: &Model, msgs: &[ChatMessage], t: &[ToolDefinition]) -> crate::error::Result<crate::provider::LlmResponse> {
        let mut s = self.stream(m, msgs, t).await?; let mut evs=vec![];
        while let Some(r) = s.next().await { if let Ok(e) = r { evs.push(e); } }
        Ok(crate::provider::LlmResponse{events:evs,usage:None})
    }
}
