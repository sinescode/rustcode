            ...
            |(mut sse, mut finished, mut buf)| async move {
                loop {
                    ...
                    match sse.next().await {
                        ...
                        Some(Err(e)) => return Some((Err(Error::ResponseStream(format!("Fireworks SSE: {e}"))), (sse, finished, buf))),
                        None => return None,
                        _ => continue,
                    }
                }
...
fn build_model_catalog() -> Vec<Model> {
    vec![
        make_simple_model("accounts/fireworks/models/llama-v3p3-70b-instruct", "Llama 3.3 70B Instruct", PROVIDER_ID, BASE_URL, "llama", 128_000, 8_192),
        make_simple_model("accounts/fireworks/models/mixtral-8x22b-instruct", "Mixtral 8x22B Instruct", PROVIDER_ID, BASE_URL, "mixtral", 65_000, 4_096),
        make_simple_model("accounts/fireworks/models/qwen2p5-72b-instruct", "Qwen 2.5 72B Instruct", PROVIDER_ID, BASE_URL, "qwen", 32_000, 4_096),
    ]
}
...
mod tests {
    ...
    fn test_model_catalog_has_llama_v3p3() {
        let models = build_model_catalog();
        let m = models.iter().find(|m| m.id == "accounts/fireworks/models/llama-v3p3-70b-instruct").expect("llama-v3p3-70b-instruct not found");
    ...
    fn test_model_catalog_has_mixtral_8x22b() {
        let models = build_model_catalog();
        let m = models.iter().find(|m| m.id == "accounts/fireworks/models/mixtral-8x22b-instruct").expect("mixtral-8x22b-instruct not found");
    ...
    fn test_model_catalog_has_qwen2p5() {
        let models = build_model_catalog();
        let m = models.iter().find(|m| m.id == "accounts/fireworks/models/qwen2p5-72b-instruct").expect("qwen2p5-72b-instruct not found");
    ...
    fn test_provider_id_and_npm() {
        let provider = FireworksProvider::with_base_url("test-key".into(), BASE_URL.into()).expect("construct");
    ...
    fn test_get_model_found() {
        let provider = FireworksProvider::with_base_url("test-key".into(), BASE_URL.into()).expect("construct");
        let m = provider.get_model("accounts/fireworks/models/llama-v3p3-70b-instruct");
        assert!(m.is_ok());
        assert_eq!(m.unwrap().id, "accounts/fireworks/models/llama-v3p3-70b-instruct");
    }
    ...
    fn test_chat_url() {
        let provider = FireworksProvider::with_base_url("test-key".into(), BASE_URL.into()).expect("construct");
        assert_eq!(provider.chat_url(), "https://api.fireworks.ai/inference/v1/chat/completions");
    }
