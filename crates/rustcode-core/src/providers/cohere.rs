                ...
                Box::pin(async move {
                    loop {
                        ...
                        match sse.next().await {
                            ...
                            Some(Err(e)) => {
                                return Some((
                                    Err(Error::ResponseStream(format!("Cohere SSE: {e}"))),
                                    (sse, finished, buf),
                                ))
                            }
                            None => return None,
...
mod tests {
    ...
    fn test_model_catalog_has_command_r_plus() {
        ...
        let m = models
            .iter()
            .find(|m| m.id == "command-r-plus")
            .expect("command-r-plus not found");
    ...
    fn test_model_catalog_has_command_r() {
        ...
        let m = models
            .iter()
            .find(|m| m.id == "command-r")
            .expect("command-r not found");
        assert_eq!(m.name, "Command R");
    ...
    fn test_provider_id_and_npm() {
        let provider =
            CohereProvider::with_base_url("test-key".into(), BASE_URL.into()).expect("construct");
    ...
    async fn test_get_model_found() {
        let provider =
            CohereProvider::with_base_url("test-key".into(), BASE_URL.into()).expect("construct");
        let m = provider.get_model("command-r-plus").await;
        assert!(m.is_ok());
