//! Cohere provider — OpenAI-compatible Chat Completions protocol.
//!
//! Cohere provides enterprise-grade LLM inference for retrieval-augmented
//! generation and agentic workloads. Uses the standard OpenAI Chat
//! Completions wire format with SSE streaming.
//!
//! Base URL: https://api.cohere.ai/v1
//! Auth: Bearer token via COHERE_API_KEY env var.
//! npm: @ai-sdk/cohere
//!
//! Ported from:
...
const NPM: &str = "@ai-sdk/cohere";

fn resolve_api_key() -> Result<String, Error> {
    std::env::var("COHERE_API_KEY")
        .ok()
        .filter(|k| !k.is_empty())
        .ok_or_else(|| Error::Auth("COHERE_API_KEY environment variable not set".into()))
}
...
impl Provider for CohereProvider {
    ...
    async fn stream(&self, model: &Model, messages: &[ChatMessage], tools: &[ToolDefinition]) -> crate::error::Result<Box<dyn futures::Stream<Item = crate::error::Result<LlmEvent>> + Send + Unpin>> {
        ...
        let response = self.http_client.post(self.chat_url())
            ...
            .map_err(|e| Error::Network(format!("Cohere request: {e}")))?;
            ...
            |(mut sse, mut finished, mut buf)| async move {
                loop {
                    ...
                    match sse.next().await {
                        ...
                        Some(Err(e)) => return Some((Err(Error::ResponseStream(format!("Cohere SSE: {e}"))), (sse, finished, buf))),
                        None => return None,
                        _ => continue,
                    }
                }
...
fn build_model_catalog() -> Vec<Model> {
    vec![
        make_simple_model("command-r-plus", "Command R+", PROVIDER_ID, BASE_URL, "command-r", 128_000, 4_096),
        make_simple_model("command-r", "Command R", PROVIDER_ID, BASE_URL, "command-r", 128_000, 4_096),
        make_simple_model("command", "Command", PROVIDER_ID, BASE_URL, "command", 4_096, 4_096),
    ]
}
...
mod tests {
    ...
    #[test]
    fn test_model_catalog_has_command_r_plus() {
        let models = build_model_catalog();
        let m = models.iter().find(|m| m.id == "command-r-plus").expect("command-r-plus not found");
        assert_eq!(m.name, "Command R+");
        assert_eq!(m.provider_id, PROVIDER_ID);
        assert_eq!(m.limit.context, 128_000);
        assert_eq!(m.family.as_deref(), Some("command-r"));
    }

    #[test]
    fn test_model_catalog_has_command_r() {
        let models = build_model_catalog();
        let m = models.iter().find(|m| m.id == "command-r").expect("command-r not found");
        assert_eq!(m.name, "Command R");
        assert_eq!(m.limit.output, 4_096);
    }

    #[test]
    fn test_model_catalog_has_command() {
        let models = build_model_catalog();
        let m = models.iter().find(|m| m.id == "command").expect("command not found");
        assert_eq!(m.name, "Command");
        assert_eq!(m.limit.context, 4_096);
    }

    #[test]
    fn test_provider_id_and_npm() {
        let provider = CohereProvider::with_base_url("test-key".into(), BASE_URL.into()).expect("construct");
    ...
    fn test_get_model_found() {
        let provider = CohereProvider::with_base_url("test-key".into(), BASE_URL.into()).expect("construct");
        let m = provider.get_model("command-r-plus");
        assert!(m.is_ok());
        assert_eq!(m.unwrap().id, "command-r-plus");
    }
    ...
    fn test_chat_url() {
        let provider = CohereProvider::with_base_url("test-key".into(), BASE_URL.into()).expect("construct");
        assert_eq!(provider.chat_url(), "https://api.cohere.ai/v1/chat/completions");
    }
