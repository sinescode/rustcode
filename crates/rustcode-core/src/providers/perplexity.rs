//! Perplexity provider — OpenAI-compatible Chat Completions protocol.
//!
//! Perplexity provides search-augmented LLM inference. Uses the standard
//! OpenAI Chat Completions wire format with SSE streaming.
//!
//! Base URL: https://api.perplexity.ai
//! Auth: Bearer token via PPLX_API_KEY env var.
...
const PROVIDER_ID: &str = "perplexity";
const NPM: &str = "@ai-sdk/perplexity";

fn resolve_api_key() -> Result<String, Error> {
    std::env::var("PPLX_API_KEY")
        .ok()
        .filter(|k| !k.is_empty())
        .ok_or_else(|| Error::Auth("PPLX_API_KEY environment variable not set".into()))
}
...
mod tests {
    ...
    fn test_model_catalog_has_sonar_pro() {
        ...
        assert_eq!(m.family.as_deref(), Some("sonar"));
    }
