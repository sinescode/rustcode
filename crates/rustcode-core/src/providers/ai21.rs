//! AI21 Labs provider — OpenAI-compatible Chat Completions protocol.
//!
//! AI21 Labs provides the Jamba series of hybrid SSM-Transformer models
//! for long-context tasks. Uses the standard OpenAI Chat Completions
//! wire format with SSE streaming.
//!
//! Base URL: https://api.ai21.com/studio/v1
//! Auth: Bearer token via AI21_API_KEY env var.
//! npm: @ai-sdk/ai21
//!
//! Ported from:
...
const PROVIDER_ID: &str = "ai21";
const NPM: &str = "@ai-sdk/ai21";

fn resolve_api_key() -> Result<String, Error> {
    std::env::var("AI21_API_KEY")
        .ok()
        .filter(|k| !k.is_empty())
        .ok_or_else(|| Error::Auth("AI21_API_KEY environment variable not set".into()))
}
...
impl Provider for Ai21Provider {
    ...
    async fn stream(&self, model: &Model, messages: &[ChatMessage], tools: &[ToolDefinition]) -> crate::error::Result<Box<dyn futures::Stream<Item = crate::error::Result<LlmEvent>> + Send + Unpin>> {
        ...
        let response = self.http_client.post(self.chat_url())
            ...
            .map_err(|e| Error::Network(format!("AI21 request: {e}")))?;
            ...
            |(mut sse, mut finished, mut buf)| async move {
                loop {
                    ...
                    match sse.next().await {
                        ...
                        Some(Err(e)) => return Some((Err(Error::ResponseStream(format!("AI21 SSE: {e}"))), (sse, finished, buf))),
                        None => return None,
                        _ => continue,
                    }
                }
...
fn build_model_catalog() -> Vec<Model> {
    vec![
        make_simple_model("jamba-1.5-large", "Jamba 1.5 Large", PROVIDER_ID, BASE_URL, "jamba", 256_000, 4_096),
        make_simple_model("jamba-1.5-mini", "Jamba 1.5 Mini", PROVIDER_ID, BASE_URL, "jamba", 256_000, 4_096),
    ]
}
...
mod tests {
    ...
    #[test]
    fn test_model_catalog_has_jamba_large() {
        let models = build_model_catalog();
        let m = models.iter().find(|m| m.id == "jamba-1.5-large").expect("jamba-1.5-large not found");
        assert_eq!(m.name, "Jamba 1.5 Large");
        assert_eq!(m.provider_id, PROVIDER_ID);
        assert_eq!(m.limit.context, 256_000);
        assert_eq!(m.family.as_deref(), Some("jamba"));
    }

    #[test]
    fn test_model_catalog_has_jamba_mini() {
        let models = build_model_catalog();
        let m = models.iter().find(|m| m.id == "jamba-1.5-mini").expect("jamba-1.5-mini not found");
        assert_eq!(m.name, "Jamba 1.5 Mini");
        assert_eq!(m.limit.context, 256_000);
    ...
    fn test_provider_id_and_npm() {
        let provider = Ai21Provider::with_base_url("test-key".into(), BASE_URL.into()).expect("construct");
    ...
    fn test_get_model_found() {
        let provider = Ai21Provider::with_base_url("test-key".into(), BASE_URL.into()).expect("construct");
        let m = provider.get_model("jamba-1.5-large");
        assert!(m.is_ok());
        assert_eq!(m.unwrap().id, "jamba-1.5-large");
    }
    ...
    fn test_chat_url() {
        let provider = Ai21Provider::with_base_url("test-key".into(), BASE_URL.into()).expect("construct");
        assert_eq!(provider.chat_url(), "https://api.ai21.com/studio/v1/chat/completions");
    }
