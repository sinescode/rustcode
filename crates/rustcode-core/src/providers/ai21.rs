                ...
                Box::pin(async move {
                    loop {
                        ...
                        match sse.next().await {
                            ...
                            Some(Err(e)) => {
                                return Some((
                                    Err(Error::ResponseStream(format!("AI21 SSE: {e}"))),
                                    (sse, finished, buf),
                                ))
                            }
                            None => return None,
...
mod tests {
    ...
    fn test_model_catalog_has_jamba_large() {
        ...
        let m = models
            .iter()
            .find(|m| m.id == "jamba-1.5-large")
            .expect("jamba-1.5-large not found");
    ...
    fn test_model_catalog_has_jamba_mini() {
        ...
        let m = models
            .iter()
            .find(|m| m.id == "jamba-1.5-mini")
            .expect("jamba-1.5-mini not found");
    ...
    fn test_provider_id_and_npm() {
        let provider =
            Ai21Provider::with_base_url("test-key".into(), BASE_URL.into()).expect("construct");
    ...
    async fn test_get_model_found() {
        let provider =
            Ai21Provider::with_base_url("test-key".into(), BASE_URL.into()).expect("construct");
        let m = provider.get_model("jamba-1.5-large").await;
        assert!(m.is_ok());
