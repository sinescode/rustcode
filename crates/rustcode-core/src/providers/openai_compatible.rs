...
use crate::provider::{ChatMessage, FinishReason, LlmEvent, Model, Provider, ToolDefinition, Usage};
...
pub const PROFILES: &[CompatProfile] = &[
    CompatProfile { provider_id: "deepseek", name: "DeepSeek", base_url: "https://api.deepseek.com/v1", env_var: "DEEPSEEK_API_KEY", npm: "@ai-sdk/deepseek", default_model: "deepseek-chat", family: "deepseek" },
    CompatProfile { provider_id: "groq", name: "Groq", base_url: "https://api.groq.com/openai/v1", env_var: "GROQ_API_KEY", npm: "@ai-sdk/groq", default_model: "llama-4-maverick", family: "llama" },
    CompatProfile { provider_id: "togetherai", name: "TogetherAI", base_url: "https://api.together.xyz/v1", env_var: "TOGETHER_API_KEY", npm: "@ai-sdk/togetherai", default_model: "mistral-large", family: "mistral" },
    CompatProfile { provider_id: "cerebras", name: "Cerebras", base_url: "https://api.cerebras.ai/v1", env_var: "CEREBRAS_API_KEY", npm: "@ai-sdk/cerebras", default_model: "llama-4-maverick", family: "llama" },
    CompatProfile { provider_id: "fireworks", name: "Fireworks", base_url: "https://api.fireworks.ai/inference/v1", env_var: "FIREWORKS_API_KEY", npm: "@ai-sdk/fireworks", default_model: "llama-4-maverick", family: "llama" },
    CompatProfile { provider_id: "deepinfra", name: "DeepInfra", base_url: "https://api.deepinfra.com/v1/openai", env_var: "DEEPINFRA_API_KEY", npm: "@ai-sdk/deepinfra", default_model: "llama-4-maverick", family: "llama" },
    CompatProfile { provider_id: "xai", name: "xAI Grok", base_url: "https://api.x.ai/v1", env_var: "XAI_API_KEY", npm: "@ai-sdk/xai", default_model: "grok-4", family: "grok" },
    CompatProfile { provider_id: "mistral", name: "Mistral", base_url: "https://api.mistral.ai/v1", env_var: "MISTRAL_API_KEY", npm: "@ai-sdk/mistral", default_model: "mistral-large", family: "mistral" },
    CompatProfile { provider_id: "perplexity", name: "Perplexity", base_url: "https://api.perplexity.ai", env_var: "PERPLEXITY_API_KEY", npm: "@ai-sdk/perplexity", default_model: "sonar-pro", family: "sonar" },
    CompatProfile { provider_id: "cohere", name: "Cohere", base_url: "https://api.cohere.ai/v1", env_var: "COHERE_API_KEY", npm: "@ai-sdk/cohere", default_model: "command-r-plus", family: "command-r" },
    CompatProfile { provider_id: "alibaba", name: "Alibaba Qwen", base_url: "https://dashscope-intl.aliyuncs.com/compatible-mode/v1", env_var: "DASHSCOPE_API_KEY", npm: "@ai-sdk/alibaba", default_model: "qwen-max", family: "qwen" },
    CompatProfile { provider_id: "vercel", name: "Vercel AI Gateway", base_url: "https://api.vercel.ai/v1", env_var: "VERCEL_AI_GATEWAY_KEY", npm: "@ai-sdk/gateway", default_model: "auto", family: "auto" },
];
...
impl OpenAICompatibleProvider {
    ...
    pub fn from_profile(profile: &CompatProfile) -> Result<Self, Error> {
        let api_key = std::env::var(profile.env_var).ok().filter(|k| !k.is_empty())
            .ok_or_else(|| Error::Auth(format!("{} environment variable not set", profile.env_var)))?;
        Self::new(&profile.provider_id, &profile.name, &api_key, &profile.base_url, &profile.npm, &profile.default_model, &profile.family)
    }

    /// Try to auto-detect a provider from environment.
    pub fn try_all() -> Vec<Self> {
        PROFILES.iter().filter_map(|p| Self::from_profile(p).ok()).collect()
    }

    /// Generic constructor.
    pub fn new(provider_id: &str, name: &str, api_key: &str, base_url: &str, _npm: &str, default_model: &str, family: &str) -> Result<Self, Error> {
        let http_client = reqwest::Client::builder()
            .user_agent(format!("rustcode/{}", env!("CARGO_PKG_VERSION")))
            .timeout(std::time::Duration::from_secs(300))
            .build().map_err(|e| Error::Network(format!("HTTP client: {e}")))?;
        let models = vec![make_simple_model(default_model, name, provider_id, base_url, family, 128_000, 16_384)];
        Ok(Self { provider_id: provider_id.into(), name: name.into(), api_key: api_key.into(), base_url: base_url.into(), npm: format!("@ai-sdk/{provider_id}"), http_client, models })
    }
...
fn msg_text(content: &crate::provider::MessageContent) -> String {
    match content {
        ...
        crate::provider::MessageContent::Parts(p) => p.iter().filter_map(|p| if let crate::provider::ContentPart::Text { text } = p { Some(text.as_str()) } else { None }).collect::<Vec<_>>().join(""),
    }
}
...
fn make_simple_model(id: &str, name: &str, provider_id: &str, base_url: &str, family: &str, ctx: u64, out: u64) -> Model {
    Model {
        ...
        capabilities: crate::provider::Capabilities { temperature: true, reasoning: false, attachment: false, toolcall: true, ..Default::default() },
...
impl Provider for OpenAICompatibleProvider {
    ...
    async fn get_model(&self, model_id: &str) -> crate::error::Result<Model> {
        self.models.iter().find(|m| m.id == model_id).cloned()
            .ok_or_else(|| Error::ModelNotFound { provider_id: self.provider_id.clone(), model_id: model_id.into() })
    }
    ...
    async fn stream(&self, model: &Model, messages: &[ChatMessage], tools: &[ToolDefinition]) -> crate::error::Result<Box<dyn futures::Stream<Item = crate::error::Result<LlmEvent>> + Send + Unpin>> {
        ...
        let response = self.http_client.post(self.chat_url())
            ...
            .json(&body).send().await.map_err(|e| Error::Network(format!("{} request: {e}", self.provider_id)))?;

        if !response.status().is_success() {
            let status = response.status().as_u16();
            let text = response.text().await.unwrap_or_default();
            return Err(Error::Llm { module: self.provider_id.clone(), method: "stream".into(), reason: Box::new(LlmErrorReason::UnknownProvider { message: format!("HTTP {status}: {text}"), status: Some(status) }) });
        }

        let sse_stream = crate::sse::parse_sse_stream(response);
        let llm_stream = futures::stream::unfold(
            (Box::pin(sse_stream) as Pin<Box<dyn futures::Stream<Item = Result<crate::sse::SseEvent, crate::sse::SseError>> + Send + Unpin>>, false, String::new(), VecDeque::new()),
            |(mut sse, mut finished, mut text, mut buf)| async move {
                loop {
                    if let Some(ev) = buf.pop_front() { return Some((ev, (sse, finished, text, buf))); }
                    ...
                    match sse.next().await {
                        ...
                        Some(Err(e)) => return Some((Err(Error::ResponseStream(format!("{} SSE: {e}", self.provider_id))), (sse, finished, text, buf))),
                        None => return None,
                        _ => continue,
                    }
                }
