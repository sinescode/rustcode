                ...
                Box::pin(async move {
                    loop {
                        ...
                        match sse.next().await {
                            ...
                            Some(Err(e)) => {
                                return Some((
                                    Err(Error::ResponseStream(format!("Cerebras SSE: {e}"))),
                                    (sse, finished, buf),
                                ))
                            }
                            None => return None,
...
mod tests {
    ...
    fn test_model_catalog_has_llama33_70b() {
        ...
        let m = models
            .iter()
            .find(|m| m.id == "llama-3.3-70b")
            .expect("llama-3.3-70b not found");
    ...
    fn test_model_catalog_has_mixtral() {
        ...
        let m = models
            .iter()
            .find(|m| m.id == "mixtral-8x7b")
            .expect("mixtral-8x7b not found");
    ...
    fn test_provider_id_and_npm() {
        let provider =
            CerebrasProvider::with_base_url("test-key".into(), BASE_URL.into()).expect("construct");
    ...
    async fn test_get_model_found() {
        let provider =
            CerebrasProvider::with_base_url("test-key".into(), BASE_URL.into()).expect("construct");
        let m = provider.get_model("llama-4-maverick").await;
        assert!(m.is_ok());
