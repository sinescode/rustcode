                ...
                ChatMessage::User { content } => {
                    ...
                    if media_parts.is_empty() {
                        result.push(OpenAIChatMessage::User {
                            content: OpenAIChatUserContent::Text(text_parts),
                        });
                    } else {
...
fn events_from_chat(event: OpenAIChatEvent, state: &mut ChatStreamState) -> Vec<LlmEvent> {
                ...
                Box::pin(async move {
                    loop {
                        ...
                        match sse.next().await {
                            ...
                            Some(Err(e)) => {
                                return Some((
                                    Err(Error::ResponseStream(format!("OpenAI SSE: {e}"))),
                                    (sse, state, buffer),
                                ))
                            }
                            None => return None,
