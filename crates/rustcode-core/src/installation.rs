...
///
/// Ported from: `packages/opencode/src/installation/index.ts` lines 87–348.
...
impl InstallationService {
    ...
    ///
    /// Ported from: `packages/opencode/src/installation/index.ts` lines 186–219.
    pub fn method(&self) -> Method {
        let exec = std::env::current_exe()
            .map(|p| p.to_string_lossy().to_lowercase())
            .unwrap_or_default();

        if exec.contains(".opencode") && exec.contains("bin") {
            return Method::Curl;
        }
        ...
        let checks: Vec<(Method, &str, &str)> = vec![
            (Method::Npm, "npm", "opencode-ai"),
            (Method::Bun, "bun", "opencode-ai"),
            (Method::Pnpm, "pnpm", "opencode-ai"),
            (Method::Yarn, "yarn", "opencode-ai"),
            (Method::Brew, "brew", "opencode"),
            (Method::Scoop, "scoop", "opencode"),
            (Method::Choco, "choco", "opencode"),
        ];
    ...
    ///
    /// Ported from: `packages/opencode/src/installation/index.ts` lines 220–276.
        ...
        rt.block_on(async {
            match m {
                ...
                Method::Npm | Method::Bun | Method::Pnpm => {
                    ...
                    let url = format!("{}/opencode-ai/latest", registry.trim_end_matches('/'));
                    let resp = self
                        .http_client
                        .get(&url)
                ...
                Method::Choco => {
                    let url = "https://community.chocolatey.org/api/v2/Packages?$filter=Id%20eq%20%27opencode%27%20and%20IsLatestVersion&$select=Version";
                    let resp = self
                        .http_client
                        .get(url)
                ...
                Method::Scoop => {
                    let url = "https://raw.githubusercontent.com/ScoopInstaller/Master/master/bucket/opencode.json";
                    let resp = self
                        .http_client
                        .get(url)
                        .header("Accept", "application/json")
    ...
    ///
    /// Ported from: `packages/opencode/src/installation/index.ts` lines 277–333.
    pub fn upgrade(&self, method: &Method, target: &str) -> Result<(), String> {
        ...
        rt.block_on(async {
            match method {
                ...
                Method::Npm => run_cmd(&["npm", "install", "-g", &format!("opencode-ai@{target}")]).await,
                Method::Pnpm => run_cmd(&["pnpm", "install", "-g", &format!("opencode-ai@{target}")]).await,
                Method::Bun => run_cmd(&["bun", "install", "-g", &format!("opencode-ai@{target}")]).await,
                Method::Yarn => run_cmd(&["yarn", "global", "add", &format!("opencode-ai@{target}")]).await,
                Method::Brew => {
                    let formula = self.get_brew_formula().await;
                ...
                Method::Choco => {
                    run_cmd(&["choco", "upgrade", "opencode", &format!("--version={target}"), "-y"]).await
                }
                Method::Scoop => {
                    run_cmd(&["scoop", "install", &format!("opencode@{target}")]).await
                }
    ...
    async fn get_brew_formula(&self) -> String {
        let tap = run_cmd_str(&["brew", "list", "--formula", "anomalyco/tap/opencode"]).await;
        if tap.contains("opencode") {
            return "anomalyco/tap/opencode".to_string();
        }
        let core = run_cmd_str(&["brew", "list", "--formula", "opencode"]).await;
        if core.contains("opencode") {
