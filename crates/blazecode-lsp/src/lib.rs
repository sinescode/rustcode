...
//! LSP integration for blazecode.
//!
//! Ported from: `packages/blazecode/src/lsp/`
//!
//! ## Architecture
//!
...
/// similar to how blazecode's Effect-based LSP service is globally available.
///
/// # Source
/// Ported from the singleton pattern in `packages/blazecode/src/lsp/lsp.ts`
...
///
/// Ported from `packages/blazecode/src/lsp/client.ts` `DIAGNOSTICS_REQUEST_TIMEOUT_MS` (line 16).
const DIAGNOSTICS_REQUEST_TIMEOUT_MS: u64 = 3_000;

/// Debounce delay for diagnostic push notifications.
///
/// Ported from `packages/blazecode/src/lsp/client.ts` `DIAGNOSTICS_DEBOUNCE_MS` (line 13).
const _DIAGNOSTICS_DEBOUNCE_MS: u64 = 150;

/// Maximum wait for document-level diagnostics (push + pull).
///
/// Ported from `packages/blazecode/src/lsp/client.ts` `DIAGNOSTICS_DOCUMENT_WAIT_TIMEOUT_MS` (line 14).
const DIAGNOSTICS_DOCUMENT_WAIT_TIMEOUT_MS: u64 = 5_000;

/// Maximum wait for full diagnostics (workspace + document).
///
/// Ported from `packages/blazecode/src/lsp/client.ts` `DIAGNOSTICS_FULL_WAIT_TIMEOUT_MS` (line 15).
#[allow(dead_code)]
...
///
/// ```
/// # use blazecode_lsp::frame_lsp_message;
...
///
/// ```
/// # use blazecode_lsp::{frame_lsp_message, parse_lsp_message};
...
///
/// Ported from: `packages/blazecode/src/lsp/server.ts` `RootFunction` (line 30).
...
///
/// Ported from: `packages/blazecode/src/lsp/server.ts` `NearestRoot` (lines 32–54).
...
///
/// Ported from: `packages/blazecode/src/lsp/server.ts` `StrictNearestRoot` (lines 56–78).
...
/// Ported from: `packages/blazecode/src/lsp/server.ts` `Info` (lines 80–86).
...
///
/// Ported from: `packages/blazecode/src/lsp/server.ts`
...
/// 28+ server definitions from TS `packages/blazecode/src/lsp/server.ts`.
///
...
///
/// Ported from: `packages/blazecode/src/lsp/server.ts` — all `export const X: Info`
...
///
/// ```
/// # use blazecode_lsp::get_server_for_file;
...
/// ```
/// # use std::path::Path;
/// # use blazecode_lsp::detect_servers_for_workspace;
...
struct LspClientState {
    ...
    ///
    /// Ported from `packages/blazecode/src/lsp/client.ts` `pullDiagnostics` (line 140).
    ...
    ///
    /// Ported from `packages/blazecode/src/lsp/client.ts` `diagnosticListeners` / `registrationListeners`.
...
impl LspClient {
    ...
    // ------------------------------------------------------------------
    //
    // Ported from `packages/blazecode/src/lsp/client.ts` lines 139–541.

    /// Return merged push + pull diagnostics for all files.
    ///
    /// Ported from `packages/blazecode/src/lsp/client.ts` `diagnostics` getter (lines 623–629).
    ...
    ///
    /// Ported from `packages/blazecode/src/lsp/client.ts` `documentPullState()` (lines 355–365).
    ...
    ///
    /// Ported from `packages/blazecode/src/lsp/client.ts` `workspacePullState()` (lines 367–377).
    ...
    ///
    /// Ported from `packages/blazecode/src/lsp/client.ts` `dedupeDiagnostics()` (lines 91–105).
    ...
    ///
    /// Ported from `packages/blazecode/src/lsp/client.ts` `requestDiagnosticReport()` (lines 293–327).
    ...
    ///
    /// Ported from `packages/blazecode/src/lsp/client.ts` `requestWorkspaceDiagnosticReport()` (lines 329–353).
    ...
    ///
    /// Ported from `packages/blazecode/src/lsp/client.ts` `requestFullDiagnostics()` (lines 429–444).
    ...
    ///
    /// Ported from `packages/blazecode/src/lsp/client.ts` `waitForDocumentDiagnostics()` (lines 499–519)
    ...
    ///
    /// Ported from `packages/blazecode/src/lsp/client.ts` `waitForDiagnostics()` (lines 630–639).
    ...
    ///
    /// # Source
    /// Ported from `packages/blazecode/src/lsp/client.ts` `notify.open()` (lines 554–621).
    pub async fn open_file(&self, file_path: &str) -> Result<u32> {
        ...
        let ext = Path::new(file_path)
            ...
            .map(|e| format!(".{e}"))
            .unwrap_or_default();
        let language_id = blazecode_core::lsp::language_id_for_extension(&ext);
    ...
    ///
    /// # Source
    /// Ported from `packages/blazecode/src/lsp/lsp.ts` `hover()` (lines 379–388).
    ...
    ///
    /// # Source
    /// Ported from `packages/blazecode/src/lsp/lsp.ts` `definition()` (lines 390–400).
    ...
    ///
    /// # Source
    /// Ported from `packages/blazecode/src/lsp/lsp.ts` `references()` (lines 402–413).
    ...
    ///
    /// # Source
    /// Ported from `packages/blazecode/src/lsp/lsp.ts` `implementation()` (lines 415–425).
    ...
    ///
    /// # Source
    /// Ported from `packages/blazecode/src/lsp/lsp.ts` (completion protocol).
    ...
    ///
    /// # Source
    /// Ported from `packages/blazecode/src/lsp/lsp.ts` `prepareCallHierarchy()` (lines 445–455).
    ...
    ///
    /// # Source
    /// Ported from `packages/blazecode/src/lsp/lsp.ts` `incomingCalls()` (lines 474–476).
    ...
    ///
    /// # Source
    /// Ported from `packages/blazecode/src/lsp/lsp.ts` `outgoingCalls()` (lines 478–480).
...
pub struct LspManager {
    ...
    /// Ported from: `packages/blazecode/src/lsp/lsp.ts` — internal dedup via Effect.cached.
    spawn_in_progress: std::sync::Mutex<HashSet<String>>,
    /// Set of server IDs that have been marked as broken (failed to start).
    /// Ported from: `packages/blazecode/src/lsp/lsp.ts` — broken server tracking.
...
impl LspManager {
    ...
    ///
    /// Ported from: `packages/blazecode/src/lsp/lsp.ts` — dedup via Effect.cached.
    ...
    ///
    /// # Source
    /// Ported from `packages/blazecode/src/lsp/lsp.ts` `getClients()` (lines 210–299)
    /// and `packages/blazecode/src/lsp/server.ts` root detection.
...
///
/// # Source
/// Ported from `packages/blazecode/src/lsp/lsp.ts` `pathToFileURL()` usage.
...
mod tests {
    ...
    fn language_id_for_rust() {
        assert_eq!(blazecode_core::lsp::language_id_for_extension(".rs"), "rust");
    }

    #[test]
    fn language_id_for_typescript() {
        assert_eq!(
            blazecode_core::lsp::language_id_for_extension(".ts"),
            "typescript"
        );
    }
    ...
    fn language_id_fallback_plaintext() {
        assert_eq!(
            blazecode_core::lsp::language_id_for_extension(".zzz"),
            "plaintext"
        );
    }
