//! Comprehensive integration tests for blazecode-core.
//!
...
use blazecode_core::config::{
    self, discover_config_files, discover_blazecode_dirs, merge_info, normalize_config,
...
use blazecode_core::database::DatabaseServiceError;
use blazecode_core::encryption::hmac::{EncryptionError, EncryptionService, KEY_LENGTH};
use blazecode_core::error::{Error, PermissionError};
use blazecode_core::event::{
    EventCursor, EventDefinition, EventId, EventPayload, EventPubSub, EventRegistry,
    SyncConfig,
};
use blazecode_core::id::{ascending, create, descending, timestamp, Direction, IdError, IdPrefix};
use blazecode_core::permission::{
...
use blazecode_core::shell_parser::{FileOp, ShellParser};
use blazecode_core::truncate::{truncate_output, TruncateOptions, TruncateService, MAX_LINES, MAX_CHARS};
...
fn test_config_env_substitution() {
    std::env::set_var("BLAZECODE_TEST_INT_CFG_ENV", "resolved_value");
    let result = substitute_variables(
        "{env:BLAZECODE_TEST_INT_CFG_ENV}",
        Path::new("."),
        None,
    )
    .unwrap();
    assert_eq!(result, "resolved_value");
    std::env::remove_var("BLAZECODE_TEST_INT_CFG_ENV");
}
...
fn test_config_discover_files_walks_up() {
    let dir = Path::new("/");
    let files = discover_config_files("blazecode", dir, None).unwrap();
    assert!(files.is_empty() || files.iter().all(|f| f.file_name().unwrap().to_string_lossy().contains("blazecode")));
}

#[test]
fn test_config_discover_blazecode_dirs() {
    let dir = Path::new("/");
    let dirs = discover_blazecode_dirs(dir, None).unwrap();
    assert!(dirs.is_empty() || dirs.iter().all(|d| d.ends_with(".blazecode")));
}
...
fn test_well_known_providers_constants() {
    ...
    assert_eq!(well_known_providers::BLAZECODE, "blazecode");
...
fn test_message_info_user_serde() {
    let info = MessageInfo::User(UserInfo {
        ...
        agent: Some("build".into()),
        model: None,
        time: blazecode_core::session::MessageTime {
            created: 1000,
            completed: None,
        },
    });
...
fn test_usable_context_calculation() {
    let model = Model {
        ...
        name: "Test".into(),
        api: blazecode_core::provider::ApiInfo {
            id: "test".into(),
            url: String::new(),
            npm: "@ai-sdk/test".into(),
        },
        family: None,
        capabilities: blazecode_core::provider::Capabilities::default(),
        cost: blazecode_core::provider::Cost::default(),
        limit: blazecode_core::provider::TokenLimit {
            context: 200000,
            input: Some(180000),
            output: 4096,
        },
        status: blazecode_core::provider::ModelStatus::Active,
        options: HashMap::new(),
        headers: HashMap::new(),
