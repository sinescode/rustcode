...
//!
//! Ported from: `packages/blazecode/src/env/index.ts`
...
///
/// # Source
/// Ported from `packages/blazecode/src/env/index.ts` lines 6–13 (`Interface`).
#[derive(Debug)]
pub struct Env {
...
impl Env {
    ...
    ///
    /// # Source
    /// Ported from `packages/blazecode/src/env/index.ts` line 22:
    ...
    ///
    /// # Source
    /// Ported from `packages/blazecode/src/env/index.ts` line 9
    /// (`Effect.fn("Env.get")`).
    ...
    ///
    /// # Source
    /// Ported from `packages/blazecode/src/env/index.ts` line 10
    /// (`Effect.fn("Env.all")`).
    ...
    ///
    /// # Source
    /// Ported from `packages/blazecode/src/env/index.ts` lines 26–28
    /// (`Effect.fn("Env.set")`).
    ...
    ///
    /// # Source
    /// Ported from `packages/blazecode/src/env/index.ts` lines 30–33
...
///
/// ```rust
/// use blazecode_core::env::EnvStore;
///
...
impl EnvStore {
    ...
    ///
    /// # Source
    /// Ported from `packages/blazecode/src/effect/instance-state.ts`
...
mod tests {
    ...
    fn get_set_round_trip() {
        let env = Env::new();
        assert!(env.get("BLAZECODE_TEST_VAR").is_none());
        env.set("BLAZECODE_TEST_VAR", "hello");
        assert_eq!(env.get("BLAZECODE_TEST_VAR").unwrap(), "hello");
    }
