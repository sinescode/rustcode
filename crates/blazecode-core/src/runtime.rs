//! Shared runtime initialisation for blazecode.
//!
//! Ported from: packages/blazecode/src/index.ts — the Effect-TS Layer setup
//! and packages/blazecode/src/cli/cmd/tui.ts — bootstrap() function.
//!
...
//! `$XDG_DATA_HOME/blazecode/blazecode.db` (or the platform equivalent).
...
/// Return the default database path: `$XDG_DATA_HOME/blazecode/blazecode.db`.
///
/// Falls back to `~/.local/share/blazecode/blazecode.db` on Linux,
/// `~/Library/Application Support/blazecode/blazecode.db` on macOS,
/// `%APPDATA%/blazecode/blazecode.db` on Windows.
...
mod tests {
    ...
    fn test_default_db_path_is_absolute() {
        ...
        assert!(p.ends_with("blazecode.db"));
    }
