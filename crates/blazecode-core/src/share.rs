...
//!
//! Ported from:
//! - `packages/blazecode/src/share/share-next.ts` — ShareNext service (API layer)
//! - `packages/blazecode/src/share/session.ts` — SessionShare service (high-level)
...
///
/// # Source
/// `packages/blazecode/src/share/share-next.ts` lines 25–30.
...
///
/// # Source
/// `packages/blazecode/src/share/share-next.ts` lines 32–36.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ShareReq {
...
///
/// # Source
/// `packages/blazecode/src/share/share-next.ts` lines 38–43.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Share {
    pub id: String,
    pub url: String,
...
///
/// # Source
/// `packages/blazecode/src/share/share-next.ts` lines 51–71.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(tag = "type")]
pub enum ShareData {
...
///
/// # Source
/// `packages/blazecode/src/share/share-next.ts` lines 45–49.
...
///
/// # Source
/// `packages/blazecode/src/share/share-next.ts` lines 73–79.
...
///
/// # Source
/// Ported from `packages/blazecode/src/share/share-next.ts` lines 112–363.
...
impl ShareNextService {
    ...
    pub fn new(pool: Option<sqlx::SqlitePool>) -> Self {
        let disabled = std::env::var("BLAZECODE_DISABLE_SHARE")
            .map(|v| v == "true" || v == "1")
            .unwrap_or(false);
    ...
    ///
    /// # Source
    /// `packages/blazecode/src/share/share-next.ts` lines 85–92.
    ...
    ///
    /// # Source
    /// `packages/blazecode/src/share/share-next.ts` lines 235–245.
    ...
    ///
    /// # Source
    /// `packages/blazecode/src/share/share-next.ts` lines 247–272.
    ...
    ///
    /// # Source
    /// `packages/blazecode/src/share/share-next.ts` lines 274–299.
    async fn full_sync(
        &self,
        session_id: &str,
        data: Vec<ShareData>,
    ...
    ///
    /// # Source
    /// `packages/blazecode/src/share/share-next.ts` lines 124–147.
...
impl ShareNextInterface for ShareNextService {
    ...
    ///
    /// # Source
    /// `packages/blazecode/src/share/share-next.ts` lines 301–304.
    async fn init(&self) -> Result<(), crate::error::Error> {
        if self.disabled {
            return Ok(());
        }
    ...
    ///
    /// # Source
    /// `packages/blazecode/src/share/share-next.ts` lines 306–308.
    async fn url(&self) -> Result<String, crate::error::Error> {
        let req = self.request().await?;
        Ok(req.base_url)
    }
    ...
    ///
    /// # Source
    /// `packages/blazecode/src/share/share-next.ts` lines 206–222.
    async fn request(&self) -> Result<ShareReq, crate::error::Error> {
        let base_url = std::env::var("BLAZECODE_CONSOLE_URL")
    ...
    ///
    /// # Source
    /// `packages/blazecode/src/share/share-next.ts` lines 310–336.
    async fn create(
        &self,
        session_id: &str,
    ) -> Result<Share, crate::error::Error> {
    ...
    ///
    /// # Source
    /// `packages/blazecode/src/share/share-next.ts` lines 338–359.
    async fn remove(
        &self,
        session_id: &str,
    ) -> Result<(), crate::error::Error> {
...
///
/// # Source
/// `packages/blazecode/src/share/session.ts` lines 9–13.
#[async_trait::async_trait]
...
mod tests {
    ...
    fn test_share_req_serde() {
        ...
        let req = ShareReq {
            ...
            base_url: "https://blazecode.ai".into(),
        };
        let json = serde_json::to_string(&req).unwrap();
        let parsed: ShareReq = serde_json::from_str(&json).unwrap();
        assert_eq!(parsed.base_url, "https://blazecode.ai");
    ...
    fn test_share_row_serde() {
        let row = ShareRow {
            ...
            id: "shr_001".into(),
            secret: "s3cret".into(),
            url: "https://blazecode.ai/share/abc".into(),
        };
