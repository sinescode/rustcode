...
//!
//! Ported from: `packages/blazecode/src/storage/storage.ts`
...
///
/// Ported from `packages/blazecode/src/storage/storage.ts` lines 81–181.
...
/// `session_diff` files.
///
/// Ported from `packages/blazecode/src/storage/storage.ts` lines 182–210.
...
///
/// Ported from `packages/blazecode/src/storage/storage.ts` lines 222–243.
...
///
/// # Source
/// Ported from `packages/blazecode/src/storage/storage.ts` lines 213–321
...
///
/// Returns `<data_dir>/blazecode/blazecode.db`.
///
/// # Source
...
pub fn default_db_path() -> Result<PathBuf> {
    ...
    Ok(data_dir.join("blazecode").join("blazecode.db"))
}
...
/// the final state after all 35 blazecode migrations.
///
...
/// All 35 migrations from blazecode, in dependency order.
///
/// # Source
...
mod tests {
    ...
    fn test_storage_write_read() {
        let dir = std::env::temp_dir().join("blazecode-storage-test-wr");
    ...
    fn test_storage_update() {
        let dir = std::env::temp_dir().join("blazecode-storage-test-up");
        let _ = std::fs::remove_dir_all(&dir);
        let storage = Storage::new(dir.clone());
    ...
    fn test_storage_remove() {
        let dir = std::env::temp_dir().join("blazecode-storage-test-rm");
        let _ = std::fs::remove_dir_all(&dir);
        let storage = Storage::new(dir.clone());
    ...
    fn test_storage_list() {
        let dir = std::env::temp_dir().join("blazecode-storage-test-ls");
        let _ = std::fs::remove_dir_all(&dir);
        let storage = Storage::new(dir.clone());
    ...
    fn test_storage_read_missing() {
        let dir = std::env::temp_dir().join("blazecode-storage-test-miss");
        let _ = std::fs::remove_dir_all(&dir);
        let storage = Storage::new(dir.clone());
    ...
    async fn test_database_open_and_pragma() {
        let dir = std::env::temp_dir().join("blazecode-db-test-open");
        let _ = std::fs::remove_dir_all(&dir);
    ...
    async fn test_database_migrations() {
        let dir = std::env::temp_dir().join("blazecode-db-test-mig");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let db_path = dir.join("test.db");
    ...
    async fn test_database_session_crud() {
        let dir = std::env::temp_dir().join("blazecode-db-test-sess");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let db_path = dir.join("test.db");
    ...
    fn test_storage_read_or_default_missing() {
        let dir = std::env::temp_dir().join("blazecode-storage-test-rod-miss");
        let _ = std::fs::remove_dir_all(&dir);
        let storage = Storage::new(dir.clone());
    ...
    fn test_storage_read_or_default_exists() {
        let dir = std::env::temp_dir().join("blazecode-storage-test-rod-exists");
        let _ = std::fs::remove_dir_all(&dir);
        let storage = Storage::new(dir.clone());
    ...
    fn test_storage_update_or_insert_new() {
        let dir = std::env::temp_dir().join("blazecode-storage-test-uoi-new");
        let _ = std::fs::remove_dir_all(&dir);
        let storage = Storage::new(dir.clone());
    ...
    fn test_storage_update_or_insert_existing() {
        let dir = std::env::temp_dir().join("blazecode-storage-test-uoi-exists");
        let _ = std::fs::remove_dir_all(&dir);
        let storage = Storage::new(dir.clone());
    ...
    fn test_storage_read_with_schema() {
        let dir = std::env::temp_dir().join("blazecode-storage-schema-test");
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn test_storage_nested_keys() {
        let dir = std::env::temp_dir().join("blazecode-storage-test-nested");
        let _ = std::fs::remove_dir_all(&dir);
        let storage = Storage::new(dir.clone());
    ...
    fn test_storage_complex_json() {
        let dir = std::env::temp_dir().join("blazecode-storage-test-complex");
        let _ = std::fs::remove_dir_all(&dir);
        let storage = Storage::new(dir.clone());
    ...
    async fn test_database_migration_failure_rollback() {
        let dir = std::env::temp_dir().join("blazecode-db-test-fail-mig");
        let _ = std::fs::remove_dir_all(&dir);
    ...
    async fn test_database_query_row() {
        let dir = std::env::temp_dir().join("blazecode-db-test-query-row");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).expect("create temp dir");
        let db_path = dir.join("test.db");
    ...
    fn test_default_db_path_returns_path() {
        ...
        assert!(
            path_str.ends_with("blazecode/blazecode.db"),
            "default_db_path should end with 'blazecode/blazecode.db', got: {path_str}"
        );
    }
    ...
    async fn test_database_open_twice_reuses_tables() {
        let dir = std::env::temp_dir().join("blazecode-db-test-reopen");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).expect("create temp dir");
        let db_path = dir.join("test.db");
