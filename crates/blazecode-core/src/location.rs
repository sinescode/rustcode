...
mod tests {
    ...
    fn test_resolve_with_fs_symlink_escape() {
        let tmp = std::env::temp_dir().join("blazecode_test_symlink_escape");
    ...
    fn test_resolve_with_fs_symlink_stays_inside() {
        let tmp = std::env::temp_dir().join("blazecode_test_symlink_inside");
        let _ = std::fs::remove_dir_all(&tmp);
    ...
    fn test_resolve_with_fs_non_directory_ancestor() {
        let tmp = std::env::temp_dir().join("blazecode_test_non_dir_ancestor");
        let _ = std::fs::remove_dir_all(&tmp);
    ...
    fn test_resolve_with_fs_missing_path_ok() {
        let tmp = std::env::temp_dir().join("blazecode_test_missing_path");
        let _ = std::fs::remove_dir_all(&tmp);
        std::fs::create_dir_all(&tmp).expect("create dirs");
    ...
    fn test_resolve_delegates_to_resolve_with_fs() {
        let tmp = std::env::temp_dir().join("blazecode_test_resolve_delegates");
        let _ = std::fs::remove_dir_all(&tmp);
