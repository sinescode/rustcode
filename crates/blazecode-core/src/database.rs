...
///
/// # Source
/// Ported from `packages/core/src/global.ts` line 11 (`const app = "blazecode"`).
pub const APP_NAME: &str = "blazecode";

/// Database file name for the default channel.
///
/// # Source
/// Ported from `packages/core/src/database/database.ts` line 53
/// (`return join(Global.Path.data, "blazecode.db")`).
pub const DEFAULT_DB_FILE: &str = "blazecode.db";
...
///
/// Priority:
/// 1. If `BLAZECODE_DB` is set and is `:memory:` or an absolute path, use it.
/// 2. If `BLAZECODE_DB` is set and is relative, join with `data` dir.
/// 3. If channel is `latest`, `beta`, or `prod`, use `blazecode.db`.
/// 4. Otherwise, use `blazecode-{sanitized_channel}.db`.
pub fn database_path(
    data_dir: &str,
    blazecode_db: Option<&str>,
    channel: Option<&str>,
    disable_channel_db: bool,
) -> String {
    if let Some(db_path) = blazecode_db {
        if db_path == ":memory:" {
    ...
    if is_default_channel {
        ...
        format!("{data_dir}/blazecode-{sanitized}.db")
    }
}
...
pub struct GlobalPaths {
    /// User home directory
    pub home: String,
    /// XDG data directory: `$XDG_DATA_HOME/blazecode`
    pub data: String,
    /// Binary cache directory: `$XDG_CACHE_HOME/blazecode/bin`
    pub bin: String,
    /// Log directory: `$XDG_DATA_HOME/blazecode/log`
    pub log: String,
    /// Repos directory: `$XDG_DATA_HOME/blazecode/repos`
    pub repos: String,
    /// XDG cache directory: `$XDG_CACHE_HOME/blazecode`
    pub cache: String,
    /// XDG config directory: `$XDG_CONFIG_HOME/blazecode`
    pub config: String,
    /// XDG state directory: `$XDG_STATE_HOME/blazecode`
    pub state: String,
    /// Temp directory: `$TMPDIR/blazecode`
    pub tmp: String,
}
...
impl GlobalPaths {
    ...
    pub fn database_path(&self, blazecode_db: Option<&str>, channel: Option<&str>) -> String {
        database_path(&self.data, blazecode_db, channel, false)
    }
}
...
mod tests {
    ...
    fn database_path_default_channel() {
        let path = database_path("/data/blazecode", None, Some("latest"), false);
        assert_eq!(path, "/data/blazecode/blazecode.db");
    }

    #[test]
    fn database_path_beta_channel() {
        let path = database_path("/data/blazecode", None, Some("beta"), false);
        assert_eq!(path, "/data/blazecode/blazecode.db");
    }

    #[test]
    fn database_path_custom_channel() {
        let path = database_path("/data/blazecode", None, Some("nightly"), false);
        assert_eq!(path, "/data/blazecode/blazecode-nightly.db");
    }

    #[test]
    fn database_path_with_special_chars() {
        let path = database_path("/data/blazecode", None, Some("dev/feature!@"), false);
        assert_eq!(path, "/data/blazecode/blazecode-dev-feature--.db");
    }

    #[test]
    fn database_path_memory() {
        let path = database_path("/data/blazecode", Some(":memory:"), None, false);
        assert_eq!(path, ":memory:");
    }

    #[test]
    fn database_path_absolute_override() {
        let path = database_path("/data/blazecode", Some("/tmp/mine.db"), None, false);
        assert_eq!(path, "/tmp/mine.db");
    }

    #[test]
    fn database_path_relative_override() {
        let path = database_path("/data/blazecode", Some("custom.db"), None, false);
        assert_eq!(path, "/data/blazecode/custom.db");
    }
    ...
    fn global_paths_default_has_all_fields() {
        ...
        assert!(paths.data.ends_with("blazecode"));
        assert!(paths.cache.ends_with("blazecode"));
        assert!(paths.config.ends_with("blazecode"));
        assert!(paths.state.ends_with("blazecode"));
