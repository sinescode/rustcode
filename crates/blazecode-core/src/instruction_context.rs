...
//! - `BLAZECODE_DISABLE_PROJECT_CONFIG` env var support
...
pub const BLAZECODE_DISABLE_PROJECT_CONFIG: &str = "BLAZECODE_DISABLE_PROJECT_CONFIG";
...
pub struct InstructionDiscovery {
    /// The global config directory path (e.g., `~/.config/blazecode`).
    config_dir: String,
}
...
impl InstructionDiscovery {
    ...
    /// `BLAZECODE_DISABLE_PROJECT_CONFIG` env var — when set, project
    ...
    /// Check if `BLAZECODE_DISABLE_PROJECT_CONFIG` is truthy (`"1"` or `"true"`).
    ///
    /// # Source
    /// Ported from `packages/core/src/flag/flag.ts` lines 54–55.
    pub fn is_project_config_disabled(&self) -> bool {
        is_truthy_env(BLAZECODE_DISABLE_PROJECT_CONFIG)
    }
...
mod tests {
    ...
    fn test_discover_files_walks_upward() {
        let tmp = std::env::temp_dir().join("blazecode_test_discovery");
    ...
    fn test_discover_files_finds_multiple_levels() {
        let tmp = std::env::temp_dir().join("blazecode_test_discovery_multi");
        let _ = fs::remove_dir_all(&tmp);
        let project = tmp.join("project");
        let nested = project.join("packages").join("core");
        fs::create_dir_all(&nested).expect("create dirs");
    ...
    fn test_discover_files_stops_at_project_root() {
        let tmp = std::env::temp_dir().join("blazecode_test_discovery_stop");
        let _ = fs::remove_dir_all(&tmp);
        let project = tmp.join("project");
    ...
    fn test_discover_files_no_project_root_walks_to_fs_root() {
        let tmp = std::env::temp_dir().join("blazecode_test_discovery_noroot");
        let _ = fs::remove_dir_all(&tmp);
    ...
    fn test_discover_global_finds_config_agents_md() {
        let tmp = std::env::temp_dir().join("blazecode_test_discovery_global");
        let _ = fs::remove_dir_all(&tmp);
        fs::create_dir_all(&tmp).expect("create dirs");
    ...
    fn test_discover_global_missing_returns_empty() {
        let tmp = std::env::temp_dir().join("blazecode_test_discovery_global_missing");
        let _ = fs::remove_dir_all(&tmp);
        fs::create_dir_all(&tmp).expect("create dirs");
    ...
    fn test_load_all_respects_disable_project_config() {
        let tmp = std::env::temp_dir().join("blazecode_test_load_all_disable");
        let _ = fs::remove_dir_all(&tmp);
    ...
    #[test]
    fn test_source_key() {
        let tmp = std::env::temp_dir().join("blazecode_test_source_key");
        let _ = fs::remove_dir_all(&tmp);
        fs::create_dir_all(&tmp).expect("create dirs");
    ...
    fn test_source_load_returns_json_array() {
        let tmp = std::env::temp_dir().join("blazecode_test_source_load");
        let _ = fs::remove_dir_all(&tmp);
        fs::create_dir_all(&tmp).expect("create dirs");
    ...
    fn test_source_baseline_renders_correctly() {
        let tmp = std::env::temp_dir().join("blazecode_test_source_baseline");
        let _ = fs::remove_dir_all(&tmp);
        fs::create_dir_all(&tmp).expect("create dirs");
    ...
    fn test_source_load_empty_when_no_files() {
        let tmp = std::env::temp_dir().join("blazecode_test_source_empty");
        let _ = fs::remove_dir_all(&tmp);
        fs::create_dir_all(&tmp).expect("create dirs");
    ...
    fn test_is_truthy_env_one() {
        std::env::set_var("BLAZECODE_TEST_TRUTHY_1", "1");
        assert!(is_truthy_env("BLAZECODE_TEST_TRUTHY_1"));
        std::env::remove_var("BLAZECODE_TEST_TRUTHY_1");
    }

    #[test]
    fn test_is_truthy_env_true() {
        std::env::set_var("BLAZECODE_TEST_TRUTHY_TRUE", "true");
        assert!(is_truthy_env("BLAZECODE_TEST_TRUTHY_TRUE"));
        std::env::remove_var("BLAZECODE_TEST_TRUTHY_TRUE");
    }

    #[test]
    fn test_is_truthy_env_case_insensitive() {
        std::env::set_var("BLAZECODE_TEST_TRUTHY_TRUE_UPPER", "TRUE");
        assert!(is_truthy_env("BLAZECODE_TEST_TRUTHY_TRUE_UPPER"));
        std::env::remove_var("BLAZECODE_TEST_TRUTHY_TRUE_UPPER");
    }

    #[test]
    fn test_is_truthy_env_false() {
        std::env::set_var("BLAZECODE_TEST_TRUTHY_FALSE", "false");
        assert!(!is_truthy_env("BLAZECODE_TEST_TRUTHY_FALSE"));
        std::env::remove_var("BLAZECODE_TEST_TRUTHY_FALSE");
    }

    #[test]
    fn test_is_truthy_env_unset() {
        std::env::remove_var("BLAZECODE_TEST_TRUTHY_UNSET");
        assert!(!is_truthy_env("BLAZECODE_TEST_TRUTHY_UNSET"));
    }
    ...
    fn test_full_source_lifecycle() {
        let tmp = std::env::temp_dir().join("blazecode_test_full_lifecycle");
        let _ = fs::remove_dir_all(&tmp);
        let global_config = tmp.join("global");
        let project = tmp.join("project");
        let nested = project.join("src");
