//! Reminders system — load `.blazecode/reminders/*.md` files and inject
//! their content into the system prompt as synthetic parts.
//!
//! Ported from: `packages/blazecode/src/session/reminders.ts` (lines 1–92)
...
///
/// # Source
/// Ported from `packages/blazecode/src/session/reminders.ts`.
#[derive(Debug, Clone)]
...
///
/// Scans `.blazecode/reminders/` and `.claude/reminders/` for `.md` files,
/// concatenates their content, and returns it for injection into the
/// system prompt.
///
/// # Source
/// Ported from `packages/blazecode/src/session/reminders.ts` — the `apply` function
...
impl SessionReminders {
    ...
    fn reminder_dirs(cwd: &std::path::Path) -> Vec<PathBuf> {
        vec![
            cwd.join(".blazecode").join("reminders"),
    ...
    ///
    /// # Source
    /// Ported from `packages/blazecode/src/session/reminders.ts` — the file
    /// scanning pattern.
    ...
    ///
    /// # Source
    /// Ported from `packages/blazecode/src/session/reminders.ts` `apply` function.
...
mod tests {
    ...
    fn setup_test_reminders() -> tempfile::TempDir {
        ...
        let reminders_dir = dir.path().join(".blazecode").join("reminders");
