//! Built-in tool implementations — 21 tools covering the full BlazeCode tool surface.
//!
...
//! - `packages/blazecode/src/tool/shell.ts` (657 lines)
//! - `packages/core/src/tool/read.ts` (105 lines)
//! - `packages/blazecode/src/tool/read.ts` (386 lines)
//! - `packages/core/src/tool/write.ts` (93 lines)
//! - `packages/blazecode/src/tool/write.ts` (104 lines)
//! - `packages/core/src/tool/edit.ts` (199 lines)
//! - `packages/blazecode/src/tool/edit.ts` (737 lines)
//! - `packages/core/src/tool/glob.ts` (98 lines)
//! - `packages/blazecode/src/tool/glob.ts` (76 lines)
//! - `packages/core/src/tool/grep.ts` (130 lines)
//! - `packages/blazecode/src/tool/grep.ts` (112 lines)
//! - `packages/core/src/tool/webfetch.ts` (217 lines)
//! - `packages/blazecode/src/tool/webfetch.ts` (192 lines)
//! - `packages/core/src/tool/websearch.ts` (246 lines)
//! - `packages/blazecode/src/tool/websearch.ts` (143 lines)
//! - `packages/core/src/tool/apply-patch.ts` (177 lines)
//! - `packages/blazecode/src/tool/apply_patch.ts` (313 lines)
//! - `packages/blazecode/src/tool/task.ts` (346 lines)
//! - `packages/blazecode/src/tool/question.ts` (44 lines)
//! - `packages/core/src/tool/question.ts` (86 lines)
//! - `packages/blazecode/src/tool/skill.ts` (71 lines)
//! - `packages/core/src/tool/skill.ts` (105 lines)
//! - `packages/blazecode/src/tool/todo.ts` (57 lines)
//! - `packages/core/src/tool/todowrite.ts` (54 lines)
//! - `packages/blazecode/src/tool/plan.ts` (79 lines)
//! - `packages/blazecode/src/tool/lsp.ts` (200 lines)
//! - `packages/blazecode/src/tool/invalid.ts` (19 lines)
//!
...
// Ported from: packages/blazecode/src/tool/edit.ts (lines 217–644)
...
/// Ported from: packages/blazecode/src/tool/edit.ts `levenshtein()`
...
/// Ported from: packages/blazecode/src/tool/edit.ts `SimpleReplacer`
...
/// Ported from: packages/blazecode/src/tool/edit.ts `LineTrimmedReplacer`
...
/// Ported from: packages/blazecode/src/tool/edit.ts `BlockAnchorReplacer`
...
/// Ported from: packages/blazecode/src/tool/edit.ts `WhitespaceNormalizedReplacer`
...
/// Ported from: packages/blazecode/src/tool/edit.ts `IndentationFlexibleReplacer`
...
/// Ported from: packages/blazecode/src/tool/edit.ts `EscapeNormalizedReplacer`
...
/// Ported from: packages/blazecode/src/tool/edit.ts `MultiOccurrenceReplacer`
...
/// Ported from: packages/blazecode/src/tool/edit.ts `TrimmedBoundaryReplacer`
...
/// Ported from: packages/blazecode/src/tool/edit.ts `ContextAwareReplacer`
...
/// Ported from: packages/blazecode/src/tool/edit.ts `isDisproportionateMatch()`
...
/// Ported from: packages/blazecode/src/tool/edit.ts `replace()`
pub fn edit_replace(
    content: &str,
    old_string: &str,
    new_string: &str,
...
/// Ported from: packages/blazecode/src/tool/edit.ts `trimDiff()`
...
///
/// # Source
/// Ported from `packages/core/src/tool/bash.ts` and `packages/blazecode/src/tool/shell.ts`.
#[derive(Debug, Clone)]
pub struct BashTool;
...
///
/// # Source
/// Ported from `packages/core/src/tool/read.ts` and `packages/blazecode/src/tool/read.ts`.
#[derive(Debug, Clone)]
pub struct ReadTool;
...
///
/// # Source
/// Ported from `packages/core/src/tool/write.ts` and `packages/blazecode/src/tool/write.ts`.
#[derive(Debug, Clone)]
...
///
/// # Source
/// Ported from `packages/core/src/tool/edit.ts` and `packages/blazecode/src/tool/edit.ts`.
#[derive(Debug, Clone)]
pub struct EditTool;
...
///
/// # Source
/// Ported from `packages/core/src/tool/glob.ts` and `packages/blazecode/src/tool/glob.ts`.
#[derive(Debug, Clone)]
pub struct GlobTool;
...
///
/// # Source
/// Ported from `packages/core/src/tool/grep.ts` and `packages/blazecode/src/tool/grep.ts`.
#[derive(Debug, Clone)]
pub struct GrepTool;
...
///
/// # Source
/// Ported from `packages/core/src/tool/webfetch.ts` and `packages/blazecode/src/tool/webfetch.ts`.
#[derive(Debug, Clone)]
...
///
/// # Configuration
/// - `BLAZECODE_WEBSEARCH_PROVIDER` — set to "exa" or "parallel" (optional)
/// - `EXA_API_KEY` — Exa API key
/// - `PARALLEL_API_KEY` — Parallel API key
///
/// # Source
/// Ported from `packages/core/src/tool/websearch.ts` and `packages/blazecode/src/tool/websearch.ts`.
#[derive(Debug, Clone)]
...
impl SearchConfig {
    ...
    fn from_env() -> Self {
        let provider_override = std::env::var("BLAZECODE_WEBSEARCH_PROVIDER").ok();
        let exa_key = std::env::var("EXA_API_KEY").ok();
        let parallel_key = std::env::var("PARALLEL_API_KEY").ok();

        // Check BLAZECODE_WEBSEARCH_PROVIDER override first
    ...
    fn headers(&self) -> Vec<(String, String)> {
        ...
        if self.provider == PROVIDER_PARALLEL {
            ...
            headers.push(("User-Agent".into(), "blazecode".into()));
        }
        headers
    }
}
...
impl ApplyPatchTool {
    ...
    /// 1. TS-style blazecode markers (`*** Begin Patch` / `*** Add File:` / etc.)
    /// 2. Legacy blazecode format (`*** Begin Patch <path> ***` / body / `*** End Patch`)
    ...
    fn parse_patch_operation(patch_text: &str) -> Result<PatchOp> {
        ...
        if let (Some(begin), Some(end)) = (begin_idx, end_idx) {
            if begin < end {
                return Self::parse_blazecode_marker_patch(&lines[begin + 1..end]);
            }
        }
    ...
    /// Parse TS-style blazecode marker patch content.
    ///
    ...
    /// @@ ... @@
    /// ```
    fn parse_blazecode_marker_patch(lines: &[&str]) -> Result<PatchOp> {
        if lines.is_empty() {
...
/// Helper: construct a PatchOp::Move from the blazecode format.
fn parse_move_patch(
    source_path: String,
    target_path: String,
    _tail_lines: &[&str],
    ...
    fn parameters_schema(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                ...
                "patch": {
                    "type": "string",
                    "description": "The patch content — unified diff or blazecode marker format"
                },
                "patchText": {
                    "type": "string",
...
///
/// # Source
/// Ported from `packages/blazecode/src/tool/task.ts` (346 lines).
...
///
/// # Source
/// Ported from `packages/blazecode/src/tool/task.ts` lines 64–79 (`renderOutput`).
...
///
/// # Source
/// Ported from `packages/blazecode/src/tool/task.ts` lines 129–141.
...
/// `packages/blazecode/src/tool/question.ts`.
#[derive(Clone)]
...
///
/// Loads the skill markdown file from `.blazecode/skills/{skill}.md`,
/// parses YAML frontmatter, and returns the skill instructions and metadata.
///
/// # Source
/// Ported from `packages/core/src/tool/skill.ts` and
/// `packages/blazecode/src/tool/skill.ts`.
#[derive(Debug, Clone)]
pub struct SkillTool;

impl SkillTool {
    /// Load a skill from `.blazecode/skills/{name}.md` and parse its frontmatter.
...
impl Tool for SkillTool {
    ...
    fn description(&self) -> &str {
        "Invoke a named skill to load its instructions into the current conversation.\
         Skills are markdown files in .blazecode/skills/ with YAML frontmatter.\
    ...
    fn parameters_schema(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                "skill": {
                    "type": "string",
                    "description": "The skill name (must match a file in .blazecode/skills/ without the .md extension)"
                },
                "args": {
                    "type": "string",
        ...
        match Self::load_skill(skill_name) {
            ...
            Err(_) => {
                ...
                let output = format!(
                    "<skill_content name=\"{}\">\n\
                     # Skill: {}\n\n\
                     Skill file not found at .blazecode/skills/{}.md.\n\
...
/// `packages/blazecode/src/tool/todo.ts`.
#[derive(Debug, Clone)]
...
///
/// # Source
/// Ported from `packages/blazecode/src/tool/plan.ts` (79 lines).
#[derive(Debug, Clone)]
...
///
/// # Source
/// Ported from `packages/blazecode/src/tool/plan.ts` — exit plan mode variant.
#[derive(Debug, Clone)]
...
///
/// Snapshots are stored as JSON files under ~/.blazecode/stashes/.
/// Supports save, restore, list, and drop operations.
///
/// # Source
/// Ported from `packages/blazecode/src/tool/` — stash pattern.
#[derive(Debug, Clone)]
...
///
/// # Source
/// Ported from `packages/blazecode/src/tool/` — notebook editing pattern.
#[derive(Debug, Clone)]
...
///
/// # Source
/// Ported from `packages/blazecode/src/tool/` — task output pattern.
#[derive(Debug, Clone)]
...
///
/// # Source
/// Ported from `packages/blazecode/src/tool/lsp.ts`.
///
...
/// [`blazecode_core::lsp::set_global_lsp_bridge`]. The `blazecode-lsp` crate
    ...
    async fn execute(&self, args: serde_json::Value, _ctx: &ToolContext) -> Result<ExecuteResult> {
        ...
        let title = if file_path.is_empty() {
            ...
            format!("{operation} {file_path}")
                ...
                // Check if an LSP bridge is available via the global in blazecode-core
        let has_bridge = crate::lsp::has_lsp_bridge();
        if !has_bridge {
            return Ok(ExecuteResult {
                title,
                output: format!(
                    "LSP tool invoked for operation '{operation}' on '{file_path}'.                      However, no LSP bridge is registered.                      To use LSP features, initialize the LSP subsystem                      by calling `blazecode_core::lsp::set_global_lsp_bridge()`."
                ),
                truncated: false,
                output_path: None,
                attachments: None,
        ...
        let result = match operation {
            ...
            "documentSymbol" => {
                serde_json::json!({
                    "operation": operation,
                    "filePath": file_path,
                    "note": "documentSymbol requires direct LSP client access via blazecode-lsp.                              The core LspBridge only supports workspaceSymbol for now."
                })
            }
            _ => {
                ...
                serde_json::json!({
                    ...
                    "character": character,
                    "query": query,
                    "note": "Position-based LSP operations require direct LSP client access via blazecode-lsp.                              The core LspBridge only supports workspaceSymbol for now.",
...
///
/// # Source
/// Ported from `packages/blazecode/src/tool/invalid.ts`.
#[derive(Debug, Clone)]
...
mod tests {
    ...
    async fn test_read_existing_file() {
        let tool = ReadTool;
        let ctx = test_ctx();
        let tmpfile = std::env::temp_dir().join("blazecode_read_test.txt");
    ...
    async fn test_read_with_offset_limit() {
        let tool = ReadTool;
        let ctx = test_ctx();
        let tmpfile = std::env::temp_dir().join("blazecode_read_offset.txt");
    ...
    async fn test_write_new_file() {
        let tool = WriteTool;
        let ctx = test_ctx();
        let tmpfile = std::env::temp_dir().join("blazecode_write_new.txt");
        let _ = std::fs::remove_file(&tmpfile);
    ...
    async fn test_write_overwrite() {
        let tool = WriteTool;
        let ctx = test_ctx();
        let tmpfile = std::env::temp_dir().join("blazecode_write_overwrite.txt");
    ...
    async fn test_write_creates_parent_dirs() {
        let tool = WriteTool;
        let ctx = test_ctx();
        let parent = std::env::temp_dir().join("blazecode_test_nested");
    ...
    async fn test_edit_single_replace() {
        let tool = EditTool;
        let ctx = test_ctx();
        let tmpfile = std::env::temp_dir().join("blazecode_edit_test.txt");
    ...
    async fn test_edit_replace_all() {
        let tool = EditTool;
        let ctx = test_ctx();
        let tmpfile = std::env::temp_dir().join("blazecode_edit_all.txt");
    ...
    async fn test_edit_not_found() {
        let tool = EditTool;
        let ctx = test_ctx();
        let tmpfile = std::env::temp_dir().join("blazecode_edit_nf.txt");
    ...
    async fn test_edit_multiple_without_replace_all() {
        let tool = EditTool;
        let ctx = test_ctx();
        let tmpfile = std::env::temp_dir().join("blazecode_edit_multi.txt");
    ...
    async fn test_grep_finds_matches() {
        let tool = GrepTool;
        let ctx = test_ctx();
        let tmpdir = std::env::temp_dir().join("blazecode_grep_test");
    ...
    async fn test_grep_no_matches() {
        let tool = GrepTool;
        let ctx = test_ctx();
        let tmpdir = std::env::temp_dir().join("blazecode_grep_nomatch");
        std::fs::create_dir_all(&tmpdir).unwrap();
    ...
    async fn test_grep_with_include_filter() {
        let tool = GrepTool;
        let ctx = test_ctx();
        let tmpdir = std::env::temp_dir().join("blazecode_grep_include");
        std::fs::create_dir_all(&tmpdir).unwrap();
    ...
    async fn test_apply_patch_empty_patch() {
        let tool = ApplyPatchTool;
        let ctx = test_ctx();
        let tmpfile = std::env::temp_dir().join("blazecode_ap_empty.txt");
    ...
    async fn test_apply_patch_simple_unified_diff() {
        let tool = ApplyPatchTool;
        let ctx = test_ctx();
        let tmpfile = std::env::temp_dir().join("blazecode_ap_simple.txt");
        std::fs::write(&tmpfile, "line1\nline2\nline3\n").unwrap();
    ...
    async fn test_apply_patch_add_lines() {
        let tool = ApplyPatchTool;
        let ctx = test_ctx();
        let tmpfile = std::env::temp_dir().join("blazecode_ap_add.txt");
        std::fs::write(&tmpfile, "line1\nline2\n").unwrap();
    ...
    async fn test_apply_patch_remove_lines() {
        let tool = ApplyPatchTool;
        let ctx = test_ctx();
        let tmpfile = std::env::temp_dir().join("blazecode_ap_del.txt");
    ...
    async fn test_apply_patch_offset_adjustment() {
        let tool = ApplyPatchTool;
        let ctx = test_ctx();
        let tmpfile = std::env::temp_dir().join("blazecode_ap_offset.txt");
    ...
    async fn test_apply_patch_bad_hunk_context() {
        let tool = ApplyPatchTool;
        let ctx = test_ctx();
        let tmpfile = std::env::temp_dir().join("blazecode_ap_badctx.txt");
