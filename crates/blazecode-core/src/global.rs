...
impl GlobalPaths {
    ...
    /// Resolve the config directory, respecting the `BLAZECODE_CONFIG_DIR` env var.
    ///
    /// # Source
    /// Ported from `packages/core/src/global.ts` line 63:
    /// `config: Flag.BLAZECODE_CONFIG_DIR ?? Path.config`
    pub fn resolve_config_dir(&self) -> String {
        std::env::var("BLAZECODE_CONFIG_DIR").unwrap_or_else(|_| self.config.clone())
    }
    ...
    /// `get home() { return process.env.BLAZECODE_TEST_HOME ?? os.homedir() }`
    pub fn resolve_home() -> String {
        std::env::var("BLAZECODE_TEST_HOME").unwrap_or_else(|_| {
            dirs::home_dir()
                .map(|p| p.to_string_lossy().to_string())
                .unwrap_or_default()
        })
...
mod tests {
    ...
    fn global_paths_resolve_config_dir_env_override() {
        ...
        std::env::remove_var("BLAZECODE_CONFIG_DIR");
