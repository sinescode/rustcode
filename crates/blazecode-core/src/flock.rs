...
pub async fn acquire(key: &str, options: &FlockOptions) -> Result<FlockLease, String> {
    let lock_root = options
        .dir
        .clone()
        .unwrap_or_else(|| PathBuf::from("/tmp/blazecode/locks"));
...
mod tests {
    ...
    async fn test_acquire_and_release() {
        let dir = std::env::temp_dir().join("blazecode-flock-test-simple");
    ...
    async fn test_with_lock() {
        let dir = std::env::temp_dir().join("blazecode-flock-test-with");
        let _ = std::fs::remove_dir_all(&dir);
    ...
    async fn test_acquire_timeout() {
        let dir = std::env::temp_dir().join("blazecode-flock-test-timeout");
        let _ = std::fs::remove_dir_all(&dir);
    ...
    async fn test_token_verified_release() {
        let dir = std::env::temp_dir().join("blazecode-flock-test-token");
        let _ = std::fs::remove_dir_all(&dir);
    ...
    async fn test_stale_detection() {
        let dir = std::env::temp_dir().join("blazecode-flock-test-stale");
        let _ = std::fs::remove_dir_all(&dir);
