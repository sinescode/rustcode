...
/// (static `.blazecode`, `.anthropic`, etc. on `ProviderV2.ID`).
pub mod well_known_providers {
    /// The built-in blazecode provider
    pub const BLAZECODE: &str = "blazecode";
...
/// (`const source = Flag.BLAZECODE_MODELS_URL || "https://models.dev"`).
