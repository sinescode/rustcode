//! Error types for blazecode-core.
//!
//! Ported from: cross-cutting error handling across `BlazeCode`.
...
/// Top-level error for the blazecode-core crate.
///
/// # Source
/// Aggregated from all `Schema.TaggedErrorClass` definitions across
/// `packages/blazecode/src/` and `packages/core/src/`.
#[derive(Debug, Error)]
pub enum Error {
    // -- I/O & filesystem --
    ...
    ///
    /// Ported from `packages/blazecode/src/provider/provider.ts` `InitError`.
    ...
    ///
    /// Ported from `packages/blazecode/src/provider/provider.ts` `NoProvidersError`.
    #[error("no providers available")]
    NoProviders,

    /// No models available for the given provider.
    ///
    /// Ported from `packages/blazecode/src/provider/provider.ts` `NoModelsError`.
    #[error("no models available for provider `{provider_id}`")]
    NoModels { provider_id: String },

    /// Model not found.
    ///
    /// Ported from `packages/blazecode/src/provider/provider.ts` `ModelNotFoundError`
    ...
    ///
    /// Ported from `packages/blazecode/src/provider/error.ts` `HeaderTimeoutError`.
    ...
    ///
    /// Ported from `packages/blazecode/src/provider/error.ts` `ResponseStreamError`.
    ...
    ///
    /// Ported from `packages/blazecode/src/tool/tool.ts` `InvalidArgumentsError`.
    ...
    ///
    /// Ported from `packages/blazecode/src/session/session.ts` `BusyError`.
    ...
    /// Worktree error.
    ///
    /// Ported from `packages/blazecode/src/worktree/index.ts`.
    #[error("{0}")]
    ...
    ///
    /// Ported from `packages/core/src/image.ts` and `packages/blazecode/src/image/image.ts`.
    #[error("{0}")]
    ...
    /// Skill error.
    ///
    /// Ported from `packages/blazecode/src/skill/index.ts`.
    #[error("{0}")]
    ...
    ///
    /// Ported from `packages/blazecode/src/lsp/client.ts` `InitializeError`.
    ...
    /// and `packages/blazecode/src/util/process.ts` `RunFailedError`.
    ...
    ///
    /// Ported from `packages/blazecode/src/auth/index.ts` `AuthError`.
    #[error("auth error: {0}")]
    Auth(String),
    ...
    /// Project not found.
    ///
    /// Ported from `packages/blazecode/src/project/project.ts` `NotFoundError`.
    ...
    ///
    /// Ported from `packages/blazecode/src/storage/storage.ts` `NotFoundError`.
...
/// `packages/blazecode/src/image/image.ts`.
#[derive(Debug, Error)]
pub enum ImageError {
...
///
/// # Source
/// Ported from `packages/blazecode/src/server/routes/instance/httpapi/errors.ts`.
#[derive(Debug, Error)]
pub enum ApiError {
    /// 400 Bad Request.
...
/// Result type alias for blazecode-core.
