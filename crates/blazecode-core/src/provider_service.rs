...
//!
//! # Source
//! Ported from `packages/blazecode/src/provider/provider.ts` lines 1300–1550.
...
///
/// # Source
/// Ported from `packages/blazecode/src/provider/provider.ts` `Service.init()`.
...
fn create_provider_from_config(
    ...
    let base_url = cfg
        ...
        .and_then(|o| o.base_url.clone())
        .or_else(|| {
            // Try to find base_url in extra options (BlazeCode uses baseURL)
            cfg.options
                .as_ref()
                .and_then(|o| {
                    o.extra.get("baseUrl")
