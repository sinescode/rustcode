...
//! frontmatter. They live in `.blazecode/skill/`, `.blazecode/skills/`,
//! `.claude/skills/`, `.agents/skills/`, and configurable paths.
//!
//! # Source
//! Ported from:
//! - `packages/blazecode/src/skill/index.ts` — Skill service, discovery, loading, errors
//! - `packages/blazecode/src/skill/discovery.ts` — Remote skill discovery (HTTP pull)
...
//! - `packages/core/src/plugin/skill.ts` — Embedded `customize-blazecode` skill
//!
...
pub struct SkillDiscoveryConfig {
    ...
    ///
    /// # Source
    /// Ported from `packages/blazecode/src/skill/index.ts` `disableClaudeCodeSkills`.
...
///
/// Ported from `packages/blazecode/src/skill/index.ts` `isSkillFrontmatter()`.
...
///
/// Ported from `packages/blazecode/src/skill/index.ts` `Info`.
#[derive(Debug, Clone)]
pub struct Skill {
...
///
/// Ported from the logic in `packages/blazecode/src/skill/index.ts` `add()` function,
...
const SKILL_PATTERNS: &[&str] = &[
    ".blazecode/skill/**/SKILL.md",
    ".blazecode/skills/**/SKILL.md",
];

/// External skill directory names (relative to home or project root).
///
/// Ported from `packages/blazecode/src/skill/index.ts` constants.
...
///
/// Ported from `packages/blazecode/src/skill/index.ts` `scan()` function.
...
///
/// Searches in order:
/// 1. `.blazecode/skill/` and `.blazecode/skills/` under `worktree`
...
///
/// # Source
/// Ported from `packages/blazecode/src/skill/index.ts` `discoverSkills()`.
pub fn discover_skill_files(
    ...
    // 1. Scan .blazecode/skill/ and .blazecode/skills/ under worktree
    ...
    // Also scan .blazecode/ under directory if different from worktree
...
///
/// Ported from `packages/blazecode/src/skill/index.ts` `State.skills` record.
...
impl SkillRegistry {
    ...
    ///
    /// # Source
    /// Ported from `packages/blazecode/src/skill/index.ts` `available(agent?)`.
    pub fn available(
        &self,
...
///
/// # Source
/// Ported from `packages/blazecode/src/skill/index.ts` `loadSkills()`.
pub async fn discover_and_load(
    ...
    // Register the built-in customize-blazecode skill first so user skills can override it
    registry.register(Skill::builtin(
        "customize-blazecode",
        "Use ONLY when the user is editing or creating blazecode's own configuration: blazecode.json, blazecode.jsonc, files under .blazecode/, or files under ~/.config/blazecode/. Also use when creating or fixing blazecode agents, subagents, skills, plugins, MCP servers, or permission rules. Do not use for the user's own application code, or for any project that is not configuring blazecode itself.",
        include_str!("skill/customize-blazecode.md"),
    ));
...
pub async fn discover_and_load_async(
    ...
    if !config.urls.is_empty() {
        let cache_dir = home.join(".cache").join("blazecode");
...
async fn discover_skill_files_and_load(
    ...
    // Register the built-in customize-blazecode skill first
    registry.register(Skill::builtin(
        "customize-blazecode",
        "Use ONLY when the user is editing or creating blazecode's own configuration: blazecode.json, blazecode.jsonc, files under .blazecode/, or files under ~/.config/blazecode/. Also use when creating or fixing blazecode agents, subagents, skills, plugins, MCP servers, or permission rules. Do not use for the user's own application code, or for any project that is not configuring blazecode itself.",
        include_str!("skill/customize-blazecode.md"),
...
///
/// Ported from `packages/blazecode/src/skill/index.ts` `fmt()`.
...
mod tests {
    ...
    fn test_registry_dirs_tracking() {
        ...
        let skill = Skill {
            ...
            description: None,
            location: "/tmp/.blazecode/skill/dir-skill/SKILL.md".to_string(),
    ...
    fn test_discover_skill_files_empty_dir() {
        let tmp = std::env::temp_dir().join("blazecode-skill-test-empty");
    ...
    #[test]
    fn test_discover_skill_files_with_blazecode_skill() {
        let tmp = std::env::temp_dir().join("blazecode-skill-test-blazecode");
        let _ = std::fs::remove_dir_all(&tmp);
        let skill_dir = tmp.join(".blazecode").join("skill").join("test-skill");
    ...
    #[test]
    fn test_discover_skill_files_with_blazecode_skills_plural() {
        let tmp = std::env::temp_dir().join("blazecode-skill-test-skills");
        let _ = std::fs::remove_dir_all(&tmp);
        let skill_dir = tmp.join(".blazecode").join("skills").join("pl-skill");
        std::fs::create_dir_all(&skill_dir).expect("create dirs");
        std::fs::write(
            skill_dir.join("SKILL.md"),
            "---\nname: pl-skill\n---\n\n# Body\n",
    ...
    async fn test_discover_and_load_with_valid_skill() {
        let tmp = std::env::temp_dir().join("blazecode-skill-integration");
        let _ = std::fs::remove_dir_all(&tmp);
        let skill_dir = tmp.join(".blazecode").join("skill").join("integration-test");
        std::fs::create_dir_all(&skill_dir).expect("create dirs");
        std::fs::write(
            skill_dir.join("SKILL.md"),
        ...
        assert!(registry.contains("customize-blazecode"));
    ...
    async fn test_discover_and_load_skips_invalid() {
        let tmp = std::env::temp_dir().join("blazecode-skill-invalid");
        let _ = std::fs::remove_dir_all(&tmp);
        // Create a skill with no frontmatter — should be skipped silently
        let skill_dir = tmp.join(".blazecode").join("skill").join("no-fm");
        std::fs::create_dir_all(&skill_dir).expect("create dirs");
        std::fs::write(
            skill_dir.join("SKILL.md"),
            "# No frontmatter\n\nJust content.\n",
    ...
    async fn test_discover_and_load_with_external_skills() {
        let tmp = std::env::temp_dir().join("blazecode-skill-external");
        let _ = std::fs::remove_dir_all(&tmp);
    ...
    async fn test_discover_and_load_external_disabled() {
        let tmp = std::env::temp_dir().join("blazecode-skill-ext-disabled");
        let _ = std::fs::remove_dir_all(&tmp);
    ...
    fn test_discover_skill_files_deeply_nested() {
        let tmp = std::env::temp_dir().join("blazecode-skill-deep-nested");
        let _ = std::fs::remove_dir_all(&tmp);
        let skill_dir = tmp
            .join(".blazecode")
            .join("skill")
    ...
    fn test_discover_skill_files_extra_paths() {
        let tmp = std::env::temp_dir().join("blazecode-skill-extra-paths");
        let _ = std::fs::remove_dir_all(&tmp);
        ...
        let worktree = std::env::temp_dir().join("blazecode-skill-extra-wt");
    ...
    fn test_glob_home_skills_empty_dir() {
        let tmp = std::env::temp_dir().join("blazecode-home-empty");
        let _ = std::fs::remove_dir_all(&tmp);
    ...
    fn test_glob_home_skills_nonexistent_dir() {
        let nonexistent = std::path::Path::new("/tmp/blazecode-definitely-does-not-exist-98765");
