...
impl TimelineState {
    ...
    pub fn build_from_messages(&mut self, messages: &[rustcode_core::session::Message]) {
        self.nodes.clear();
    ...
    fn preview_from_parts(parts: &[rustcode_core::session::Part], max_len: usize) -> String {
        for part in parts {
            if let rustcode_core::session::Part::Text(tp) = part {
