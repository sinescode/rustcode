...
//! MCP (Model Context Protocol) integration for blazecode.
//!
//! Ported from: `packages/blazecode/src/mcp/`
//!
//! This crate provides:
//!
//! - **Re-exports** of core MCP types from [`blazecode_core::mcp`] for convenience.
...
//!   BlazeCode config, and environment variables.
//!
//! ## Architecture
//!
//! The core MCP types and the [`McpClient`] / [`McpServerRegistry`] live in
//! `blazecode-core` (`blazecode_core::mcp`). This crate adds transport
//! abstractions and tool-execution convenience wrappers on top.

use async_trait::async_trait;
use blazecode_core::error::{Error, Result};
use blazecode_core::mcp::McpClient;
use blazecode_core::tool::{ExecuteResult, PluginToolDef, ToolContext};
...
// ---------------------------------------------------------------------------
// Re-exports from blazecode_core::mcp
// ---------------------------------------------------------------------------

pub use blazecode_core::mcp::{
...
///
/// # Source
/// Ported from `packages/blazecode/src/mcp/index.ts` — the `connectRemote()`
...
/// Wraps an [`McpClient`] to execute a single tool as a blazecode tool.
///
...
///
/// # Source
/// Ported from `packages/blazecode/src/mcp/index.ts` — the tool dispatch
...
impl McpToolExecutor {
    ...
    /// [`ToolRegistry`](blazecode_core::tool::ToolRegistry).
    ///
...
/// Configuration format used by BlazeCode's `.blazecode/config.json`.
///
/// The file format is:
/// ```json
/// {
...
///
/// # Source
/// Ported from `packages/blazecode/src/mcp/index.ts` — config loading
/// and `packages/blazecode/src/config/config.ts` MCP section parsing.
...
impl McpDiscovery {
    ...
    /// Parse MCP server configs from an BlazeCode project config file.
    ///
    /// BlazeCode stores MCP servers in `.blazecode/config.json` under the
    /// `mcp` key. Each entry is a full [`McpServerConfig`].
    ///
    /// # Errors
    /// Returns an error if the file cannot be read or parsed.
    pub fn from_blazecode_config(path: &Path) -> Result<Vec<McpServerConfig>> {
        let content = std::fs::read_to_string(path).map_err(|e| Error::FileSystem {
            path: path.display().to_string(),
            message: format!("failed to read BlazeCode config: {e}"),
        })?;

        let parsed: BlazeCodeMcpSection = serde_json::from_str(&content).map_err(|e| {
            Error::Config(format!(
                "failed to parse BlazeCode config `{}`: {e}",
                path.display()
            ))
        })?;
        ...
        debug!(
            "Discovered {} MCP servers from BlazeCode config",
            configs.len()
        );
        Ok(configs)
    }
...
mod tests {
    ...
    // ── McpDiscovery: BlazeCode config ─────────────────────────────────

    #[test]
    fn test_parse_blazecode_config_empty() {
        let json = r#"{"mcp": {}}"#;
        let parsed: BlazeCodeMcpSection = serde_json::from_str(json).expect("valid");
        assert!(parsed.mcp.is_empty());
    }

    #[test]
    fn test_parse_blazecode_config_local_server() {
        let json = r#"{
            "mcp": {
                "my-server": {
                    "type": "local",
    ...
    #[test]
    fn test_parse_blazecode_config_remote_server() {
        let json = r#"{
            "mcp": {
                "http-server": {
                    "type": "remote",
